...
local nPosCpoMrc	:= 0
local nPosCpoNum	:= 0
local cMarca		:= If(SM0->M0_CODIGO == "20","A.MOTION","PELMEX")//Acrescentado por Ulisses Jr em 28/07/10
Local __xMPed       := ""					 
local cNumeracao	:= ""
local cCnpjPart	    := ""
Local lVinEstDev	:= .F.
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							ElseIf !(IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, ALLTRIM(SC5->C5_MENNOTA)+" "+ALLTRIM(SC5->C5_XMSGNOT)+" "+ALLTRIM(SC5->C5_OBS1)+" "+ALLTRIM(SC5->C5_OBS2)+" "+ALLTRIM(SC5->C5_OBS3),ALLTRIM(SC5->C5_MENNOTA)+" "+ALLTRIM(SC5->C5_XMSGNOT)+" "+ALLTRIM(SC5->C5_OBS1)+" "+ALLTRIM(SC5->C5_OBS2)+" "+ALLTRIM(SC5->C5_OBS3)) $ cMensCli)
								cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, ALLTRIM(SC5->C5_MENNOTA)+" "+ALLTRIM(SC5->C5_XMSGNOT)+" "+ALLTRIM(SC5->C5_OBS1)+" "+ALLTRIM(SC5->C5_OBS2)+" "+ALLTRIM(SC5->C5_OBS3),ALLTRIM(SC5->C5_MENNOTA)+" "+ALLTRIM(SC5->C5_XMSGNOT)+" "+ALLTRIM(SC5->C5_OBS1)+" "+ALLTRIM(SC5->C5_OBS2)+" "+ALLTRIM(SC5->C5_OBS3))
							EndIf
							
						EndIf
						If Empty(__xMPed)   
							__xMPed := Alltrim(SC5->C5_XMSGNOT) 
							If !(__xMPed $ cMensCli)
								cMensCli += __xMPed
							EndIf
						...
						cEspNota := ""
						nFatConv := 0
						// Utilizado em substituição da linha acima por Ulisses Jr em 28/07/10.
						If SC5->C5_DESC1 > 0
							cStrDesc := "DESCONTO PROMOCIONAL DE "+Alltrim(str(SC5->C5_DESC1,5,2))+"%"
							If SC5->C5_DESC2 > 0
								cStrDesc += " + "+Alltrim(Str(SC5->C5_DESC2,5,2))+"%" 
								If SC5->C5_DESC3 > 0
									cStrDesc += " + "+Alltrim(Str(SC5->C5_DESC3,5,2))+"%"
									If SC5->C5_DESC4 > 0
										cStrDesc += " + "+Alltrim(Str(SC5->C5_DESC4,5,2))+"%"
									EndIf
								EndIf
							EndIf
						EndIf
						If( SC5->C5_DCR == "S")
							aAdd(axInfAdProd,"DCR Nro. - " + Alltrim(SB1->B1_DCR) )
						EndIf
						If !Empty(SF4->F4_MENS1)  .And. !AllTrim(SF4->F4_MENS1) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(SF4->F4_MENS1)
						EndIf

						cxNFOri   := If(Empty((cAliasSD2)->D2_NFORI),"",(cAliasSD2)->D2_NFORI+", DATA "+Dtoc(SD1->D1_EMISSAO))

						If !Empty(cxNFOri)  .And. !AllTrim(cxNFOri) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(cxNFOri)
						EndIf

						If !Empty(SF4->F4_MENS2)  .And. !AllTrim(SF4->F4_MENS2) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(SF4->F4_MENS2)
						EndIf

						If !Empty(SF4->F4_MENS3)  .And. !AllTrim(SF4->F4_MENS3) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(SF4->F4_MENS3)
						EndIf

						If !Empty(SC5->C5_NTCONS)  .And. !AllTrim(SC5->C5_NTCONS + " do dia ") $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(SC5->C5_NTCONS + " do dia ")
						EndIf

						SX5->(dbSetOrder(1))
						If SX5->(dbSeek(XFILIAL("SX5")+"Z2"+SC5->C5_VIAT))   // Alterado pelo Ener
							cxTransp := If(SF2->(xFilial("SF2")) <> '02',"Via de Transporte: "+Left(SX5->X5_DESCRI,15),"")

							if !Empty(cxTransp) .And. !AllTrim(cxTransp) $ cMensFis
								If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
									cMensFis += " "
								EndIf
								cMensFis += Alltrim(cxTransp)
							EndIf

						EndIf

						cxCodCli := "Codigo do Cliente: "+SA1->A1_COD
						if !Empty(cxCodCli) .And. !AllTrim(cxCodCli) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += Alltrim(cxCodCli)
						EndIf

						cxMensIpi := "ISENTO DE IPI - PRODUZIDO NA ZONA FRANCA DE MANAUS"

						If SC5->C5_DCR == "S"
							if !Empty(cxMensIpi) .And. !AllTrim(cxMensIpi) $ cMensFis
								If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
									cMensFis += " "
								EndIf
								cMensFis += Alltrim(cxMensIpi)
							EndIf
						EndIf

						if Len(Alltrim(SC5->C5_VEND1)) > 0
							cxVendInf := "Vendedor: "+SC5->C5_VEND1
							if !Empty(cxVendInf) .And. !AllTrim(cxVendInf) $ cMensFis
								If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
									cMensFis += " "
								EndIf
								cMensFis += Alltrim(cxVendInf)
							EndIf
						EndIf

						If Len(Alltrim( (cAliasSD2)->D2_PEDIDO) ) > 0
							cxPedVen := "Pedido: "
							if !Empty(cxPedVen) .And. !AllTrim(cxPedVen) $ cMensFis
								If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
									cMensFis += " "
								EndIf
								cMensFis += Alltrim(cxPedVen)
							EndIf

							If !Alltrim((cAliasSD2)->D2_PEDIDO) $ cMensFis
								cMensFis += If(Len(cMensFis) > 0, ", ","")+Alltrim((cAliasSD2)->D2_PEDIDO)
							EndIf
...
//PELMEX			  
						If lNfCup //Customização
							aAreaSD2  	:= SD2->(GetArea())
							//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
							//³Pesquisa itens de nota                                                  ³
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ ÄÄÄÄÙ
							DbSelectArea("SD2")
							DbSetOrder(3)
						...
						Else
							...
							//³Quando nao for cupom fiscal,							³
							//³	o CFOP deve ser atualizado com o CFOP de cada ITEM, |
							//³	e nao com o CFOP do primeiro item                   |
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
							cD2Cfop := (cAliasSD2)->D2_CF

						EndIf
							/* Trecho retirado por Ulisses Junior em 14/07/10 para buscar dados informados pelo usuário direto da nota de saída
							// WERMESON 
							nPLiq  := iIf( SC5->C5_PESOL  > 0 , SC5->C5_PESOL  , nPLiq  + ((cAliasSD2)->D2_QUANT * SB1->B1_PESO) )
							nPBrut := iIf( SC5->C5_PBRUTO > 0 , SC5->C5_PBRUTO , nPBrut + ((cAliasSD2)->D2_QUANT * SB1->B1_PESBRU) )


							If SC6->C6_XVOLUME > 0 
								nwVol += SC6->C6_XVOLUME
							ElseIf SC5->C5_VOLUME1 > 0 
								nwVol += SC5->C5_VOLUME1

							Else 
								nwVol += (cAliasSD2)->D2_QUANT
							...
							mEspecie:="M3"
							Case SB1->B1_UM=="KG"
							mEspecie:="KG" // .and. nFator2 > 0 //Diego - 16/10/09
							Case SB1->B1_UM=="VL"
							mEspecie:="VOLUMES"
							Case SB1->B1_UM=="BL"
							mEspecie:="BLOCO"
						...
						// Acrescentado por Ulisses Jr em 30/07/10  
						cProdCli  := Posicione("SA7",2,xFilial("SA7")+SB1->B1_COD+SF2->F2_CLIENTE+SF2->F2_LOJA,"A7_CODCLI")

						cDescProd := If(Empty(SC6->C6_DESCRI),ALLTRIM(SB1->B1_DESC),ALLTRIM(SC6->C6_DESCRI))+If(!Empty(SB1->B1_COR)," Cor-","")+Alltrim(SB1->B1_COR)
						cDescProd += If(SC5->C5_DCR == "S" .And. !Empty(alltrim(SB1->B1_DCR)), "-DCR: "+Alltrim(SB1->B1_DCR),"")
						cDescProd += If(!Empty(cProdCli)," Cod. Cli.: ","")+Alltrim(cProdCli)
						cDescProd += If(!Empty(SB1->B1_CODBAR)," EAN: ","")+Alltrim(SB1->B1_CODBAR)	
			            ...
			            Else 
							...
							If	SD2->(FieldPos("D2_DESCICM"))<>0
								...
								If SF4->F4_AGREG == "D" .and.  (!Empty(SF4->F4_MOTICMS) .and. (AllTrim(SF4->F4_MOTICMS) $ "3-8-9" .or.  AllTrim(SF4->F4_MOTICMS) =='90')) .and. Empty(SF4->F4_CSOSN)
									nDescIcm:=0
								EndIF
							EndIF
			            EndIf
...
//PELMEX
						...
						*/
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							ElseIf SFT->(FieldPos("FT_TRFICM")) <> 0 .And. SFT->FT_TRFICM <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) $ "RS/GO/PB/BA/MS"
...
//PELMEX
					    Elseif (Alltrim((cAliasSD2)->D2_CF) $ "6922") .OR. (Alltrim((cAliasSD2)->D2_CF) $ "5922")
					    	cMensFis += " " 
					    Else
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*/
				...
				If SD1->(FieldPos("D1_DESCICM"))<>0
					...
					If SF4->F4_AGREG == "D" .and. (!Empty(SF4->F4_MOTICMS) .and. (!AllTrim(SF4->F4_MOTICMS) $ "8|9" .or. AllTrim(SF4->F4_MOTICMS) != "90")) .and. Empty(SF4->F4_CSOSN)
						nDescIcm:=0
					EndIF						
				EndIF
				...
				If lEECFAT
					...
					*/
					...
					Else
						aadd(aExp,{})
					EndIf
				ElseiF AliasIndic("CDL") .and. !Empty( (cAliasSD1)->D1_NFORI ) .and. ( (cAliasSD2)->D2_DOC == (cAliasSD1)->D1_NFORI ) .and. ( (cAliasSD2)->D2_SERIE == (cAliasSD1)->D1_SERIORI )
					aadd(aExp,{})
					DbSelectArea("CDL")
					DbSetOrder(1)
					DbSeek(xFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
				...
				*/
				...
				Else				
					...
					+ IIF(((cAliasSD1)->D1_TIPO $"IP" .Or. ((cAliasSD1)->D1_TIPO == "D" .And. cTpOrig == "P")),0,(cAliasSD1)->D1_VALIPI)+(cAliasSD1)->D1_ICMSRET;      
...
IF lPe01Nfe     
	...
	//PELMEX
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aICMS,aIPI,aPIS,aCOFINS} 
	...
	If ( Len(aParam) >= 5 )
		...
		If ( Len(aParam) >= 17)
			aObsCont    := aParam[17]
		EndIf	
		aICMS		:= aParam[18]	//Aglutinaçao ICMS - Bruno Garcia
		aIPI		:= aParam[19]	//Aglutinaçao IPI - Bruno Garcia
		aPIS		:= aParam[20]	//Aglutinaçao PIS - Bruno Garcia 
		aCOFINS		:= aParam[21]	//Aglutinaçao COFINS - Bruno Garcia
	EndIf
Endif 
...
If Len(aPISST)>0
	...
	cString += If(retNT2005(), '<indSomaPISST>'+ Iif(aPISST[07] == "1", ConvType(aPISST[07],1), "0") +'</indSomaPISST>', '')
	cString += '</Tributo>'
	cString += '</imposto>'
	nValPis += aPISST[04]
EndIf
...
If Len(aCOFINSST)>0
	...
	cString += If(retNT2005(), '<indSomaCOFINSST>'+ Iif(aCOFINSST[07] == "1", ConvType(aCOFINSST[07],1), "0") +'</indSomaCOFINSST>', '')
	cString += '</Tributo>'
	cString += '</imposto>'
...
Local cChave	:= ""  
Local lAtuSC5	:= .F. 
Local cMsgNfs  := ALLTRIM(SC5->C5_MENNOTA)+" "+ALLTRIM(SC5->C5_XMSGNOT)+" "+ALLTRIM(SC5->C5_OBS1)+" "+ALLTRIM(SC5->C5_OBS2)+" "+ALLTRIM(SC5->C5_OBS3)
...
IF len(aDetPag) > 0  
	...
	For nX := 1 To Len(aDetPag)
		...
		if aDetPag[nX][1] == "99" .And. len(aDetPag[nX]) >= 9
			cString +='<xPag>' + "Negociação Futura" + '</xPag>'
...
Do Case
	...
	Case ((aNota[5]) $ "I-D-C-B" .And. SF4->F4_AJUSTE == "S") .or. ( len(aNfVinc)>=1 .and. aNfVinc[1][10] == "B"  .and. aNota[5] == "N"  .and. SF4->F4_PODER3 == "D"  .and. SF4->F4_AJUSTE == "S")	
   		cTPNota:= "3"
...
Retorna o conteúdo do parâmetro MV_NT2005 
