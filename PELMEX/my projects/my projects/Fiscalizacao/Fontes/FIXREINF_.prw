...
para os tùtulos lanùados no sistema antes da vigùncia do bloco 40
e que serùo baixados apùs o inicio da vigùncia (Set/2023).
/*/
//-------------------------------------------------------------------
User Function FIXREINF()
    ...
    Private cDtIni      := "20211231" //Data inicial a ser considera para filtrar a emissùo dos tùtulos (Formato AAAAMMDD)
...
Static Function REINFLOG()
    ...
    aAdd(aStruct,{"EMPFIL"      ,"C", 008, 00}) //Empresa/filial do documento/tùtulo
    aAdd(aStruct,{"DATAPROC"    ,"C", 020, 00}) //Data e hora do processamento
    aAdd(aStruct,{"TIPO"        ,"C", 002, 00}) //DE=Documento de entrada / DS = Documento de Saùda / CP = Contas a Pagar / CR = Contas a Receber
    aAdd(aStruct,{"CHAVE"       ,"C", 220, 00}) //Chave do documento/tùtulo
    aAdd(aStruct,{"FATC6"       ,"C", 001, 00}) //Identifica se foi atualizada natureza de rendimento no pedido de venda
    aAdd(aStruct,{"FATFKW"      ,"C", 001, 00}) //Identifica se foi criado o registro na FKW referente ao documento de saùda
    ...
    aAdd(aStruct,{"FINFKF"      ,"C", 001, 00}) //Identifica se foi atualizado o registro na tabela FKF referente ao tùtulo avulso
    aAdd(aStruct,{"FINFKW"      ,"C", 001, 00}) //Identifica se foi criado o registro na tabela FKW referente ao tùtulo avulso
...
/*  
    Inicio das funùùes de FIX do mùdulo SIGACOM
    que abrange a gravaùùo da FKW para os 
...
Static Function FIXDHR(aDadosDoc)
    ...
    For nI := 1 To Len(aDadosDoc)
            ...
            If Len(aAuxIt) > 0 .And. Len(aAuxNat) > 0 .And. Len(aAuxPrd) > 0
                ...
                For nX := 1 To Len(aAuxIt)
                    ...
                    If SD1->(DbSeek(aDadosDoc[nI,1] + aDadosDoc[nI,2] + aDadosDoc[nI,3] + aDadosDoc[nI,4] + aDadosDoc[nI,5] + aAuxPrd[nX] + aAuxIt[nX]))
                        A103INCDHR(aHeadDHR,aColsDHR,nX,.F.) //Gravaùùo DHR
                        lGeraDHR := .T.
                    Endif
                Next nX
            Endif
        ...
        If Len(aFKW) > 0
            ...
            A103FKW("I",aFKW[1],aFKW[2]) //Gravaùùo FKW
            lGeraFKW    := .T.
        Endif
...
/*  
    Inicio das funùùes de FIX do mùdulo SIGAFIN
    que abrange a gravaùùo da FKW para os 
    tùtulos a pagar/recebe sem vinculo com 
    notas (doc. entrada/saùda)
*/
...
Static Function FIXFINREINF(aFiliais As Array)
    ...
    Else   
        Help(" ", 1, "ATUAMBREINF", Nil, "Ambiente desatualizado", 2, 0, Nil, Nil, Nil, Nil, Nil, {"Para realizar o ajuste da base, ù necessùrio atualizar o ambiente"})
    EndIf
...
/*/{Protheus.doc} FinCPag    
    @type User Function   
    @param aFiliais, array unidimensional, lista de filais que serùo processadas
    @return Logical, lRetorno, Logico que indica se ocorreu o processamento de atualizaùùo
    da natureza de rendimento do tùtulos a pagar
/*/
    ...
    While (cTblPagar)->(!Eof())
        ...
        For nTpImpos := 1 To 5                
            Do Case
                ...
                OtherWise // Tùtulos sem impostos
                    If Len(aDados) == 0
                        cTpImpos   := "SEMIMP"
                        nVlrImpos  := 0
...
/*/{Protheus.doc} FinCRec    
    ...
    @param aFiliais, array unidimensional, lista de filais que serùo processadas
    @return Logical, lRetorno, Logico que indica se ocorreu o processamento de atualizaùùo
    da natureza de rendimento do tùtulos a receber
/*/
...
/*  
    Inicio das funùùes de FIX do mùdulo SIGAFAT 
    que abrange a gravaùùo da FKW para os 
    Documentos de Saùda e atualizaùùo do campo 
