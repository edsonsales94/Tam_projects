...
TRCell():New(oSection1,'B1_COD'		,cAliasSB1	,STR0027				,/*Picture*/,TamSx3("B1_COD")[1],/*lPixel*/,/*{|| code-block de impressao }*/)
...
TRFunction():New(oSection1:Cell('QUANTDATA'	),NIL,"MAX",oBreak01,/*Titulo*/,/*cPicture*/,/*uFormula*/,.F.,.F.)
...
BeginSql Alias cAliasSB1
	...
	SELECT 	SB1.R_E_C_N_O_ SB1REC , SB1.B1_FILIAL, SB1.B1_COD, SB1.B1_LOCPAD, SB1.B1_TIPO, 
    	    ...
    	    SB2.B2_FILIAL, SB2.B2_COD, SB2.B2_LOCAL, SB2.B2_DINVENT,
        	SB7.B7_FILIAL, SB7.B7_COD, SB7.B7_LOCAL, SB7.B7_DATA,SB7.B7_LOCALIZ,
	        SB7.B7_NUMSERI, SB7.B7_LOTECTL, SB7.B7_NUMLOTE, SB7.B7_DOC, SUM(SB7.B7_QUANT) B7_QUANT,
	...
	GROUP BY SB1.R_E_C_N_O_ , SB1.B1_FILIAL, SB1.B1_COD, SB1.B1_LOCPAD, SB1.B1_TIPO, 
	        SB1.B1_GRUPO, SB1.B1_DESC, SB1.B1_UM, SB1.B1_CODITE, SB2.R_E_C_N_O_,
    	    SB2.B2_FILIAL, SB2.B2_COD, SB2.B2_LOCAL, SB2.B2_DINVENT,
        	SB7.B7_FILIAL, SB7.B7_COD, SB7.B7_LOCAL, SB7.B7_DATA,SB7.B7_LOCALIZ,
	        SB7.B7_NUMSERI, SB7.B7_LOTECTL, SB7.B7_NUMLOTE, SB7.B7_DOC,
	        SB7.B7_ESCOLHA, SB7.B7_CONTAGE ORDER BY %Exp:cOrderBy%
...
While !oReport:Cancel() .And. !(cAliasSB7)->(Eof())
	...
	If nSB7Cnt > 0 .AND.(lImprime .Or. mv_par14 == 3)
		...
		If lFirst
			...
			oSection1:Cell('DIFQUANT'	):SetValue(SB7->B7_QUANT - aSaldo[1])
			oSection1:Cell('DIFVALOR'	):SetValue((SB7->B7_QUANT - aSaldo[1])*aCM[mv_par10])
		Else
...
cQuery    += "SB7.B7_FILIAL, SB7.B7_COD, SB7.B7_LOCAL, SB7.B7_DATA, SB7.B7_LOCALIZ, SB7.B7_NUMSERI, SB7.B7_LOTECTL, SB7.B7_NUMLOTE, SB7.B7_DOC, SUM(SB7.B7_QUANT) B7_QUANT "
If lContagem
...
EndIf
cQuery    += 'GROUP BY SB1.R_E_C_N_O_ , SB1.B1_FILIAL, SB1.B1_COD, SB1.B1_LOCPAD, SB1.B1_TIPO,'+;  
             'SB1.B1_GRUPO, SB1.B1_DESC, SB1.B1_UM, SB1.B1_CODITE, SB2.R_E_C_N_O_,'+;
    	      'SB2.B2_FILIAL, SB2.B2_COD, SB2.B2_LOCAL, SB2.B2_DINVENT,'+;
             'SB7.B7_FILIAL, SB7.B7_COD, SB7.B7_LOCAL, SB7.B7_DATA,SB7.B7_LOCALIZ,'+;
             'SB7.B7_NUMSERI, SB7.B7_LOTECTL, SB7.B7_NUMLOTE, SB7.B7_DOC,'+;
...
While &cCondicao
	...
	IncRegua()
		...
		While !(cAliasSB7)->(Eof()) .And. (cAliasSB7)->(B7_FILIAL+DtoS(B7_DATA)+B7_COD+B7_LOCAL) == xFilial('SB7')+DtoS(mv_par03)+cProduto+cLocal
			...
			nSB7Cnt  := 0
			aRegInv  := {}
			cSeek    := xFilial('SB7')+DtoS(mv_par03)+(cAliasSB7)->B7_COD+(cAliasSB7)->B7_LOCALIZ+(cAliasSB7)->B7_NUMSERI+(cAliasSB7)->B7_LOTECTL+(cAliasSB7)->B7_NUMLOTE
			cCompara := "B7_FILIAL+DTOS(B7_DATA)+B7_COD+B7_LOCALIZ+B7_NUMSERI+B7_LOTECTL+B7_NUMLOTE"
			cLocaliz := (cAliasSB7)->B7_LOCALIZ
			cNumSeri := (cAliasSB7)->B7_NUMSERI
			cLoteCtl := (cAliasSB7)->B7_LOTECTL
			cNumLote := (cAliasSB7)->B7_NUMLOTE
			...
			While !(cAliasSB7)->(Eof()) .And. cSeek == (cAliasSB7)->&(cCompara)				
				...
				aRetSal := CalcEst((cAliasSB7)->B7_COD,(cAliasSB7)->B7_LOCAL,mv_par03+1)

				aAdd(aSaldoIt,aRetSal[1])
			...
			If lImprime .Or. mv_par14 == 3
				...
				If mv_par12 == 1
						aCM:={}
				If QtdComp(aRetSal[1]) > QtdComp(0)
					For i:=2 to Len(aRetSal)
						AADD(aCM,aSaldo[i]/aRetSal[1])
					Next i
        	    Else
						aCm := PegaCmAtu(cProduto,cLocal)
	           EndIf
               Else
				...
				EndIf
					If mv_par11 == 1  
						...
						@ Li,If(mv_par13==1,129,092) + nCOL1 PSAY Transform(aRegInv[nX,12], (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19))) //B7_QUANT
						@ Li,If(mv_par13==1,149,110) + nCOL1 PSAY Transform(aSaldoIt[nX], (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,169,130) + nCOL1 PSAY Transform((aRegInv[nX,12] - aSaldoIt[nX]), (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,188,150) + nCOL1 PSAY Transform((aRegInv[nX,12] - aSaldoIt[nX])*aCM[mv_par10],(cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19))) //VALOR
					Else
						If mv_par13 == 1
						...
						@ Li,If(mv_par13==1,110,073) + nCOL1 PSAY Transform(aRegInv[nX,12], (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19))) //B7_QUANT
						@ Li,If(mv_par13==1,130,94)  + nCOL1 PSAY Transform(aSaldoIt[nX], (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,152,112) + nCOL1 PSAY Transform((aRegInv[nX,12] - aSaldoIt[nX]), (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,171,133) + nCOL1 PSAY Transform((aRegInv[nX,12] - aSaldoIt[nX])*aCM[mv_par10],(cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19))) //VALOR						
					EndIf
		
					Li++
					nTotalQtd  += (aRegInv[nX,12])
					If Nx == 1
						nTotalInv  +=  aSaldoIt[nX]
					EndIf
					nTotalDif  += (aRegInv[nX,12]) - aSaldoIt[nX]
					nTotalVl   += ((aRegInv[nX,12]) - aSaldoIt[nX])*aCM[mv_par10] 
				Next nX
				...
				nCntImpr++
					...
					If (nSB7Cnt == 1 .and. (Len(aRegInv)== 1)) .or.  (nSB7Cnt > 1 .and. (Len(aRegInv) > 1))
					 Li++
					If mv_par11 == 1
						@ Li,If(mv_par13==1,102,065) + nCOL1 PSAY STR0017 // 'TOTAL .................'
						@ Li,If(mv_par13==1,129,092) + nCOL1 PSAY Transform(nTotal, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
					Else
						@ Li,If(mv_par13==1,084,047) + nCOL1 PSAY STR0017 // 'TOTAL .................'
						@ Li,If(mv_par13==1,110,073) + nCOL1 PSAY Transform(nTotal, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
					EndIf
				EndIf
				   nSaldoT := 0
				For i:=1 to len(aSaldoIt)				
				  nSaldoT := aSaldoIt[i]
			 	Next
				If mv_par11 == 1				 			
					@ Li,If(mv_par13==1,149,110) + nCOL1 PSAY Transform(nSaldoT, (cAliasSB2)->(PesqPict("SB2",'B2_QFIM', 19)))
				Else
					@ Li,If(mv_par13==1,130,094) + nCOL1 PSAY Transform(nSaldoT, (cAliasSB2)->(PesqPict("SB2",'B2_QFIM', 19)))
				EndIf
				...
				If nSB7Cnt > 0
					...
					If mv_par11 == 1 
						@ Li,If(mv_par13==1,169,130) + nCOL1 PSAY Transform(nTotalDif, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,188,150) + nCOL1 PSAY Transform(nTotalDif*aCM[mv_par10], (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19)))
					Else
						@ Li,If(mv_par13==1,152,112) + nCOL1 PSAY Transform(nTotalDif, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
						@ Li,If(mv_par13==1,171,133) + nCOL1 PSAY Transform(nTotalDif*aCM[mv_par10], (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19)))
					EndIf
					nTotVal += nTotalDif
					nSubVal += nTotalVl
				EndIf
	...
	If aReturn[8] == 2
		If cAnt # B1_TIPO .And. nSB7Cnt >= 1
			If mv_par11 == 1
				...
				@ Li,If(mv_par13==1,188,151) + nCOL1 PSAY Transform(nSubVal, (cAliasSB2)->(PesqPict("SB2",'B2_QFIM', 19)))
			Else     
				@ Li,If(mv_par13==1,142,098) + nCOL1 PSAY STR0018 + Left(cAnt,2) + ' ............' // 'TOTAL DO TIPO '
				@ Li,If(mv_par13==1,171,133) + nCOL1 PSAY Transform(nSubVal, (cAliasSB2)->(PesqPict("SB2",'B2_QFIM', 19)))
			EndIf
			cAnt    := B1_TIPO
			nSubVal := 0
			Li += 2
...
If nTotVal # 0
	Li++
	If mv_par11 == 1
		@ Li,If(mv_par13==1,085,050) + nCOL1 PSAY STR0020 // 'TOTAL DAS DIFERENCAS EM VALOR .............'
		@ Li,If(mv_par13==1,188,148) + nCOL1 PSAY Transform(nSubVal, (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19)))
		
	Else
		@ Li,If(mv_par13==1,070,050) + nCOL1 PSAY STR0020 // 'TOTAL DAS DIFERENCAS EM VALOR .............'
		@ Li,If(mv_par13==1,171,133) + nCOL1 PSAY Transform(nSubVal, (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19)))
	EndIf
	Li++
EndIf
Li++
If mv_par11 == 1 
   @ Li,If(mv_par13==1,102,065) + nCOL1 PSAY STR0049 // 'T o t a l  G e r a l :'
   @ Li,If(mv_par13==1,129,092) + nCOL1 PSAY Transform(nTotalQtd, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
   @ Li,If(mv_par13==1,149,110) + nCOL1 PSAY Transform(nTotalInv, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19))) 
   @ Li,If(mv_par13==1,169,130) + nCOL1 PSAY Transform(nTotVal, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
   @ Li,If(mv_par13==1,188,150) + nCOL1 PSAY Transform(nSubVal ,(cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19))) //VALOR
Else 
   @ Li,If(mv_par13==1,084,047) + nCOL1 PSAY STR0049 // 'T o t a l  G e r a l :'
   @ Li,If(mv_par13==1,110,073) + nCOL1 PSAY Transform(nTotalQtd, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
   @ Li,If(mv_par13==1,130,094) + nCOL1 PSAY Transform(nTotalInv, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19))) 
   @ Li,If(mv_par13==1,152,112) + nCOL1 PSAY Transform(nTotVal, (cAliasSB7)->(PesqPict("SB7",'B7_QUANT', 19)))
   @ Li,If(mv_par13==1,171,133) + nCOL1 PSAY Transform(nSubVal ,(cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 19))) //VALOR
EndIf
If Li # 80
