...
User Function PLESTP03()
	Local oDlg, aCabec, oMul, cVar2, aPosObj, aSize
	...
	DEFINE MSDIALOG oDlg TITLE cCadastro From aSize[7],0 TO aSize[6],990 PIXEL OF oMainWnd
	
	@ aPosObj[1,1]-0,aPosObj[1,2]+000 SAY "Nota Fiscal"          SIZE 40,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[1,1]-1,aPosObj[1,2]+050 MSGET oDoc   VAR cNumNota  Picture "@!" F3 "SF1NFE" VALID BuscaNota(@cSeek,@lTudoLido) SIZE 60,10 PIXEL OF oDlg FONT oFont2 WHEN lDigita
	@ aPosObj[1,1]-0,aPosObj[1,2]+120 SAY "Serie"                SIZE 60,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[1,1]-1,aPosObj[1,2]+150 MSGET oSer   VAR cSerNota  Picture "@!" /*VALID CodigoBarras()*/ SIZE 20,10 PIXEL OF oDlg FONT oFont2 WHEN lDigita
	@ aPosObj[1,1]-0,aPosObj[1,2]+185 SAY "Emissão"              SIZE 60,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[1,1]-0,aPosObj[1,2]+220 SAY oEmi     VAR dEmissao  Picture "@!" SIZE 40,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HRED
	@ aPosObj[1,1]-0,aPosObj[1,2]+260 SAY "Fornecedor"           SIZE 60,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[1,1]-0,aPosObj[1,2]+305 SAY oFor     VAR cFornece  Picture "@!" SIZE 120,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HRED
	
	oLbx1 := TWBrowse():New(aPosObj[1,1]+15,002,460,aPosObj[1,3]-aPosObj[1,1],,aCabec,,oDlg,,,,{|| MostraItem() },,,oFont1,,,,,,,.T.)
	...
	@ aPosObj[2,1]+16,aPosObj[2,2]+000 SAY "Item"  SIZE 20,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[2,1]+16,aPosObj[2,2]+025 SAY oIte VAR cItem SIZE 150,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HRED
	
	@ aPosObj[2,1]+16,aPosObj[2,2]+308 SAY "Quantidade"  SIZE 40,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[2,1]+14,aPosObj[2,2]+353 MSGET oQtd   VAR nQuant  Picture cPictQtd VALID Vazio() .Or. Positivo() SIZE 75,10 PIXEL OF oDlg FONT oFont2 WHEN !lTudoLido
	
	@ aPosObj[2,1]+16,aPosObj[2,2]+430 SAY "X"  SIZE 10,10 PIXEL OF oDlg FONT oFont2 COLOR CLR_HBLUE
	@ aPosObj[2,1]+14,aPosObj[2,2]+438 MSGET oMul   VAR nMult  Picture "@E 999" VALID AddQuantidade() SIZE 20,10 PIXEL OF oDlg FONT oFont2 WHEN !lTudoLido
	
	@ aPosObj[2,1]+30,aPosObj[2,2]+000 LISTBOX oLbx2 VAR cVar2 FIELDS HEADER "","Volume","Lote","Quantidade" SIZE 460,aPosObj[2,3]-aPosObj[2,1]-30 OF oDlg PIXEL FONT oFont1 ;
	...
	DEFINE SBUTTON FROM aPosObj[2,1]+31,466 TYPE 3 ENABLE ACTION ApagaVolume() OF oDlg WHEN !lDigita .And. !lTudoLido
	DEFINE SBUTTON FROM aPosObj[2,1]+46,466 TYPE 6 ENABLE ACTION ImprimeVolume() OF oDlg WHEN !lDigita
	
	@ aPosObj[3,1],aPosObj[3,2] CHECKBOX oCbx VAR lCheck PROMPT "Marcar / Desmarcar" SIZE 60,15 OF oDlg PIXEL ON;
...
Static Function BuscaNota(cSeek,lTudoLido)
	...
	If lRet
		...
		While !SD1->(Eof()) .And. cSeek == SD1->(D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA)
			AAdd( vItens , { SD1->D1_ITEM, SD1->D1_COD, PADR(Posicione("SB1",1,XFILIAL("SB1")+SD1->D1_COD,"B1_DESC"),50), SD1->D1_UM, SD1->D1_QUANT, 0, SD1->D1_NUMSEQ, lCheck} )
			SD1->(dbSkip())
		Enddo
...
Static Function Gravacao(cSeek)
	...
	Private nQtde, cOP, cOPReq, cNumSerie, nResto
