...
ฑฑณPrograma  ณ PEFINR02ณ Autor ณ Microsiga             ณ Data ณ 26/01/23 ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescri…o ณ IMPRESSAO DO BOLETO DO BANCO SANTANDER                     ณฑฑ
...
Tamanho  := "M"
titulo   := "Boleto do Banco Santander"
...
wnrel    := "ARFINR01"
lEnd     := .F.
cPerg    := PADR("PEFINR02",Len(SX1->X1_GRUPO))     
...
dbSelectArea("SX1") 
dbSetOrder(1)
If dbSeek(cPerg+"11")   // Limpo o conteudo da pergunta sobre o pagamento
   RecLock("SX1",.F.)
   SX1->X1_CNT01 := Space(Len(X1_CNT01))
   MsUnLock()
Endif
If dbSeek(cPerg+"12")   // Limpo o conteudo da pergunta sobre o valor do desconto
   RecLock("SX1",.F.)
   SX1->X1_CNT01 := Space(Len(X1_CNT01))
   MsUnLock()
Endif
...
cFilter    += "(E1_XBANCO ='033' .Or. E1_XBANCO ='  ')"
...
oMark:bAval               := { || ( MMarcar( cMarca , .F. ), oMark:oBrowse:Refresh() ) }
...
LOCAL aDadosEmp    := {	SM0->M0_NOMECOM                                    ,; //[1]Nome da Empresa
						...
						Subs(SM0->M0_CGC,1,2)+"."+Subs(SM0->M0_CGC,3,3)+"."+             ; //[6]
...
While !EOF()
   ...
   While !EOF() .And. cDocumen == E1_PREFIXO+E1_NUM+E1_CLIENTE+E1_LOJA
      ...
      cBanco := "033"
      cAgencia := substr(GetMv("MV_XAGST"),01,05)   
      cConta := substr(GetMv("MV_XCCST"),01,11)      
      cSbConta := substr(GetMv("MV_XSBST"),01,03)
      
      DbSelectArea("SA6")
      DbSetOrder(1)
      DbSeek(xFilial("SA6")+cBanco+PadR(cAgencia,05)+PadR(cConta,11),.T.)

      //Posiciona na Arq de Parametros CNAB
      DbSelectArea("SEE")
      DbSetOrder(1)
      DbSeek(xFilial("SEE")+cBanco+PadR(cAgencia,05)+PadR(cConta,11)+PadR(cSbConta,03),.T.)
      ...
      aDadosBanco := {SA6->A6_COD,;                                                             // [1]Codigo do Banco
                      ...
                      ALLTRIM(SEE->EE_CODEMP)                                                ,; // [4]Ident. Cendente
      ...
      aCB_RN_NN := Ret_cBarra( SE1->E1_PREFIXO , SE1->E1_NUM, SE1->E1_PARCELA, SE1->E1_TIPO,;
                   ...
                   aDadosBanco[7], cNroDoc , (SE1->E1_VALOR-(nVlrAbat/*+nDescFin*/)+SE1->E1_ACRESC), aDadosBanco[6], "9")
      ...
      aDadosTit := {E1_NUM+If(Empty(E1_PARCELA),"","-"+E1_PARCELA)+;
                    ...
                    (E1_SALDO - nVlrAbat + E1_ACRESC)  ,;  // [5] Valor do tํtulo
...
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณDescri…o ณ IMPRESSAO DO BOLETO LASER DO SANTANDER COD. DE BARRAS      ณฑฑ
ฑฑรฤฤฤฤฤฤฤฤฤฤลฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤดฑฑ
ฑฑณUso       ณ Especifico para Clientes Microsiga                         ณฑฑ
ฑฑภฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤูฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
...
cBmp := cStartPath + "SANTANDER.BMP" //Logo do Banco Santander
cLogo:= ""//cStartPath + "LOGOARTRIN.BMP" //Logo da Empresa
...
oPrint:Line (nRow1+0150,535,nRow1+0070, 535)
oPrint:Line (nRow1+0150,745,nRow1+0070, 745)

//oPrint:Say  (nRow1+0084,100,aDadosBanco[2],oFont14 )	// [2]Nome do Banco
If File(cBmp)
   oPrint:SayBitmap(nRow1+0080,100,cBmp,0350,0070)
Endif

oPrint:Say  (nRow1+0075,550,aDadosBanco[1]+"-7",oFont21 )		// [1]Numero do Banco     
oPrint:Say  (nRow1+0075,800,substr(aDadosEmp[1],1,47),oFont10)				//Nome + CNPJ
...
oPrint:Say  (nRow1+0200,100 ,substr(aDadosEmp[1],1,40),oFont10)				//Nome + CNPJ

oPrint:Say  (nRow1+0150,1060,"Ag๊ncia/Ident. Beneficiแrio",oFont8)
oPrint:Say  (nRow1+0200,1060,Alltrim(aDadosBanco[3])+"-"+aDadosBanco[8]+"/"+ALLTRIM(aDadosBanco[4]), oFont8)
...
oPrint:Say  (nRow1+0300,100 ,substr(aDatSacado[1],1,30),oFont8)				//Nome

oPrint:Say  (nRow1+0250,1060,"Vencimento",oFont8)
oPrint:Say  (nRow1+0300,1060,StrZero(Day(aDadosTit[4]),2) +"/"+ StrZero(Month(aDadosTit[4]),2) +"/"+ Right(Str(Year(aDadosTit[4])),4),oFont8)

oPrint:Say  (nRow1+0250,1220,"Nosso N๚mero",oFont8)
   cString  := Transform(aDadosTit[6],"@R 999999999999-9")
oPrint:Say  (nRow1+0300,1220,cString,oFont8) //Prefixo +Numero+Parcela
...
If File(cBmp)
   oPrint:SayBitmap(nRow2+0080,100,cBmp,0350,0070)
Endif

nRow2 += 20
oPrint:Say  (nRow2+0080,1750,"Recibo do Pagador" ,oFont14 )     // Legenda do Cabecalho
...
oPrint:Say  (nRow2+0150,100 ,"Beneficiแrio "+substr(aDadosEmp[1],1,42)+" "+alltrim(aDadosEmp[6]) ,oFont8n)  
oPrint:Say  (nRow2+0195,100 ,"Endere็o "+alltrim(aDadosEmp[2])+" "+alltrim(aDadosEmp[3])   ,oFont8n) //Nome + CNPJ

oPrint:Say  (nRow2+0150,1510,"Vencimento",oFont8n)
cString := StrZero(Day(aDadosTit[4]),2) +"/"+ StrZero(Month(aDadosTit[4]),2) +"/"+ Right(Str(Year(aDadosTit[4])),4)
nCol	 	 := 1510 + Int((380-(len(cString)*22))/2)
oPrint:Say  (nRow2+0195,nCol,cString,oFont11c)

oPrint:Say  (nRow2+0150,1910,"Valor do Documento",oFont8n) 
cString := Alltrim(Transform(aDadosTit[5],"@E 99,999,999.99"))
nCol 	 := 1810+(374-(len(cString)*22))
oPrint:Say  (nRow2+0190,nCol,cString,oFont11c)

oPrint:Say  (nRow2+0260,100,"(-)Desconto"                                  ,oFont8n)
cString := Alltrim(Transform(aDadosTit[9],"@EZ 99,999,999.99"))
nCol := 250+(374-(len(cString)*22))
oPrint:Say  (nRow2+0260,nCol,cString ,oFont11c)

oPrint:Say  (nRow2+0260, 630,"(-)Outras Dedu็๕es"                          ,oFont8n)
oPrint:Say  (nRow2+0260,1070,"(+)Mora / Multa"                             ,oFont8n)
oPrint:Say  (nRow2+0260,1510,"(+)Outros Acr้scimos"                        ,oFont8n)
oPrint:Say  (nRow2+0260,1910,"(=)Valor Cobrado"                            ,oFont8n)

oPrint:Say  (nRow2+0360,100 ,"Data de Emissใo"                             ,oFont8n)
...
oPrint:Say  (nRow2+0400,nCol, cString , oFont10)

oPrint:Say  (nRow2+0360, 630,"Ag๊ncia / Ident. Beneficiแrio",oFont8n)
cString  := Alltrim(aDadosBanco[3]+"-"+aDadosBanco[8]+"/"+aDadosBanco[4])
nCol 	 := 630+(374-(len(cString)*22))
oPrint:Say  (nRow2+0400,nCol,cString ,oFont10)

oPrint:Say  (nRow2+0360,1510,"Nosso N๚mero"                                ,oFont8n)
   cString  := Transform(aDadosTit[6],"@R 999999999999-9")
nCol 	 := 2300 - (len(cString)*26)
oPrint:Say  (nRow2+0400,nCol,cString,oFont10)

oPrint:Say  (nRow2+0500, 100,"Dados do Pagador" ,oFont10)
...
If Len(AllTrim(SA1->a1_CGC)) > 11
   cCnpj  := Transform(SA1->A1_CGC,"@R 99999999/9999-99")
Else                                   
   cCnpj  := Transform(SA1->A1_CGC,"@R 999999999-99")
Endif  
                  
oPrint:Say  (nRow2+0540,100 ,"Nome do Pagador"  ,oFont8n)
oPrint:Say  (nRow2+0585,145 ,aDatSacado[1] +" - "+cCnpj,oFont10)

oPrint:Say  (nRow2+0540,1910,"N๚mero.Documento"                             ,oFont8n)
oPrint:Say  (nRow2+0585,1910,aDadosTit[7]+aDadosTit[1]                      ,oFont10) //Prefixo +Numero+Parcela

oPrint:Say  (nRow2+0650,100 ,"Endere็o"                                     ,oFont8n)
oPrint:Say  (nRow2+0695,145 ,aDatSacado[3]                                  ,oFont10)

oPrint:Say  (nRow2+0650,1710,"Bairro / Distrito"                            ,oFont8n)
oPrint:Say  (nRow2+0695,1750,aDatSacado[9]                                  ,oFont10)

oPrint:Say  (nRow2+0750,100 ,"Municํpio"                                    ,oFont8n)
oPrint:Say  (nRow2+0795,145 ,aDatSacado[4]                                  ,oFont10)

oPrint:Say  (nRow2+0750,1710,"UF"                                           ,oFont8n)
oPrint:Say  (nRow2+0795,1750,aDatSacado[5]                                  ,oFont10)

oPrint:Say  (nRow2+0750,1910,"CEP"                                          ,oFont8n)
oPrint:Say  (nRow2+0795,1950,Transform(aDatSacado[6],"@R 99999-999")        ,oFont10)

oPrint:Say  (nRow2+0870,100 ,"Instru็๕es(Termo de responsabilidade do cendente)"  ,oFont8)

//--------------------------------------------------------------------------------------
oPrint:SayBitmap(nRow2+0860,1800,cLogo,0400,0454)
oPrint:Say  (nRow2+1000,100 ,aBolText[5]  ,oFont8)
oPrint:Say  (nRow2+1040,100 ,aBolText[6]  ,oFont8)
oPrint:Say  (nRow2+1080,100 ,aBolText[1]  ,oFont8)
oPrint:Say  (nRow2+1120,100 ,aBolText[2]  ,oFont8)
oPrint:Say  (nRow2+1160,100 ,aBolText[3]  ,oFont8)
oPrint:Say  (nRow2+1200,100 ,aBolText[4]  ,oFont8)

oPrint:Line (nRow2+1370,100,nRow2+1370,2300 ) //nRow1+1840  - alterei para 1340

oPrint:Say  (nRow2+1380,1480,"Autentica็ใo Mecโnica"                        ,oFont8n)
oPrint:Say  (nRow2+1380,1810,"Recibo do Pagador" ,oFont10)

oPrint:Line (nRow2+1390,1280,nRow2+1390,1380 )
oPrint:Line (nRow2+1390,2200,nRow2+1390,2300 )
oPrint:Line (nRow2+1390,1280,nRow2+1450,1280 )
oPrint:Line (nRow2+1390,2300,nRow2+1450,2300 )

vMens    := Array(4)
vMens[1] := "Este recibo somente terแ validade com a autentica็ใo mecโnica ou acompanhado do"
vMens[2] := "recibo de pagamento emitido pelo Banco."
vMens[3] := "Recebimento atrav้s do cheque n.                                             do banco"
vMens[4] := "Esta quita็ใo s๓ terแ validade ap๓s o pagamento do cheque pelo banco pagador."

oPrint:Say  (nRow2+1400,100 , vMens[1]                                     ,oFont8n)
oPrint:Say  (nRow2+1430,100 , vMens[2]                                     ,oFont8n)
oPrint:Say  (nRow2+1460,100 , vMens[3]                                     ,oFont8n)
oPrint:Say  (nRow2+1490,100 , vMens[4]                                     ,oFont8n)
...
nRow2 := nRow2 + 425 //1025 - alterei para 525
...
If File(cBmp)
   oPrint:SayBitmap(nRow3+0080,100,cBmp,0350,0070)
Endif
...
oPrint:Say  (nRow3+0190,100 ,"Pagar preferencialmente no Grupo Santander GC",oFont10)
...
oPrint:Say  (nRow3+0250,100 ,"Beneficiแrio "+alltrim(aDadosEmp[1]) +" "+alltrim(aDadosEmp[6]),oFont10)
oPrint:Say  (nRow3+0290,100 ,"Endere็o " +alltrim(aDadosEmp[2])+" "+alltrim(aDadosEmp[3]) ,oFont10) //Nome + CNPJ

oPrint:Say  (nRow3+0250,1810,"Ag๊ncia / Ident. Beneficiแrio",oFont8n)
cString  := Alltrim(aDadosBanco[3]+"-"+aDadosBanco[8]+"/"+aDadosBanco[4])
nCol 	 := 1810+(374-(len(cString)*22))
...
oPrint:Say  (nRow3+0350,1005,"Esp้cie Documento"                            ,oFont8n)
...
oPrint:Say  (nRow3+0450,100 ,SUBSTR(aDadosBanco[6],1,3)+" - Cobran็a Rแpida com Registro",oFont8)

oPrint:Say  (nRow3+0420,755 ,"Moeda"                                        ,oFont8n)
oPrint:Say  (nRow3+0450,805 ,"REAL"                                           ,oFont10)
...
oPrint:Say  (nRow3+0490,100 ,"Instru็๕es (texto de responsabilidade do Beneficiแrio)",oFont8)
oPrint:Say  (nRow3+0530,100 ,aBolText[5]  ,oFont8)
oPrint:Say  (nRow3+0570,100 ,aBolText[6]  ,oFont8)
oPrint:Say  (nRow3+0610,100 ,aBolText[1]  ,oFont8)
oPrint:Say  (nRow3+0660,100 ,aBolText[2]  ,oFont8)
oPrint:Say  (nRow3+0710,100 ,aBolText[3]  ,oFont8)
oPrint:Say  (nRow3+0760,100 ,aBolText[4]  ,oFont8)
...
oPrint:Say  (nRow3+0870,200 ,aDatSacado[1] +" - "+cCnpj                    ,oFont10)
      
oPrint:Say  (nRow3+0840,1810,"Ficha de Compensa็ใo"                        ,oFont11)                                                                                                                                                                                               

oPrint:Say  (nRow3+0923,200 ,aDatSacado[3]+" - "+aDatSacado[9]             ,oFont10)
oPrint:Say  (nRow3+0976,200 ,Transform(aDatSacado[6],"@R 99999-999")+"    "+aDatSacado[4]+" - "+aDatSacado[5],oFont10) // CEP+Cidade+Estado
oPrint:Say  (nRow3+0976,1850,Substr(aDadosTit[6],1,3)+Substr(aDadosTit[6],4)  ,oFont10)
oPrint:Say  (nRow3+1025,100 ,"Sacado Avalista:",oFont8n)  
oPrint:Say  (nRow3+1025,500 ,"CPF/CNPJ:",oFont8n)                                                             

oPrint:Say  (nRow3+1080,1700,"Autentica็ใo Mecโnica"                       ,oFont8n)
...
oPrint:Line (nRow3+1070,100 ,nRow3+1070,2300 )

//MSBAR2("INT25",  12.5,      1,aCB_RN_NN[1],oPrint,.F.,Nil,Nil,0.014   ,1.0    ,Nil,Nil,"A",.F.,100,100)
MSBAR2("INT25",  26.4,1.4,aCB_RN_NN[1],oPrint,.F.,Nil,Nil,0.025,1.5,Nil,Nil,"A",.F.,100,100)
          

For nI := 100 to 2300 step 50
	oPrint:Line(nRow3+1350, nI, nRow3+1350, nI+30)
Next nI
...
Local cDigital		:= ""
Local aRet			:= {}
//Local cCodCli       := SubStr(cNumBco,1,12) 
Local cNumTmp       := ""

cAgencia := Left(Alltrim(cAgencia),4)

If Empty(cNosso)   // Se ainda nใo foi calculado   
   
   cNumTmp := NossoNum()

      cNosso := strzero(Val(cNumTmp),12)
      cNosso += Modulo11(cNosso ,.T.) 
Endif
         //    1            7             13              1   3    = 25
   cCampoL := "9"+substr(cConta,1,7)+SUBSTR(cNosso,1,13)+"0"+cCart    
...
cParte1  := cBanco + cMoeda + SubStr(cCampoL,1,5)    //10
cDig1    := DIGIT001( cParte1 )                    //1
cParte2  := SUBSTR(cCampoL,6,10)                     //10                                  
cDig2    := DIGIT001( cParte2 )                    //1
cParte3  := SUBSTR(cCampoL,16,10)                    //10
cDig3    := DIGIT001( cParte3 )                    //1
cParte4  := cDigBarra                                //1
cParte5  := cFatorValor                              //14
...
ฑฑณFun…o    ณ AjustaSx1    ณ Autor ณ Microsiga            	ณ Data ณ 13/10/03 ณฑฑ
...
User Function CALC_5p(cVariavel,lNosso)
   ...
   Local nCli := If( Upper(TCGetDB()) == "MSSQL" , 1, 2)   
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบFuncao    ณModulo11   บAutor  ณMarcel Grosselli   บ Data ณ  20/05/12   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณCalculo do digito do nosso numero pelo Modulo 11 com base 9 บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ BOLETOS                                                    บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
