...
±±ºPrograma  PMPCPR32    ºAutor  ³STAN LEE LOPES    				º Data ³  16/05/21 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³  PICKLIST SIMPLIFICADO ATIPICO PELMEX				        			 ±
±±º          ³                                                            			   º±±
...
User Function PMPCPR32()
	Local oReport := nil
	Local cPerg:= Padr("PMPCPR32",10)
...
Static Function RptDef(cNOME)
	...
	oReport := TReport():New(cNOME,"PICKLIST SIMPLIFICADO ATIPICO PELMEX",cNOME,{|oReport| ReportPrint(oReport)},"PICKLIST")
	oReport:SetPortrait()
...
cQuery := " SELECT TB.PK2_SETOR SETOR,TB.G1_COMP CODIGO,TB.B1_DESC DESCRICAO,TB.B1_UM UNIDADE,SUM(QTD_NECESSARIA)QTD_NECESSARIA,"
cQuery += "         SUM(ISNULL(QTD_EMPENHO,0))QTD_EMPENHO,"
cQuery += " 		ISNULL(QTD_10,0)QTD_10,"
cQuery += " 		(ISNULL(QTD_10,0)-SUM(ISNULL(QTD_EMPENHO,0)))SALDO_10,"
cQuery += " 		ISNULL(QTD_01,0)QTD_01 "
cQuery += " FROM( "
cQuery += " 	  SELECT PK2_SETOR,PK2_PRODUTO,PK2_DESCRI,G1_COMP,SB1.B1_DESC,SB1.B1_UM,PK2_QUANT, "
cQuery += " 	         G1_QUANT,PK2_QUANT*(G1_QUANT+G1_QUANT*G1_PERDA/100) QTD_NECESSARIA ,QTD_EMPENHO"
cQuery += " 	  FROM PK2 (NOLOCK)"
cQuery += " 	  INNER JOIN SG1100 SG1 (NOLOCK) ON PK2_PRODUTO = G1_COD AND SG1.D_E_L_E_T_ = '' AND G1_FIM >= CONVERT(VARCHAR,GETDATE(),112) "
cQuery += " 	  INNER JOIN SB1100 SB1 (NOLOCK) ON G1_COMP = B1_COD AND SB1.D_E_L_E_T_ = '' AND B1_XFANTAS != 'S'"
cQuery += " 	  LEFT JOIN (  SELECT SUBSTRING(D4_OP,1,6)D4_OP,D4_TRT,D4_COD,SUM(D4_QTDEORI) QTD_EMPENHO"
...
cQuery += " 				   )SD4 ON G1_TRT = D4_TRT AND G1_COMP = D4_COD AND PK2_OP = D4_OP  "
cQuery += " 	  WHERE G1_COMP NOT LIKE 'MOD%' "
cQuery += " 	  AND CONVERT(VARCHAR,CAST(PK2_DTPROPEL AS DATE),112) BETWEEN '"+DTOS(mv_par01)+"' AND '"+DTOS(mv_par02)+"'"
//cQuery += "		 GROUP BY PK2_SETOR,PK2_PRODUTO,PK2_DESCRI,B1_QB,G1_COMP,SB1.B1_DESC,SB1.B1_UM,PK2_QUANT,G1_QUANT,QTD_EMPENHO"
cQuery += "		)TB"
cQuery += " LEFT JOIN (SELECT B2_FILIAL,B2_COD,B2_QATU QTD_10,B2_LOCAL"
cQuery += "           FROM SB2100 SB2 (NOLOCK)"
...
cQuery += " GROUP BY TB.PK2_SETOR,TB.G1_COMP,TB.B1_DESC,TB.B1_UM,QTD_10,QTD_01"
cQuery += " ORDER BY TB.PK2_SETOR,TB.B1_DESC"
	
	//cQuery += " AND CONVERT(VARCHAR,CAST(PK2_DATINC AS DATE),112) BETWEEN '"+DTOS(mv_par01)+"' AND '"+DTOS(mv_par02)+"' "
	/*
	cQuery += " AND CONVERT(VARCHAR,CAST(PK2_DTPROPEL AS DATE),112) BETWEEN '"+DTOS(mv_par01)+"' AND '"+DTOS(mv_par02)+"' "
	cQuery += " AND (PK2_DESCRI LIKE '%"+ALLTRIM(mv_par03)+"%' OR PK2_PRODUTO LIKE '%"+ALLTRIM(mv_par05)+"%' )"
	cQuery += " )TB "
	cQuery += " GROUP BY TB.PK2_SETOR,TB.G1_COMP,TB.B1_DESC,TB.B1_UM "
	cQuery += " ORDER BY TB.PK2_SETOR,TB.G1_COMP "*/
...
Static Function AjustaSX1(cPerg)
	...
	u_InPutSX1(cPerg,"01",PADR("DT.Producao de  ?    ",20)+"","","","mv_ch1","D",10,0,0,"G","","","","","mv_par01") 
	u_InPutSX1(cPerg,"02",PADR("DT.Producao Até ?    ",20)+"","","","mv_ch2","D",10,0,0,"G","","","","","mv_par02")
	//u_InPutSX1(cPerg,"03",PADR("Produto de  ?    ",20)+"","","","mv_ch3","C",50,0,0,"G","","SB1","","","mv_par03") 
Return
