...
Static Function fPreencItens(oDet,cCodFor,cLojFor, cNomeFor)
	...
	*/
	...
	AADD(aItens, {"DKD_XDEXML" , cDesXprod})
	AADD(aItens, {"DKD_XUMXML"  , cUmXML})
	AADD(aItens, {"DKD_XQTXML" , nQtd})
...
Static Function fGeraPreNota()
	...
	If fAllProdOK(.T.)
		...
		For nX:=1 To Len(aCols)
			...
			aAdd(aLinha, {"D1_X_DESCR" ,aCols[nX][fGetIndice("B1_DESC"   , aHeader)]		, NIL})
			...
			aAdd(aLinha, {"DKD_XDEXML" , aCols[nX][fGetIndice("DKD_XDEXML", aHeader)]	, NIL})
			aAdd(aLinha, {"DKD_XUMXML"  , aCols[nX][fGetIndice("DKD_XUMXML", aHeader)]	, NIL})
			aAdd(aLinha, {"DKD_XQTXML" , aCols[nX][fGetIndice("DKD_XQTXML", aHeader)]	, NIL})
...
Static Function fDadosForn()
	...
	If SF1->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor)) 
		DKD->(DbSetOrder(1))
		If DKD->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor))
			nLinAtu := 0
			While !DKD->(EOF()) .and. DKD->(DKD_FILIAL+DKD_DOC+DKD_SERIE+DKD_FORNEC+DKD_LOJA) == xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor
				nLinAtu++
				If aCols[nLinAtu,fGetIndice("B1_COD", aHeader)] == SD1->D1_COD
					RecLock("DKD", .F.)
						DKD->DKD_XDEXML := aCols[nLinAtu][fGetIndice("DKD_XDEXML", aHeader)]	
						DKD->DKD_XUMXML  := aCols[nLinAtu][fGetIndice("DKD_XUMXML" , aHeader)]	
						DKD->DKD_XQTXML := aCols[nLinAtu][fGetIndice("DKD_XQTXML", aHeader)]	
					MsUnlock()
				EndIf
				DKD->(DbSkip())
			Enddo
	...
	ElseIf SF1->(DbSeek(xFilial("SF1")+cDoc+PADL(val(cSerie),tamsx3("F1_SERIE")[1],"0")+cCodFor+cLojFor)) 
		DKD->(DbSetOrder(1))
		If DKD->(DbSeek(xFilial("SF1")+cDoc+PADL(val(cSerie),tamsx3("F1_SERIE")[1],"0")+cCodFor+cLojFor))
			nLinAtu := 0
			While !DKD->(EOF()) .and. DKD->(DKD_FILIAL+DKD_DOC+DKD_SERIE+DKD_FORNEC+DKD_LOJA) == xFilial("SF1")+cDoc+PADL(val(cSerie),tamsx3("F1_SERIE")[1],"0")+cCodFor+cLojFor
				nLinAtu++
				If aCols[nLinAtu,fGetIndice("B1_COD", aHeader)] == SD1->D1_COD
					RecLock("DKD", .F.)
						DKD->DKD_XDEXML := aCols[nLinAtu][fGetIndice("DKD_XDEXML", aHeader)]	
