...
User Function PLCOMA01()
	...
	*/
	...
	oButAtuXml := TButton():New(nPosLin+8,aPosGet[1,3]+(4*90), "Atu. NF"       ,oDlg,{|| fDadosForn()         },60,10,,,.F.,.T.,.F.,,.F.,,,.F.)
	nPosLin += 23
...
Static Function fPreencItens(oDet,cCodFor,cLojFor, cNomeFor)
	...
	*/
	...
	Local nIcmsSt  := 0
	Local nBCIcmsSt:= 0
	Local cDescriXML := replace(RTrim(oDet:_prod:_xprod:text),"|"," ") //RTrim(oDet:_prod:_xprod:text)
	/*
	...
	*/
	...
	AADD(aItens, {"DKD_XDESXM", cDescriXML})
	AADD(aItens, {"DKD_XUMXML" , cUmXML})
	AADD(aItens, {"DKD_XQTDXM", nQtd})
		...
		Else
			...
			If SF1->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor))
				...
				If DKD->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor))
					While !DKD->(EOF()) .and. DKD->(DKD_FILIAL+DKD_DOC+DKD_SERIE+DKD_FORNECE+DKD_LOJA) == xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor
						nLinha := Ascan(aCols,{|x| x[fGetIndice("D1_ITEM", aHeader)] == DKD->DKD_ITEM})
						If nLinha > 0
							RecLock("DKD", .F.)
							DKD->DKD_XDESXM := aCols[nLinha][fGetIndice("DKD_XDESXM", aHeader)]	
							DKD->DKD_XUMXML  := aCols[nLinha][fGetIndice("DKD_XUMXML" , aHeader)]	
							DKD->DKD_XQTDXM := aCols[nLinha][fGetIndice("DKD_XQTDXM", aHeader)]	
							MsUnlock()
						EndIf
						DKD->(DbSkip())
					Enddo
...
Static Function fDadosForn()
	...
	If SF1->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor))
		...
		If DKD->(DbSeek(xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor))
			While !DKD->(EOF()) .and. DKD->(DKD_FILIAL+DKD_DOC+DKD_SERIE+DKD_FORNECE+DKD_LOJA) == xFilial("SF1")+cDoc+cSerie+cCodFor+cLojFor
				nLinha := Ascan(aCols,{|x| x[fGetIndice("B1_COD", aHeader)] == SD1->D1_COD})
				If nLinha > 0
					RecLock("DKD", .F.)
						DKD->DKD_XDESXM := aCols[nLinha][fGetIndice("DKD_XDESXM", aHeader)]	
						DKD->DKD_XUMXML  := aCols[nLinha][fGetIndice("DKD_XUMXML" , aHeader)]	
