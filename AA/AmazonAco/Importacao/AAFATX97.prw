#Include 'Protheus.ch'
#include 'TBICONN.ch'

User Function AAFATX97(_xdAuto,_xdFile)

    //Prepare Environment Empresa '01' Filial '01' 
    
	Default _xdAuto    := .F.
	Default _xdFile    := Space(400)
        
	Private _cdFil := FwCodFil()
	Private _cdEmp := FwCodEmp()
	Private _ldAuto    := _xdAuto
	Private _cdArquivo := iIf( _ldAuto,_xdFile ,Space(400))
	Private _adEmp := FwLoadSM0()
	Private lAviso := .T.
	
	PRIVATE cNumOrc   := getLastOrc()
	Private _adFormas := {}
	Private _cdXml    := " "
	
	aAdd(_adFormas , {"01","Dinhero","R$"} )
	aAdd(_adFormas , {"02","CHEQUE" ,"CH"} )
	aAdd(_adFormas , {"03","CARTAO CREDITO", "CC"} )
	aAdd(_adFormas , {"04","CARTAO DEBITO" , "CD"} )
	aAdd(_adFormas , {"05","CREDITO LOJA"  , "CR"} )
	aAdd(_adFormas , {"10","VALE ALIMENTACAO",""} )
	aAdd(_adFormas , {"11","VALE REFEICAO"   ,""} )
	aAdd(_adFormas , {"12","VALE PRESENTE"   ,""} )
	aAdd(_adFormas , {"13","COMBUSTIVEL"     ,""} )
	aAdd(_adFormas , {"99","OUTROS"          ,""} )
	
	Private _cdFile := ""
	PRivate _cdDir  := ""
	if getXmls()
		Processa( {|| ChecaDir( _cdDir,_cdFile) } ,"Processando Xmls's Encontrados")
	EndIf
	
Return Nil

Static Function getLastOrc()

	_cdQry :=  ' select isNull(MAX(L1_NUM),"I00000") ORC from ' + RetSqlName('SL1')
	_cdQry +=  "  Where Left(L1_NUM,1) = 'I' "
	_cdtbl := getNextAlias()
	dbUseArea(.F., "TOPCONN", tcGenQry(,,_cdQry), _cdtbl , .T., .F.)
	_cdNumOrc := (_cdTbl)->ORC
	(_cdTbl)->(dbCloseArea(_cdTbl))
	
Return _cdNumOrc

Static Function getXmls( )
    
	If !_ldAuto .Or. (Empty(_cdFile) .And. _ldAuto)
	
		DEFINE MSDIALOG oDlg TITLE "Importação Notas Fiscais" From 0,0 To 200,600 Pixel 
		nOpc := 00
	
		_cdFile := Padr(Getmv('MV_XAFATX97'),400)//IiF( ALLTRIM(Getmv('MV_XACOMP11')) == 'XX', Padr(Getmv('MV_XACOMP04'),400) , SPACE(400) )
		_cdDir  := Padr(Getmv('MV_XBFATX97'),400)//IiF( ALLTRIM(Getmv('MV_XBCOMP11')) == 'XX', Padr(Getmv('MV_XBCOMP04'),400) , SPACE(400) )

		@ 05,07 SAY " Arquivo para Importação:" SIZE 100,80 PIXEL OF oDlg
		@ 15,07 MSGET _cdFile PICTURE "@!" SIZE 250, 10 PIXEL OF oDlg  F3 'XMLFIL'
		
		If !_ldAuto // So Aparece quando não e Automatica
			@ 30,07 SAY " Diretorio para Importação:" SIZE 100,80 PIXEL OF oDlg
			@ 45,05 MSGET _cdDir  PICTURE "@!" SIZE 250, 10 PIXEL OF oDlg  F3 'XMLDIR'
		EndIf
		@ 60,050 BUTTON "Importar" SIZE 40,12 PIXEL OF oDlg ACTION (nOpc:=1,oDlg:End())
		@ 60,110 BUTTON "Cancelar" SIZE 40,12 PIXEL OF oDlg ACTION (nOpc:=0,oDlg:End())

		ACTIVATE MSDIALOG oDlg CENTERED
	Else
		nOpc := 01
	EndIf

//-- Validacoes
	If nOpc == 1
		If Empty(_cdFile) .And. Empty(_cdDir)
			MsgInfo("Nenhum Arquivo/Diretorio selecionado!.","Atenção!!")
			lRet := .F.
		Else
		    //Putmv('MV_XAFATX08',_cdFile)
			Putmv('MV_XBFATX97',_cdDir)
			lRet := .T.
		EndIf
	Else
		lRet := .F.
	EndIf
		
Return lRet

Static Function ChecaDir(xdPath,xdFile)

	Local _adProcess := {}
	Local _adFiles := {}
	Local cError   := ""
	Local cWarning := ""
	Default xdFile := ""
	Default xdPath := ""
	Private _cdFile := xdFile
	Private _cdPath := xdPath
	Private aSM0    := SM0->(GetArea())
	_cdPath := Alltrim(_cdPath)
	
	If !Empty(_cdPath)
		_cdPath  += iIf(Right(Alltrim(_cdPath),1) != '\','\','')
		_adFiles := Directory(alltrim(_cdPath)+"*.xml")
	EndIf
    
	If !Empty(_cdFile)
		aAdd(_adFiles,{_cdFile})
	EndIf

	ProcRegua(Len(_adFiles))

	For _ndI:=1 to Len(_adFiles)
	
		_ldErro := .F.
		_cdFile := ALLTRIM(_adFiles[_ndI][1])
	
		fHandle := FT_FUSE(_cdPath + _cdFile)
		FT_FGOTOP()
		_cdXml := ""
		While !FT_FEOF()
			_cdXml += FT_FREADLN()
			FT_FSKIP()
		EndDo
		FT_FUSE()
		
		oXML := XmlParser(_cdXml, "_", @cError, @cWarning )
	
		If !Empty(cError) .Or.  !Empty(cWarning)
			aAdd(_adProcess, {_cdFile , oXML,_ldErro}  )
			_xdMensagem := ""
			_ldErro := .T.
		EndIf
	 
		If !_ldErro
		   // cNumOrc := Soma1(cNumOrc)
			_adRet := aClone( ChecaXml(oXML) )
		EndIf
	  
		IncProc()
	Next
	IF Len(_adProcess) == 0
		aAdd(_adProcess, { .F. ,.F.,"" , "" , "" , Nil , "", {}  ,"" , {{},{}}  } )
	EndIf
Return _adProcess


Static Function ChecaXml(oXml)
	Local _ldRet := .T.
	Local _cdRet := ' '
	Local oRet   := Nil

	Private oNfe      := IIf(Type("oXML:_NFEPROC")=="U",	Nil ,	oXML:_NFEPROC)
	Private oNF       := iIf(Type("oNFe:_NFe") == 'U', Nil, oNFe:_NFe)
	Private oEmitente := iIF(Type("oNF:_InfNfe:_Emit") == 'U', Nil,oNF:_InfNfe:_Emit)
	Private oIdent    := iIF(Type("oNF:_InfNfe:_IDE") == 'U', Nil,oNF:_InfNfe:_IDE)
	Private oDestino  := iIF(Type("oNF:_InfNfe:_Dest") == 'U', Nil,oNF:_InfNfe:_Dest)
	Private oTotal    := iIF(Type("oNF:_InfNfe:_Total") == 'U', Nil,oNF:_InfNfe:_Total)
	Private oTransp   := iIF(Type("oNF:_InfNfe:_Transp") == 'U', Nil,oNF:_InfNfe:_Transp)
	Private oDet      := iIF(Type("oNF:_InfNfe:_Det") == 'U', Nil,oNF:_InfNfe:_Det)
	Private _cHoraEmi := ""
	Private _cdCgc    := ""
	Private _cdSerie  := ""
	Private _cdDoc    := ""
	Private _cdCvh    := ""
	
	If (_ldRet := oNf == Nil )
		_cdRet := 'XML em Formato Invalido, Arquivo Não se Refere a Uma NF-e'
	ElseIf  (oEmitente == Nil)
		_cdRet := 'XML em Formato Invalido, Dados do Emitente não Encontrado'
	ElseIf (oIdent == Nil)
		_cdRet := 'XML em Formato Invalido, Dados Referente aos Detalhes da Nota Não Encontrados'
	ElseIf (oDet == Nil)
		_cdRet := 'XML em Formato Invalido, Dados dos Produtos da Nota não Encontrados'
	EndIf
	_ldRet := Empty(_cdRet)
	oRet   := iIf(_ldRet,oNf,Nil)
	_cdCvh := iIf(_ldRet,oNfe:_PROTNFE:_INFPROT:_CHNFE:TEXT,'')
    
	ProcessaXml(oNfe:_NFE,_cdCvh)
    
Return _ldRet

Static Function ProcessaXml(_xdNfe,_cdCvh)
	
	Private _odNfe := _xdNfe
		
	_cdCnpj   := iIf(type("_odNfe:_InfNfe:_Emit:_CNPJ:TEXT") != "U", _odNfe:_InfNfe:_Emit:_CNPJ:TEXT,"")
	_cdInsc   := iIf(type("_odNfe:_InfNfe:_Emit:_IE:TEXT")   != "U", _odNfe:_InfNfe:_Emit:_IE:TEXT,"")
	_cdDoc    := iIf(Type("_odNfe:_InfNfe:_IDE:_NNF:TEXT")   != "U", StrZero(Val(_odNfe:_InfNfe:_IDE:_NNF:TEXT),9),"")
	_cdSerie  := iIf(Type("_odNfe:_InfNfe:_IDE:_SERIE:TEXT") != "U", _odNfe:_InfNfe:_IDE:_SERIE:TEXT,"")
	_cdCgc    := iIf(Type("_odNfe:_InfNfe:_DEST:_CNPJ:TEXT") != "U", _odNfe:_InfNfe:_DEST:_CNPJ:TEXT,iIf(Type("_odNfe:_InfNfe:_DEST:_CPF:TEXT") != "U", _odNfe:_InfNfe:_DEST:_CPF:TEXT,""))
	_cdNsc    := iIf(Type("_odNfe:_INFNFE:_DEST:_IE:TEXT") != "U" , _odNfe:_INFNFE:_DEST:_IE:TEXT,"")
	
	_dEmissao := Stod(Left(iIf(Type("_odNfe:_InfNfe:_IDE:_dEmi:TEXT") != "U" , StrTran(_odNfe:_InfNfe:_IDE:_dEmi:TEXT,"-"),""),8) )
	_cHoraEmi := SubString( iIf(Type("_odNfe:_InfNfe:_IDE:_dEmi:TEXT") != "U" , StrTran(_odNfe:_InfNfe:_IDE:_dEmi:TEXT,"-"),""),10,08)
	_cdNome   := iIf(Type("_odNfe:_InfNfe:_EMIT:_XNOME:TEXT") != "U" , _odNfe:_InfNfe:_EMIT:_XNOME:TEXT,"")
	_ndDesc   := IiF(tYPE("_odNfe:_InfNfe:_Total:_ICMSTOT:_VDESC:TEXT") != "U", Val(_odNfe:_InfNfe:_Total:_ICMSTOT:_VDESC:TEXT) ,0)
	_ndTotal  := iIf(Type("_odNfe:_InfNfe:_Total:_ICMSTOT:_VNF:TEXT")   != "U", Val(_odNfe:_InfNfe:_Total:_ICMSTOT:_VNF:TEXT)   ,0)
	_ndMerc   := iIf(Type("_odNfe:_InfNfe:_Total:_ICMSTOT:_VPROD:TEXT") != "U", Val(_odNfe:_InfNfe:_Total:_ICMSTOT:_VPROD:TEXT) ,0)
	_ndfrete  := iIf(Type("_odNfe:_InfNfe:_Total:_ICMSTOT:_VFRETE:TEXT") != "U", Val(_odNfe:_InfNfe:_Total:_ICMSTOT:_VFRETE:TEXT) ,0)
	_ndSeguro := iIf(Type("_odNfe:_InfNfe:_Total:_ICMSTOT:_VSEG:TEXT") != "U", Val(_odNfe:_InfNfe:_Total:_ICMSTOT:_VSEG:TEXT) ,0)
	
	_cdBairro := iIf(Type("_odNfe:_InfNfe:_DEST:_EnderDest:_xBairro:TEXT") != "U" , _odNfe:_InfNfe:_DEST:_EnderDest:_xBairro:TEXT,"")
    
	_xdPag    := iIf(Type("_odNfe:_InfNfe:_Pag") != "U",_odNfe:_InfNfe:_Pag,"")
	
	SF2->(dbSetOrder(1))
	if SF2->(dbSeek(xFilial("SF2") + _cdDoc + _cdSerie ))
		AViso('Atençao','NFE JA IMPORTADA',{'OK'})
		_cdSucess := _cdPath + "Ja Importadas\"
		If !ExistDir(_cdSucess)
			MakeDir(_cdSucess)
		EndIf
		_xdFile := StrTokArr(_cdFile,"\")
		_xdFile := _xdFile[Len(_xdFile)]
		fRename(_cdPath + _cdFile , _cdSucess + _xdFile )
		
		Return Nil
	EndIf
	
	_ndPos := aScan(_adEmp, {|x| x[01] == _cdEmp .And. x[02] == _cdFil }  )
	If _ndPos > 0
		If _cdCnpj != _adEmp[_ndPos][18]
			If lAviso
				Aviso('Atenção', "Este XML é da Filial:   ("+  _cdBairro  +")" + Chr(13) + Chr(10) + ;
					" CNPJ:  ("+  _cdCnpj  +")" + Chr(13) + Chr(10) +;
					"Só e Permitido Importar XML da Filial Logada ", {"OK"})
			Else
				_xdMensagem += " Este XML é da Filial:   ("+  _cdBairro  +")" + Chr(13) + Chr(10) + ;
					" CNPJ:  ("+  _cdCnpj  +")" + Chr(13) + Chr(10) +;
					" Só e Permitido Importar XML da Filial Logada "
			EndiF
			Return .F.
		EndIf
	Else
		MsFinal()
	EndIf
	
	
	xoCds  := FLocalz(_cdCgc,_cdNsc)
	SA1->(dbSetOrder(1))
	If !SA1->(dbSeek(xFilial('SA1')+ xoCds )) .Or. Empty(xoCds)
	    //CreateCliente(_cdCgc)
	    /*
	    If Empty(_odNfe:_InfNfe:_DEST:_CNPJ:TEXT) 
			cCnpjCli   := _odNfe:_InfNfe:_DEST:_cpf:text
		Else 
			cCnpjCli   := _odNfe:_InfNfe:_DEST:_CNPJ:TEXT
		EndIf
		*/
		cCnpjCli   := _cdCgc
		cNomeCli   := _odNfe:_InfNfe:_DEST:_XNOME:text
		cEndCli    := _odNfe:_InfNfe:_DEST:_ENDERDEST:_XLGR:text
		cNroCli    := _odNfe:_InfNfe:_DEST:_ENDERDEST:_NRO:text
		cUfCli     := _odNfe:_InfNfe:_DEST:_ENDERDEST:_UF:text
		cMunCli    := _odNfe:_InfNfe:_DEST:_ENDERDEST:_XMUN:text
		cBairroCli := _odNfe:_InfNfe:_DEST:_ENDERDEST:_XBAIRRO:text
		cCepCli    := _odNfe:_InfNfe:_DEST:_ENDERDEST:_CEP:text
		cIECli 	   := _odNfe:_InfNfe:_DEST:_IE:TEXT
		
		fGravaCli(cNomeCli,cCnpjCli,cEndCli,cNroCli,cUfCli,cMunCli,cBairroCli,cCepCli,_cdNsc)
	
		xoCds  := FLocalz(cCnpjCli,_cdNsc)
		SA1->(dbSeek(xFilial('SA1')+xoCds ))
		
	EndIf
	
	dDatabase := _dEmissao
	
	
	nDinheiro := 0
	nCheque   := 0
	nCartaoC  := 0
	nCartaoD  := 0
	nCredito  := 0
	nOutros   := 0
    
    
    //Sem PAG e NF Grande
    /*
	if ValType(_xdPag) == "O"

	   _xdForma  := Alltrim(_xdPag:_tPag:TEXT)
	   _xdValor  := _xdPag:_vPag:TEXT
	   
	   nDinheiro := iIf(_xdForma == _adFormas[01][01] , Val(_xdValor) , 0)
	   nCheque   := iIf(_xdForma == _adFormas[02][01] , Val(_xdValor) , 0)
	   nCartaoC  := iIf(_xdForma == _adFormas[03][01] , Val(_xdValor) , 0)
	   nCartaoD  := iIf(_xdForma == _adFormas[04][01] , Val(_xdValor) , 0)
	   nCredito  := iIf(_xdForma == _adFormas[05][01] , Val(_xdValor) , 0)
	   nOutros   := iIf(_xdForma == _adFormas[10][01] , Val(_xdValor) , 0)
	   
	ElseIf ValType(_xdPag) == "A"

	   For _ndX := 1 To Len(_xdPag)
	       _xdForma  := Alltrim(_xdPag[_ndX]:_tPag:TEXT)
	       _xdValor  := _xdPag[_ndX]:_vPag:TEXT
	       
		   
	       nDinheiro += iIf(_xdForma == _adFormas[01][01] , Val(_xdValor) , 0)
		   nCheque   += iIf(_xdForma == _adFormas[02][01] , Val(_xdValor) , 0)
		   nCartaoC  += iIf(_xdForma == _adFormas[03][01] , Val(_xdValor) , 0)
	       nCartaoD  += iIf(_xdForma == _adFormas[04][01] , Val(_xdValor) , 0)
		   nCredito  += iIf(_xdForma == _adFormas[05][01] , Val(_xdValor) , 0)
	       nOutros   += iIf(_xdForma == _adFormas[10][01] , Val(_xdValor) , 0)
	   Next

	EndIf
	*/
	aCabecSF1 := {}
	aItensSD1 := {}
	aCabecSF2 := {}
	aItensSD2 := {}


	aadd(aCabecSF2,{"F2_TIPO"   ,"N"})
	aadd(aCabecSF2,{"F2_FORMUL" ,"S"})
	aadd(aCabecSF2,{"F2_DOC"    ,_cdDoc})
	aadd(aCabecSF2,{"F2_SERIE"  ,_cdSerie})
	aadd(aCabecSF2,{"F2_EMISSAO",dDataBase})
	aadd(aCabecSF2,{"F2_CLIENTE", SA1->A1_COD })
	aadd(aCabecSF2,{"F2_LOJA"   , SA1->A1_LOJA})
	aadd(aCabecSF2,{"F2_ESPECIE","SPED" })
	aadd(aCabecSF2,{"F2_COND"   ,"449"  })
	aadd(aCabecSF2,{"F2_ESPECI1", "FARDO "})
	aadd(aCabecSF2,{"F2_EMINFE" ,dDataBase})
	aadd(aCabecSF2,{"F2_CREDNFE",00 })
	aadd(aCabecSF2,{"F2_CHVNFE" ,_cdCvh   })
	aadd(aCabecSF2,{"F2_DESCONT", _ndDesc   })
	aadd(aCabecSF2,{"F2_FRETE" , 0   })
	aadd(aCabecSF2,{"F2_DESPESA" , 0   })
	aadd(aCabecSF2,{"F2_SEGURO" , 0   })
	    			
	_aItem := {}
	_ldErro := .F.
	_ldDev  := .F.
	
	If Type("_odNfe:_INFNFE:_DET") == "O"
		oProd := _odNfe:_INFNFE:_DET:_Prod
	   
		cProduto := oProd:_cProd:TEXT
		nQuant   := Val( oProd:_qCom:TEXT )
		cUM      := oProd:_uCom:TEXT
		nVDesc   := iIf(Type("oProd:_vDesc:TEXT") != "U",Val( oProd:_vDesc:TEXT  ),0)
		nVProd   := Val( oProd:_vProd:TEXT  )
		nVUnit   := nVProd / nQuant
		nDesc    := Round(nVDesc / nVProd * 100,2)
		_cdCfop  := iIf(Type("oProd:_CFOP:TEXT") != "U", oProd:_CFOP:TEXT ,"")
		_ldDev  := _cdCfop < '5000'
		_cdtes := getTes(_cdDoc,_cdCfop,_cdFil)
	   
		if Len(Alltrim(_cdtes)) == 0
			_ldErro := .T.
		EndIf
	   
		aLinha := {}
		aadd(aLinha,{"D2_ITEM"    ,"01"    ,Nil})
		aadd(aLinha,{"D2_COD"     ,cProduto,Nil})
		aadd(aLinha,{"D2_QUANT"   ,nQuant  ,Nil})
		aadd(aLinha,{"D2_PRCVEN"  , nVUnit ,Nil})
		aadd(aLinha,{"D2_TOTAL"   , nVProd ,Nil})
		aadd(aLinha,{"D2_TES"     ,_cdtes  ,Nil})
		aadd(aLinha,{"D2_DESCON"  ,nVDesc  ,Nil})
		aadd(aLinha,{"D2_CF"      ,_cdCfop ,Nil})

		aadd(aItenssD2,aLinha)
		
	ElseIf Type("_odNfe:_INFNFE:_DET") == "A"
		For _xdI := 01 To Len(_odNfe:_INFNFE:_DET)
			oProd := _odNfe:_INFNFE:_DET[_xdI]:_Prod
			cProduto := oProd:_cProd:TEXT
			nQuant   := Val( oProd:_qCom:TEXT )
			cUM      := oProd:_uCom:TEXT
			nVDesc   := iIf(Type("oProd:_vDesc:TEXT") != "U",Val( oProd:_vDesc:TEXT  ),0)
	        
			nVProd   := Val( oProd:_vProd:TEXT  )
			nVUnit   := nVProd / nQuant
			nDesc    := Round(nVDesc / nVProd * 100,4)
	   
			_cdCfop  := iIf(Type("oProd:_CFOP:TEXT") != "U", oProd:_CFOP:TEXT  ,"")
			_cdtes := getTes(_cdDoc,_cdCfop,_cdFil)
			_ldDev  := _cdCfop < '5000'
			aLinha := {}
			aadd(aLinha,{"D2_ITEM"  ,StrZero(_xdI,2),Nil})
			aadd(aLinha,{"D2_COD"   ,cProduto,Nil})
			aadd(aLinha,{"D2_QUANT" ,nQuant,Nil})
			aadd(aLinha,{"D2_PRCVEN", nVUnit,Nil})
			aadd(aLinha,{"D2_TOTAL" , nVProd,Nil})
			aadd(aLinha,{"D2_TES"   ,_cdTes,Nil})
			aadd(aLinha,{"D2_DESCON",nVDesc,Nil})
			aadd(aLinha,{"D2_CF"    ,_cdCfop,Nil})

			aadd(aItenssD2,aLinha)
			if Len(Alltrim(_cdtes)) == 0 .Or. _ldDev
				_ldErro := .T.
				_xdI := Len(_odNfe:_INFNFE:_DET) + 01
			EndIf
		
		Next
	EndIf
		
	Private lMsHelpAuto := .T. // Variavel de controle interno do ExecAuto
	Private lMsErroAuto := .F. // Variavel que informa a ocorrência de erros no ExecAuto
		
	//SetFunName("LOJA701")
	//Private Inclui := .T.
	
	//MSExecAuto({|a,b,c,d,e,f,g,h| Loja701(a,b,c,d,e,f,g,h)},.F.,3,"","",{},_aCab,_aItem ,_aParcela)
	if !_ldErro
		SM0->(RestArea(aSM0))
		MATA920(aCabecSF2,aItensSD2,3)
	Else
		lMsErroAuto := .T.
	EndiF
	//MSExecAuto({|x,y,z| MATA920(x,y,z)},aCabecSF2,aItensSD2,3)	
	
	_cdSucess := _cdPath + "Sucesso"
	_cdErro   := _cdPath + "Erro"
	_cdDev    := _cdPath + "Devolucao"
	_cdTES    := _cdPath + "Tes"
	
	If !ExistDir(_cdSucess)
		MakeDir(_cdSucess)
	EndIf
	_cdSucess = Alltrim(_cdSucess) + "\"
	
	If !ExistDir(_cdErro)
		MakeDir(_cdErro)
	EndIf
	_cdErro = Alltrim(_cdErro) + "\"
	
	If !ExistDir(_cdDev)
		MakeDir(_cdDev)
	EndIf
	_cdDev = Alltrim(_cdDev) + "\"
	
	If !ExistDir(_cdTES)
		MakeDir(_cdTES)
	EndIf
	_cdTES = Alltrim(_cdTES) + "\"
	
	if _ldDev
		_xdFile := StrTokArr(_cdFile,"\")
		_xdFile := _xdFile[Len(_xdFile)]
		fRename(_cdPath + _cdFile , _cdDev + _xdFile )
		_ldOk := .F.
	EndIf
	if _ldErro .And. !_ldDev
		_xdFile := StrTokArr(_cdFile,"\")
		_xdFile := _xdFile[Len(_xdFile)]
		fRename(_cdPath + _cdFile , _cdTES + _xdFile )
		_ldOk := .F.
	EndIf
	
	If lMsErroAuto .And. !_ldDev
		Mostraerro()
		DisarmTransaction()
		RollBackSx8()
		_ldOK := .F.
		 
		_xdFile := StrTokArr(_cdFile,"\")
		_xdFile := _xdFile[Len(_xdFile)]
		fRename(_cdPath + _cdFile , _cdErro + _xdFile )
	ElseIf !_ldDev
		_ldOK := .T.
		
		_xdFile := StrTokArr(_cdFile,"\")
		_xdFile := _xdFile[Len(_xdFile)]
		fRename(_cdPath + _cdFile , _cdSucess + _xdFile )
	EndIf
			
Return _ldOk



Static Function FormatXml(__cdXml,_cdTp)
	_xdXml  := " "
	_xdNfe  := '<NFe xmlns="http://www.portalfiscal.inf.br/nfe"> '
	_xdXml  := __cdXml
	_xdPos1 := At(_xdXml,"<NFe>")
   //_xdXml  := Replace(__cdXml,"<NFe>",_xdNfe)   
	_xdPos2 := At(_xdXml,"</infNFe>")
	_xdXml  := SubString(_xdXml,_xdPos1,_xdPos2 - _xdPos1 + 9)
   
	if _cdTp == "ERP"
		_xdXml := "</NFe>"
	ElseIf _cdTp == "SIG"
		_xdPos1 := At(Upper(__cdXml),Upper("<Signature") )
		_xdPos2 := At(Upper(__cdXml),Upper("</Signature>") )
	   
		_xdXml += SubString(__cdXml,_xdPos1,_xdPos2 - _xdPos1 + 12)
	EndIf
   
Return _xdXml


Static Function CreateParm()

	SX6->(RecLock('SX6',.T.))
	SX6->X6_FIL := cFilAnt
	SX6->X6_VAR := 'MV_XAFATX97'
	SX6->X6_TIPO := 'C'
	SX6->X6_DESCRIC := 'Define o Valor do Ultimo Arquivo Importado'
	SX6->X6_PROPRI := 'U'
	SX6->(MsUnlock())


	SX6->(RecLock('SX6',.T.))
	SX6->X6_FIL := cFilAnt
	SX6->X6_VAR := 'MV_XBFATX97'
	SX6->X6_TIPO := 'C'
	SX6->X6_DESCRIC := 'Define o Valor do Ultimo Diretorio Importado'
	SX6->X6_PROPRI := 'U'
	SX6->(MsUnlock())
Return

	
STATIC FUNCTION getTes(_cdDoc,_cdCfop,_cdFil)
	_sxArea := SX6->(getArea())
	_aArea  := getArea()
    
	_cdTable := "FIL" + _cdFil + "NFXCFOP"
	_cdQry := " "
	_cdQry += " SELECT * FROM " + _cdTable
	_cdQry += " WHERE  CFOP = '" + _cdCfop + "' "
	_cdQry += " And NNF = '" + _cdDoc + "' "
    
	_cdtbl := getNextAlias()
	//dbUseArea(.F., "TOPCONN", tcGenQry(,,_cdQry), _cdtbl , .F., .F.)
	dbUseArea(.T.,"TOPCONN",TcGenQry(,, _cdQry ),"XDRO",.T.,.T.)
	If !XDRO->(Eof())
		_cdTes := alltrim(Str(XDRO->TES))
	Else
		_cdTes := ""
	EndIf
	XDRO->(dbCloseArea("XDRO"))

	SX6->(RestArea(_sxArea ))
	RestArea(_aArea)
Return _cdTes


Static Function fGravaCli(cNomeCli,cCnpjCli,cEndCli,cNroCli,cUfCli,cMunCli,cBairroCli,cCepCli,cIECli) //,cFoneCli)
		
	RecLock("SA1",.T.)
	SA1->A1_FILIAL  := xFilial()
	SA1->A1_COD     := AACONTZE()
	SA1->A1_LOJA    := "01"
	SA1->A1_NOME    := cNomeCli
	SA1->A1_NREDUZ  := cNomeCli
	SA1->A1_PESSOA  := IIF(Len(Alltrim(cCnpjCli)) > 11, "J", "F")
	SA1->A1_TIPO    := "F"
	SA1->A1_TIPOCLI := ""
	SA1->A1_CGC     := cCnpjCli
	SA1->A1_END     := cEndCli+cNroCli
	SA1->A1_EST     := IIF(Alltrim(cUfCli) == "AM", "AM", "EX")
	SA1->A1_MUN     := cMunCli
	SA1->A1_COD_MUN := Posicione("CC2",2,XFILIAL("CC2")+SA1->A1_MUN,"CC2_CODMUN")
	SA1->A1_BAIRRO  := cBairroCli
	SA1->A1_CEP     := cCepCli
	SA1->A1_DDD     := ""
	SA1->A1_INSCR   := cIECli
	SA1->A1_CONTA   := ""
	SA1->A1_CODPAIS := "01058"
	MsUnLock()

Return


Static Function AACONTZE()
	Local cRet   := ""
	Local aArea  := getArea()
	Local cQry   := ""
	

	cQry += "	select ISNULL(MAX( SUBSTRING(A1_COD,1, 6) ),'HDS000') AS C2_NUM from "+RetSQLName("SA1")+" "
	cQry += "	WHERE A1_COD BETWEEN 'HDS000' AND 'HDS999' "
	cQry += "	AND D_E_L_E_T_ = ' ' "
 	
	dbUseArea(.T.,"TOPCONN",TcGenQry(,,ChangeQuery(cQry)),"TMP",.T.,.T.)
	
	cRet := Soma1(C2_NUM,,.T.)
	
	TMP->(dbCloseArea())
  	
	//dbSelectArea(cAlias)
	RestArea(aArea)
   
Return cRet


Static Function FLocalz(_cdCnpj,_cdNsc)
	Local cArea  := GetArea()
	Local cQry   := ""
	Local cRet   := ""

	cQry += "  SELECT TOP 1 A1_COD+A1_LOJA AS CODS
	cQry += "    FROM "+RetSQLName("SA1")+" WHERE D_E_L_E_T_ = ' ' "
	cQry += "    AND A1_FILIAL = '" + xFilial("SA1") + "' "
	cQry += "    AND LTRIM(RTRIM(A1_CGC)) = '" + ALLTRIM(_cdCnpj) + "' "
	cQry += "    AND LTRIM(RTRIM(REPLACE(REPLACE(Replace(A1_INSCR,'/',''),'.',''),'-',''))) = '" + ALLTRIM(_cdNsc) + "' "
	cQry += "    ORDER BY A1_COD
//cQry += "    ANd A1_MSBLQL != '1' "

	dbUseArea(.T.,"TOPCONN",TcGenQry(,, cQry ),"TMP",.T.,.T.)

	cRet :=  TMP->CODS

	TMP->(dbCloseArea())

	RestArea(cArea)

Return cRet

User Function AAFATX96()
Return Financeiro()
static function Financeiro()
     
	local aparcela := {"A","B","C","D","E","F","G","H","I","J","K","L","M","N"}
     
	cSql := " Select
	cSql += " CONVERT(VARCHAR(8),EMISSAO,112) EMISSAO,
	cSql += " TIPO,
	cSql += " PREFIXO,
	cSql += " NUMERO,
	cSql += " ISNULL(CONVERT(VARCHAR(8),VENCIMENTO,112),'') VENCIMENTO,
	cSql += " VALOR,
	cSql += " CLIENTE,
	cSql += " isNull(VENDEDOR,'000099') VENDEDOR,
	cSql += " ROW_NUMBER() over (PARTITION BY NUMERO ORDER BY VENCIMENTO )  PARCELA
	cSql += " from(
	cSql += " select * from CT_FINANCEIRO
	//cSql += " where P	REFIXO != 'PED'
	cSql += " union all
	cSql += " SELECT * from DC_FINANCEIRO
	cSql += " union all
	cSql += " SELECT EMISSÃO,ORÇAMENTO,TIPO,PREFIXO,NÚMERO,VENCIMENTO,VALOR,CLIENTE,VENDEDOR FROM PED_FINANCEIRO	
	cSql += " )A "

	cTab := getNextAlias()
	dbusearea(.T.,"TOPCONN",tcGenQry(,,cSql),cTab,.T.,.T.)
     
	WHILE !(cTab)->(Eof())
		cCliente  := PADR((cTab)->CLIENTE,6)
		cPrefixo  := PADR( (cTab)->PREFIXO,3)
		cNumero   := PADR( (cTab)->NUMERO ,9)
		cParcela  := PADR( aParcela[(cTab)->PARCELA], LEN(SE1->E1_PARCELA) )
		cTipo     := PADR( (cTab)->TIPO ,LEN(SE1->E1_TIPO))
		
		SA1->(dbSetOrder(1))
		if SA1->(dBSeek(xFilial('SA1') + cCliente ))
        
			_dEmissao := STOD( (cTab)->EMISSAO )
			dDatabase := STOD( (cTab)->EMISSAO )
			If Len( Alltrim( (cTab)->VENCIMENTO ))  == 0
			   SAE->(dbSetOrder(1))
			   If SAE->(dbSeek(xFilial('SAE') + cCliente ))
			      _dVencto  := STOD( (cTab)->EMISSAO ) + SAE->AE_DIAS
			      cCliente := SAE->AE_CODCLI
			   Else
			      _dVencto  := STOD( (cTab)->EMISSAO ) + iIf( cTipo == "CD",1,30)
			   EndIf
			Else
			   _dVencto := STOD( (cTab)->VENCIMENTO )			   
			EndIf
            _dVencto := iIf(  _dVencto <  _dEmissao,_dEmissao,_dVencto)
			SE1->(dbSetOrder(1))
			If !SE1->(dbSeek(xFilial('SE1') + cPrefixo + cNumero + cParcela + cTipo))
        
				aTitulo := {}
				aAdd(aTitulo,{"E1_PREFIXO  ", cPrefixo           		  ,nil})
				aAdd(aTitulo,{"E1_NUM      ", cNumero	        		  ,nil})
				aAdd(aTitulo,{"E1_PARCELA  ", cParcela     		  	      ,nil})
				aAdd(aTitulo,{"E1_TIPO     ", cTipo             		  ,nil})
  
				aAdd(aTitulo,{"E1_EMISSAO  ",  _dEmissao         ,nil})
				aAdd(aTitulo,{"E1_VENCTO   ",  _dVencto          ,nil})
				aAdd(aTitulo,{"E1_VENCREA  ",  _dVencto          ,nil})
				aAdd(aTitulo,{"E1_CLIENTE  ", SA1->A1_COD        		  ,nil}) //cCliente
				aAdd(aTitulo,{"E1_LOJA     ", SA1->A1_LOJA         		  ,nil})
				aAdd(aTitulo,{"E1_NOMCLI   ", SA1->A1_NREDUZ              ,nil})
				aAdd(aTitulo,{"E1_VALOR    ", (cTab)->VALOR     		  ,nil})
 
				aAdd(aTitulo,{"E1_VLCRUZ   ", (cTab)->VALOR       		  ,nil})
				aAdd(aTitulo,{"E1_MOEDA"    , 1             		  ,nil})
				aAdd(aTitulo,{"E1_MOVIMEN"  ,   _dEmissao ,nil})
 
				aAdd(aTitulo,{"E1_ORIGEM"   , "IMP2" 		   		  ,nil})
				aAdd(aTitulo,{"E1_VEND1"    , (ctab)->VENDEDOR 		   		  ,nil})
				aAdd(aTitulo,{"E1_SALDO"    , (cTab)->VALOR 	         	  ,nil})
				aAdd(aTitulo,{"E1_VENCORI"  , _dVencto  ,nil})
				aAdd(aTitulo,{"E1_EMIS1"    , _dEmissao   ,nil})
				aAdd(aTitulo,{"E1_FILIAL"   , xFilial("SE1")  	  ,nil})
		
		
				Private lMsHelpAuto := .T. // Variavel de controle interno do ExecAuto
				Private lMsErroAuto := .F. // Variavel que informa a ocorrência de erros no ExecAuto
	
				MsExecAuto({|x,y| FINA040(x,y) } , aTitulo, 3)
	
				If lMsErroAuto
					Mostraerro()
					DisarmTransaction()
					RollBackSx8()
				Else
					//alert('ok')
				EndIf
			EndIf
		EndIf
		(cTab)->(dbSkip())
		dDatabase := date()
		
	EndDo
Return

Static function validperg(cPerg)
	PutSX1(cPerg,"01","Financeiro?","",""   ,"mv_ch3","N",01,0,0,"C","","   ","","","mv_par03","Sim ","","","","Nao ","","","Ambos","")
Return Nil
