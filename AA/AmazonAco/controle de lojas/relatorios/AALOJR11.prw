...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦ Função    ¦ AALOJR11   ¦ Autor ¦                      ¦ Data ¦ 01/02/2011 ¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
...
User Function AALOJR11(lSetPrint,cPorta,cdExigOs,cdOrc,cdOrcTo,cdFilial,_wdrel,_xdFils,_xdOpt)
...
Private _xdFil  := _xdFils
Private _xdOpc  := _xdOpt
...
titulo   := "IMPRESSAO DO PEDIDO "+Iif(_xdOpt==01," (CHAPA CORTADA)","(OUTROS PRODUTOS)")
...
nLastKey := 0
cPerg    := Padr("AA_LOJR11", Len(SX1->X1_GRUPO) )
_cdNome  := "Nome"//iIf(Empty(_wdRel),"AALOJR11"+CUSERNAME,_wdRel)
...
/*    wnrel := SetPrint(cString,wnRel,cPerg,Titulo,cDesc1,cDesc2,cDesc3,.F.,,.F.,"P",,.F.,.F.,"EPSON.DRV", ,.F.,.F.,cPorta)
...
PortPrint			Caracter			Define uma porta de impressão padrão.
*/
//cPorta := "LPT5"
//alert(cPorta)
...
If lSetPrint .And. !Empty(cPorta)
	//wnrel := SetPrint(cString,_cdNome,cPerg,@Titulo,cDesc1,cDesc2,cDesc3,.T.,aOrd,.T.,Tamanho,,.F.,.F.,"EPSON.DRV",.T.,.T.,cPorta)
...
If Empty(_wdrel)
	...
	aDriver := ReadDriver() //Instrução caveira do mestre Ronilton
	@ 00,000 PSAY &(aDriver[aReturn[4]+2]) // Complemento da instru;áo
EndIf
_ldImpresso := .F.

	For nX:=1 To 2
		_ldImpresso := RunReport(lSetPrint,cdExigOs)
	Next nX
//RptStatus({|| _ldImpresso := RunReport(lSetPrint,cdExigOs) }, Titulo)

//If Empty(_wdrel) .And. _ldImpresso
...
While !(cTable)->(EOF())
	...
	If SL1->(DbSeek((cTable)->(L1_FILRES + L1_ORCRES)))
		...
		DescTotal  := 0
		_ndTotIcms := 0
		cChave := (cTable)->(L1_FILRES + L1_ORCRES )
		_ndTotDesc := 0
		While (cTable)->(!Eof()) .And. cChave == (cTable)->(L1_FILRES + L1_ORCRES )
			
			Conout("Produto:"+SL2->L2_PRODUTO)
			DbSelectArea("SB1")
			DbSetOrder(1)
		...
		while(li <= _ndMaxLin * nPag  + (nPag - 1)*1/* nCont*/ )
			@ li,108 PSay ""
			li++
		EndDo
		nCont++
...
@ li,001 PSay PADC("LISTA DE SEPARACAO DO ORCAMENTO "+ SL1->L1_NUM+ "       Data Emissao :" +DTOC(SL1->L1_EMISSAO)+" - "+Time(),nwTam)
li := li + 1
@ li,001 PSay Replicate("-",nwTam)
li := li + 1
Return
...
PutSX1(cPerg,"01","Orcamento De?","","","mv_ch1","C",06,0,0,"G","","SC5","","","mv_par01")
PutSX1(cPerg,"02","Orcamento Ate?","","","mv_ch2","C",06,0,0,"G","","SC5","","","mv_par02")
...
cQry2 +=   "   Left Outer Join " + RetSqlName("SL1") + " L1 on L1_ORCRES = L2_NUM and L1_FILRES = L2_FILIAL
...
cQry2 +=   "   Left Outer Join " + RetSqlName("SL1") + " (NOLOCK) L1B on L1B.L1_NUM = L1.L1_ORCRES and L1.L1_FILRES = L1B.L1_FILIAL And L1B.D_E_L_E_T_ = ''
...
cQry2 +=   " And L1.L1_ORCRES BetWeen '" + mv_par01 + "' And '" + mv_par02 + "'
cQry2 +=   " And L1.L1_FILRES = '" + mv_par03 + "'"
...
If !Empty(_xdFil)
	cQry2 +=   " And L1.L1_FILIAL In " + FormatIn(_xdFil,';')
EndIf
