...
IF lPe01Nfe     
	...
	//Diego Inicio
	// Compatibilização para adicionar os Vetores dos Impostos para serem aglutinados
	aAdd(aParam, aCST   )  	//19
	aAdd(aParam, aICMS  )	//20
	aAdd(aParam, aICMSST)	//21
	aAdd(aParam, aIPI   )	//22
	aAdd(aParam, aPIS   )	//23
	aAdd(aParam, aPISST )	//24
	aAdd(aParam, aCOFINS)	//25
	aAdd(aParam, aCOFINSST)	//26
	aAdd(aParam, aISSQN)	//27
	aadd(aParam, aExp)		//28
	aadd(aParam, aPisAlqZ)	//29
	aadd(aParam, aCofAlqZ)	//30
	aadd(aParam, aCsosn)	//31
	aadd(aParam, aICMSZFM)	//32
	aadd(aParam, aFCI)		//33   
	aadd(aParam, aISS)		//34     	
	// Diego Final
	...
	If ( Len(aParam) >= 5 )
		...
		// Especificos AA Corrigir as definiçõe Fixas quando ocorrer Mudanças no vetor Acima
		// Inicio
		aCST     := aParam[19]
		aICMS    := aParam[20]
		aICMSST  := aParam[21]
