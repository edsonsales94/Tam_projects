...
#DEFINE MAXITEMP2  068                                                // Mximo de produtos para a pagina 2 em diante
...
#DEFINE MAXITEMC   050                                                // Mxima de caracteres por linha de produtos/servios
...
#DEFINE MAXVALORC  010                                                // Mximo de caracteres por linha de valores numricos
...
Private _ldContigencia := .F.
Private lReimprime := .F.
			...
			For nX := 1 To nLenNotas
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						//-----------------------------------------------------------------------												
						oRetNF := XmlParser(EncodeUTF8(aXML[nX][2]),"_",@cAviso,@cErro)				 	
				...
				If ( !Empty(aXML[nX][2]) .AND. (AllTrim((cAliasSFT)->FT_ESPECIE)== 'SPED') .Or. (lImpDir .And. !Empty(aXML[nX][2])) ) .And. Empty((cAliasSFT)->FT_DTCANC)
					...
					ElseIf lIsLoja
						...
						If !Empty(aXML[nX][9]) .AND. Val(aXML[nX][9]) > 200 
							...
							cNaoAut := "A impressão do DANFE referente ao Doc/Série " + SF2->F2_DOC + "/" + SF2->F2_SERIE + " NÂO FOI REALIZADA pelo motivo abaixo:"							
							cNaoAut += CRLF + "[" + aXML[nX][9] + ' - ' + aXML[nX][10] + "]"
							cNaoAut += CRLF + "Se possível, faça o ajuste e retransmita a NF-e."
...
Private lArt274SP := .F. 
	...
	oEmitente:_xNome:TEXT := Alltrim( StrTran(StrTran(oEmitente:_xNome:TEXT," ACO"," AÇO"),"COMERCIO","COMÉRCIO") )
...
aTotais[07] := Transform(Val(oTotal:_ICMSTOT:_vSeg:TEXT),"@e 9,999,999,999,999.99")
	...
	//Diego a Pedido do Marcio para exibir no campo de desconto o valor do icms desonerado
	// pois na atualizao de agosto/2016 o mesmo deixou de ser exibido para ser informado
	// nas Informaes adicionas da NF-e no Campo Dados Adicionais no Rodap
	// Adicionado as 5 linhas abaixo para computar o valor no campo de Desconto novamente
	IF(oNF:_INFNFE:_VERSAO:TEXT >= "3.10")
...
For nZ := 1 To nLenDet
	...
	EndIf
		...
		If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
			_cdAux := SubStr(oDet[nX]:_Infadprod:TEXT,1)
			_adAdic := aClone(StrToKArr(_cdAux,"#***") )
			If Len(_adAdic) >= 03
				_cdInfAdic := _adAdic[03]
				aadd(aItens,{_adAdic[01],;
				NoChar(_adAdic[02],lConverte),;
				"",;
				"",;
				"",;
				"",;
	...
	If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
			...
			cAux := stripTags(AllTrim(SubStr(_cdInfAdic,1)), .T.)
			If at("<", AllTrim(SubStr(_cdInfAdic,1))) <> 0
				cAux := stripTags(AllTrim(SubStr(_cdInfAdic,1)), .T.) + " "
				cAux += stripTags(AllTrim(SubStr(_cdInfAdic,1)), .F.)
			else
				cAux := stripTags(AllTrim(SubStr(_cdInfAdic,1)), .T.)
			EndIf
...
If GetNewPar("MV_BCREFIS",.F.) .And. SuperGetMv("MV_ESTADO")$"PR"
	If Val(&("oTotal:_ICMSTOT:_VBCST:TEXT")) <> 0
		cAux := "Substituição Tributária: Art. 471, II e 1 do RICMS/PR: "
   		nLenDet := Len(oDet)
   		For nX := 1 To nLenDet
	   		oImposto := oDet[nX]
	   		If Type("oImposto:_Imposto")<>"U"
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	nLinCalc+=10
		If cEmpAnt $ "01,02,03" 
			//oDanfe:Say(nLinCalc,098, "Fone: (92) 2129-9898 ", oFont08n:oFont)
		ElseIf cEmpAnt $ "05"
			//oDanfe:Say(nLinCalc,098, "Fone: (92) 3613-1000 ", oFont08n:oFont)
		EndIf
...
Else
	...
	EndIf
	nLinCalc+=10
	//oDanfe:Say(nLinCalc,098, "Site: www.amazonaco.com.br",oFont08N:oFont)
	nLinCalc+=10

	If cEmpAnt $ "01,02,03" 
		//oDanfe:Say(nLinCalc,098, "e-mail: amazonaco@amazonaco.com.br",oFont08N:oFont)   
	ElseIf cEmpAnt $ "05"
		//oDanfe:Say(nLinCalc,098, "e-mail: amazonaco@amazonaco.com.br",oFont08N:oFont)
	EndIf
...
If nfolha==1
	If lMv_Logod
			oDanfe:SayBitmap(045,005,cLogoD,242,090)//oDanfe:SayBitmap(045,005,cLogoD,090,090)
	Else
			oDanfe:SayBitmap(045,005,cLogoD,242,090)//oDanfe:SayBitmap(045,005,cLogoD,090,090)
	EndIF
Endif
...
oDanfe:Say(729,352,"RESERVADO AO FISCO",oFont08N:oFont)
    ...
    If lReimprime .And. !_ldContigencia
		cBitMap := "\SYSTEM\fundo.Bmp" 
		oDanfe:SayBitmap(760,360,cBitMap,230,100) // teste impressao logo tipo no fundo da danfe
		lReimprime := .F.
	Endif
	If _ldContigencia
		cBitMap := "\SYSTEM\fundocontigencia.Bmp" 
		oDanfe:SayBitmap(760,360,cBitMap,230,100) // teste impressao logo tipo no fundo da danfe
		_ldContigencia := .F.
	EndIf
//?
