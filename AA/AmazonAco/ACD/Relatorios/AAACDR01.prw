...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦ Função    ¦ AAACDR01   ¦ Autor ¦                      ¦ Data ¦ 01/02/2011 ¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦ Descriçäo ¦ Comprovante de Entrega do pedidos de venda                  	¦¦¦
¦¦+-----------+---------------------------------------------------------------+¦¦
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
User Function AAACDR01(_cdPedido,_cdFilial,_cdOrcamento,_cdFilialO,lSetPrint,lBloc,cOrde)
	...
	Li       := 01
	nLastKey := 0
	cPerg    := Padr("AAACDR01", Len(SX1->X1_GRUPO) )
...
Static Function RunReport(lSetPrint)
	...
	if !aRet[01]
		While (cTable)->(!EOF())
			...
			lExec := .F.
			lExec := AAFATP07(SC5->C5_FILIAL,SC5->C5_NUM,cOrdem)
		...
		//tudo que esta na SC9, que nao esta bloqueado
			...
			While (cTable)->(!Eof()) .And. cChave == (cTable)->(L2_FILIAL + L2_NUM + L1_PEDRES)
				...
				//			   @ li,132 - 12 PSay "Pedido"
					...
					//nTotal1  += _ndQtdLib1 //_ndQtdLib1 //SC6->C6_QTDLIB //RAGE
		...
		While (cTable)->(!Eof()) .And. cChave == (cTable)->(L2_FILIAL + L2_NUM + L1_PEDRES)
			...
			If SB1->(DbSeek(xFilial("SB1")+(cTable)->L2_PRODUTO))
				...
				MaFisAdd( SB1->B1_COD        ,; 	  // 1-Codigo do Produto ( Obrigatorio )
					...
					MaFisAlt("IT_PESO"   ,_ndQtdLib1*SB1->B1_PESO,Len(aItemPed))
					//MaFisAlt("IT_PESO"   ,_ndQtdLib1,Len(aItemPed)) RAGE
			        MaFisAlt("IT_PRCUNI" ,nPrcUnit,Len(aItemPed))
...
// Função que soma a quantidade de produtos com Orcamentos em abertos e NAO vencidos
	...
	cQry += "  Left Outer Join  " + RetSqlName("SL1") + " (NOLOCK) L1  on L1_ORCRES = L2_NUM and L1_FILRES = L2_FILIAL AND L1.D_E_L_E_T_='' "
	cQry += "  Left Outer Join  " + RetSqlName("SL2") + " (NOLOCK) L2B on L2B.L2_NUM = L1.L1_NUM and L1.L1_FILIAL = L2B.L2_FILIAL and L2B.L2_PRODUTO = L2.L2_PRODUTO And L2B.L2_QUANT = L2.L2_QUANT AND L2B.D_E_L_E_T_='' "
	...
	cQry += "  Left Outer Join  " + RetSqlName("SC6") + " (NOLOCK) C6  on  C6.D_E_L_E_T_=''  AND  C6.C6_PRODUTO = L2.L2_PRODUTO AND C6.C6_NUM = L1.L1_PEDRES and C6.C6_QTDVEN = L2.L2_QUANT AND C6.D_E_L_E_T_='' "
	cQry += "  Left Outer Join  " + RetSqlName("SC9") + " (NOLOCK) C9  on  C9.D_E_L_E_T_=''  AND  C9.C9_PRODUTO = C6.C6_PRODUTO AND C6.C6_NUM = C9.C9_PEDIDO And C6.C6_ITEM = C9.C9_ITEM AND C9.D_E_L_E_T_='' "
...
//  cQry += "       And L2.L2_FILIAL  = '" + SL2->(xFilial()) + "'
	...
	//cQry := StrTran(cQry,"Or","And")
		...
		If (cTable)->(EOF())
			...
			cQry += "    Left Outer Join " + RetSqlName("SBM") + " (NOLOCK) SBM ON SBM.D_E_L_E_T_='' AND SBM.BM_GRUPO = SB1.B1_GRUPO
			cQry += "    Left Outer Join " + RetSqlName("SC9") + " (NOLOCK) C9  ON C9.D_E_L_E_T_=''  AND  C9.C9_PRODUTO = C6.C6_PRODUTO And C6.C6_NUM = C9.C9_PEDIDO And C6.C6_ITEM = C9.C9_ITEM
...
Static Function AAFATP07(_cFilial,_cPedido,cOrdem)
	Local cQuery := ""
	...
	If Empty(SC5->C5_XORCRES)
		...
		If lRet := (SA1->A1_RISCO != 'A')
			...
			If lRet
				Aviso('Atencao','Ordem de carregamento Encontra-se Bloqueada por Credito, Verifique com o Setor Financeiro',{'OK'},1)
			EndIf
		Endif
	Endif
