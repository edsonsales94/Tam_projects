...
cTitulo        := "TRANSFERENCIA DE MATERIAIS INJEÇÃO PLASTICA"
cAliasEnchoice := "SB8"
cAliasGetD     := "SB1"
cLinOk         := "AllwaysTrue()"
cTudOk         := "AllwaysTrue()"
...
While nOpca == 1
	cProd    := Space(40) //Space(Len(SB1->B1_COD))//Space(15)
	cLote    := Space(10)
	...
	@ 75,50 Get nQtd Picture "@E 999,999.99" Size 50,10 Valid VldQtd() When .T.   
   	...
   	ACTIVATE MSDIALOG oDlgA ON INIT EnchoiceBar(oDlgA,{||nOpca:=1,If(.T.,nOpca:=1,nOpca := 0),oDlgA:End()},{||nOpca := 0,oDlgA:End()},,aButtons) Centered
    	if nOpca == 1         
 		if MsgYesNo("Transferido Produto : "  +ALLTRIM(cProd)+  "  - Quantidade :   " +cValtoChar(nQuant),"Atenção")     
