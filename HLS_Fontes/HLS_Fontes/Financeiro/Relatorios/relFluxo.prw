...
Static Function RepDef()
	...
	oAplica := TRSection():New(oSaidas, "APLICAÇÃO", {"TMP1"}, /*[Ordem]*/)       	
	oAplica:SetReadOnly()	
	oAplica:AutoSize()						
	TRCell():New(oAplica, "A6_NOME",		"TMP1", "APLICAÇÃO", 			PesqPict("SA6", "A6_NOME"), 		TamSX3("A6_NOME")[1])
	
	oTotais := TRSection():New(oAplica, "TOTAL", {"TMP1"}, /*[Ordem]*/)       	
	oTotais:SetReadOnly()	
	oTotais:AutoSize()						
...
Static Function RepPrint(oReport)
	...
	Local oAplica	  	:= oReport:Section(1):Section(1):Section(1):Section(1)
	Local oTotais	  	:= oReport:Section(1):Section(1):Section(1):Section(1):Section(1)
	Local nSldAux		:= 0		
	...
	While TMP1->(!Eof())				
		...
		EndIf		
		For nH := 1 to len(aDatas)
			nSldAux := retSldBanco(dtos(aDatas[nH][01]),TMP1->A6_COD,TMP1->A6_NUMCON)
    ...
    For nH := 1 to len(aDatas)
    	TRCell():New(oAplica,dtoc(aDatas[nH][01]),,dtoc(aDatas[nH][01]), "@E 9,999,999.99", 12)
    Next
	...
	While TMP1->(!Eof())				
		...
		EndIf		
		For nH := 1 to len(aDatas)
			nSldAux := retAplica(dtos(aDatas[nH][01]),TMP1->A6_COD,TMP1->A6_NUMCON)
			aDatas[nH][05] += nSldAux
	    	oAplica:Cell(dtoc(aDatas[nH][01])):SetValue(nSldAux)
	    Next
	    oAplica:Cell("A6_NOME"):SetValue(TMP1->A6_NOME)
		oAplica:PrintLine()
		TMP1->(dbSkip())		
	EndDo		
	oAplica:Cell("A6_NOME"):SetValue("Total Aplicações")
	For nH := 1 to len(aDatas)
    	oAplica:Cell(dtoc(aDatas[nH][01])):SetValue(aDatas[nH][05])
    Next
    oAplica:PrintLine()
	oAplica:Finish()
