...
±±ºPrograma  ºCSVMenu   º Autor º Mauro Paladini Han º Data ³  23/02/04   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºObjetivo  ³Lista em formato CSV o conteudo dos arquivos XNU visando umaº±±
±±º          ³melhor visualizacao das rotinas disponiveis p/ os usuarios  º±±
...
User Function CSVMenu()
	...
	Local cPerg := Padr("XNU000001",Len(SX1->X1_GRUPO))
...
Static Function RunReport()
	...
	//³ A funcao XNUADMIN	Load ira retornar para o      ³
    ...
    For z := 1 To Len(aMenuAll)
		...
		For a := 1 To Len(aEstrutura)
			...
			For b := 1 To Len(aGrupos)
						...
						For c := 1 To Len(aFuncoes)
							IF Len(aFuncoes[c]) >= 7
								...
								aEval( aAlias,{ |h| cArquivo += h + " " },,)
					  			If	aFuncoes[c][2]=="E"    
					  	            vQuery := " insert into MENUS (MENU, GRUPO, MODO, TIPO, FUNCAO, ACS, MODULO,ARQS,DESCRI,A1,A2,A3,A4,A5,A6,A7,A8,A9,A0,LOGIN,NOME,DEPTO,CARGO,EMAIL,R_E_C_N_O_ )  VALUES('"+aMenuAll[z,1]+"','"+RTrim(aFuncoes[c][1][1])+"','"+aFuncoes[c][2]+"','"+cTipo+"','"+Upper(RTrim(aFuncoes[c][3]))+"','"+Upper(aFuncoes[c][5])+"','"+RTrim(aFuncoes[c][6])+"','"+RTrim(cArquivo)+"','"+RTrim(cGrupo)+"','"+Substr(Upper(aFuncoes[c][5]) ,1,1)+"','"+substr(Upper(aFuncoes[c][5]) ,2,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,3,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,4,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,5,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,6,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,7,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,8,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,9,1)+"','"+Substr(Upper(aFuncoes[c][5]) ,10,1)+"','"+RTrim(aMenuAll[z,2])+"','"+RTrim(Capital(aMenuAll[z,3]))+"','"+RTrim(Capital(aMenuAll[z,4]))+"','"+RTrim(Capital(aMenuAll[z,5]))+"','"+RTrim(lower(aMenuAll[z,6]))+"',"+ALLTRIM(STR(XIZ))+") "
									...
									/*			DbSelectArea("MENUS")
									RecLock("MENUS",.T.)
										MENUS->MENU	:= aMenuAll[z,1]
										MENUS->DESCRI	:= RTrim(aFuncoes[c][1][1])
										MENUS->MODO		:= aFuncoes[c][2]
										MENUS->TIPO		:= cTipo
										MENUS->FUNCAO	:= Upper(RTrim(aFuncoes[c][3]))
										MENUS->ACS		:= Upper(aFuncoes[c][5])
										MENUS->MODULO	:= RTrim(aFuncoes[c][6])
										MENUS->ARQS		:= RTrim(cArquivo)
										MENUS->GRUPO	:= RTrim(cGrupo)
										MENUS->A1		:= Substr(Upper(aFuncoes[c][5]) ,1,1)
										MENUS->A2		:= Substr(Upper(aFuncoes[c][5]) ,2,1)						
										MENUS->A3		:= Substr(Upper(aFuncoes[c][5]) ,3,1)						
										MENUS->A4		:= Substr(Upper(aFuncoes[c][5]) ,4,1)
										MENUS->A5		:= Substr(Upper(aFuncoes[c][5]) ,5,1)
										MENUS->A6		:= Substr(Upper(aFuncoes[c][5]) ,6,1)												
										MENUS->A7		:= Substr(Upper(aFuncoes[c][5]) ,7,1)
										MENUS->A8		:= Substr(Upper(aFuncoes[c][5]) ,8,1)
										MENUS->A9		:= Substr(Upper(aFuncoes[c][5]) ,9,1)
										MENUS->A0		:= Substr(Upper(aFuncoes[c][5]) ,10,1)						
										MENUS->LOGIN	:= RTrim(aMenuAll[z,2])
										MENUS->NOME		:= RTrim(Capital(aMenuAll[z,3]))
										MENUS->DEPTO	:= RTrim(Capital(aMenuAll[z,4]))
										MENUS->CARGO	:= RTrim(Capital(aMenuAll[z,5]))
										MENUS->EMAIL	:= RTrim(lower(aMenuAll[z,6]))
									MsUnLock()	*/							
								Endif
							EndIf
						Next
				...
				Else 
				cGrupo    := ""//aGrupos[b][1][1]    
			    vQuery := " Insert into MENUS (MENU, GRUPO, MODO, TIPO, FUNCAO, ACS, MODULO,ARQS,DESCRI,"+;
				    				"A1,A2,A3,A4,A5,A6,A7,A8,A9,A0,LOGIN,NOME,DEPTO,CARGO,EMAIL,R_E_C_N_O_)"+;
				    				"  VALUES('"+aMenuAll[z,1]+"','"+;
				    				RTrim(cGrupo)+"','"+RTrim(aGrupos[b][2])+"','"+;
				    				cTipo+"','"+RTrim(Upper(RTrim(aGrupos[b][3])))+"','"+;
