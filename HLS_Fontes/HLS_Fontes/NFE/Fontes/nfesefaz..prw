			...
			Else
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							alltrim(SB1->B1_CODMAT2)+' '+cDescProd,;
							...
							SC6->C6_PEDCLI,SC6->C6_NUMPCOM,; // [50],[51] PEDICLI
							})
...
//Lfss 20-01-2023
If  !Empty(aProd[50])
	cString += '<xPed>'+ConvType(aProd[50])+'</xPed>'
ENDIF
 If  !Empty(aProd[51])
	cString += '<nItemPed>'+ConvType(aProd[51])+'</nItemPed>'
Endif
//  Mudanca de tag
