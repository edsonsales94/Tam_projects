...
////////////////////////////////////////////////////////////////////////////////////
//                                                                                //
//  Função:    ZZABSENT 										Módulo: SIGAPON   //
//                                                                                //
//  Autor:     Rodrigo Caetano  								Data: 04/03/2019  //
//                                                                                //
//  Descrição: Relatório de Absenteísmo baseado no fonte padrão ABSENT, onde foi  //
//             incluído tratamento para geração em Excel e não considerar deter-  //
//             minados abonos no Cálculo                                          //
//                                                                                //
//  Uso:       Específico da DTMIT                                                //
//                                                                                //
////////////////////////////////////////////////////////////////////////////////////
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
wnrel := 'ZZABSENT' //-- Nome Default do relatorio em Disco
...
_cArqExcel := ""
_aCabExcel := {}
_cCabExcel := "Matricula;Nome;Periodo;Hrs.Prev.;Hrs.Real;%;(1)Hrs.Adic.;%;(2)Hrs.N.Trab.;%;(3)Hrs.Abonadas;%;"
_cIteExcel := ""
//Pergunta se deseja gerar o relatório em Excel
_lExcel := If(!lSintetico,GeraExcel(),.F.)
MV_PAR01 := _cMV_PAR01
MV_PAR02 := _cMV_PAR02
//Excel - Modelo 2
Private oFWMsExcel
...
While SRA->(!Eof()) .And. &cInicio <= cFim
	...
	For nX := 1 To Len(aPeriodos)
		...
		If ! lSintetico
			...
			If _lExcel
				If _nModExcel == 1
					aAdd(_aFunc,{SRA->RA_MAT,1,nX,If(nX==1,SRA->RA_MAT+";"+Left(SRA->RA_NOME,20),";")+";"+;
									StrZero(Month(aPeriodos[nX,2]),2)+'/'+Transf(Year(aPeriodos[nX,2]),'9999')+";"+;
									Transf(nHrsPrev, "@E 999.99")+";"+;
									Transf(nHrsReal, "@E 999.99")+";"+Transf(nPReal,"@E 999.99")+";"+;
									Transf(nHrsAdic, "@E 999.99")+";"+Transf(nPAdic,"@E 999.99")+";"+;
									Transf(nHrsnTra, "@E 999.99")+";"+Transf(nPNTra, "@E 999.99")+";"+;
									Transf(nHrsAbon, "@E 999.99")+";"+Transf(nPAbo, "@E 999.99")+";"})
				ElseIf _nModExcel == 2
					aAdd(_aFunc,{SRA->RA_MAT,1,nX,;
									If(nX==1,SRA->RA_MAT,""),;
									If(nX==1,Left(SRA->RA_NOME,20),""),;
									StrZero(Month(aPeriodos[nX,2]),2)+'/'+Transf(Year(aPeriodos[nX,2]),'9999'),;
									Round(nHrsPrev,2),;
									Round(nHrsReal,2),;
									Round(nPReal,2),;
				...
				ElseIf _nModExcel == 3
					aAdd(_aFunc,{	SRA->RA_CC,;
									...
									StrZero(Month(aPeriodos[nX,2]),2)+'/'+Transf(Year(aPeriodos[nX,2]),'9999'),;
									cTno,;
									Posicione("SR6",1,xFilial("SR6")+cTno,"R6_DESC"),;
									Round(nHrsPrev,2),;
									Round(nHrsReal,2),;
									Round(_aTotMod3[1],2),;  //AtestM<15 
									Round(_aTotMod3[2],2),;  //Hrs.Injust
									Round(_aTotMod3[3],2),;  //Falta Just
									Round(_aTotMod3[4],2),;  //Suspensão
									Round(_aTotMod3[5],2),;  //Aus Legal
									Round(nPReal,2),;  //Real %
									If(SRA->(FieldPos("RA_XGESTOR"))>0,Posicione("ZZZ",1,xFilial('ZZZ')+SRA->RA_XGESTOR,'ZZZ_NOME'),""),;  //Nome Gestor
									If(SRA->(FieldPos("RA_XGERBP"))>0,SRA->RA_XGERBP,"")})  //Nome BP
	...
	If lDiscri .And. !lSintetico .And. (Len(aAdic)+Len(aNTra)+Len(aAbon)) > 0
	  //	AAdd(aImp, STR0010) //'(1) Descricao                  Horas      % (2) Descricao                  Horas      %  (3) Descricao                  Horas      %'
		...
		For nX := 1 To Max(Max(Len(aAdic), Len(aNTra)),Len(aAbon))
			...
			If _lExcel
				For _i := 1 To 3
					_cTexto := &("cColuna0"+AllTrim(Str(_i)))
					If !Empty(_cTexto)
						_nPos := aScan(_aCabExcel,Left(_cTexto,23))
						If _nPos == 0
							aAdd(_aCabExcel,Left(_cTexto,23))
							_cCabExcel += Left(_cTexto,23)+";"+"%;"
							_nPos := Len(_aCabExcel)
						EndIf
						If _i == 1
							If _nModExcel == 1
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Transf(aAdic[nX,3],"@E 99999.99")+";"+Transf(aAdic[nX,4],"@E 999.99")+";"})
							ElseIf _nModExcel == 2
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Round(aAdic[nX,3],2),Round(aAdic[nX,4],2)})
							EndIf
						ElseIf _i == 2
							If _nModExcel == 1
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Transf(aNTra[nX,3],"@E 99999.99")+";"+Transf(aNTra[nX,4],"@E 999.99")+";"})
							ElseIf _nModExcel == 2
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Round(aNTra[nX,3],2),Round(aNTra[nX,4],2)})
							EndIf
						Else
							If _nModExcel == 1
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Transf(aAbon[nX,3],"@E 99999.99")+";"+Transf(aAbon[nX,4],"@E 999.99")+";"})
							ElseIf _nModExcel == 2
								aAdd(_aFunc,{SRA->RA_MAT,2,_nPos,Round(aAbon[nX,3],2),Round(aAbon[nX,4],2)})
							EndIf
						EndIf
					EndIf
				Next
...
If _lExcel
	...
	If _nModExcel == 1
		...
		If Len(_aFunc) > 0
			...
			_aFunc := aSort(_aFunc,,,{ |x,y| x[1]+Str(x[2])+Str(x[3]) < y[1]+Str(y[2])+Str(y[3]) })
			_nPosAnt := 0
			_cMatAnt := ""
			For _i := 1 To Len(_aFunc)
				If _aFunc[_i,1] == _cMatAnt
					If _aFunc[_i,2] == 1
						_cIteExcel += CRLF
					EndIf
				Else
					If !Empty(_cMatAnt)
						For _k := _nPosAnt To Len(_aCabExcel)-1
							_cIteExcel += "0;0;"
						Next
						_cIteExcel += CRLF
						_nPosAnt := 0
				...
				EndIf
				If _aFunc[_i,2] == 1
					_cIteExcel += _aFunc[_i,4]
				Else
					If _aFunc[_i,3] - _nPosAnt > 1
						_nPosAnt++
						For _j := _nPosAnt To _aFunc[_i,3]-1
							_cIteExcel += "0;0;"
						Next
					EndIf
					_cIteExcel += _aFunc[_i,4]
					_nPosAnt := _aFunc[_i,3]
				EndIf
				_cMatAnt := _aFunc[_i,1]
			Next

			If !Empty(_cIteExcel) .And. _nPosAnt # 0
				For _k := _nPosAnt To Len(_aCabExcel)-1
					_cIteExcel += "0;0;"
				Next
				_cIteExcel += CRLF
			...
			MemoWrite( _cArqExcel, _cCabExcel + CRLF + _cIteExcel )
			If !ApOleClient("MsExcel")
				MsgBox("Microsoft Excel Não Instalado nesta Máquina !!!", "Atenção !!!", "INFO")
			Else
				oExcelApp := MsExcel():New()
				oExcelApp:WorkBooks:Open(_cArqExcel)
	...
	If _nModExcel == 2
	    ...
	    oFWMsExcel:AddworkSheet("ZZABSENT")
	    
	    _cPlanilha := "Grupo de Empresa: "+AllTrim(SM0->M0_NOME)+" / Filial: "+AllTrim(SM0->M0_FILIAL)+Space(Len(_aCabExcel)*30)+"Absenteísmo - "+Transf(dDataDe,'@E')+' - '+Transf(dDataAte,'@E')+Space(Len(_aCabExcel)*30)+"Emissão: "+DToC(MsDate())
	    
        oFWMsExcel:AddTable("ZZABSENT",_cPlanilha)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Matrícula",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Nome",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Periodo",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Hrs.Prev.",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Hrs.Real",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"%",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"(1)Hrs.Adic.",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"%",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"(2)Hrs.N.Trab.",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"%",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"(3)Hrs.Abonadas",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"%",3,2)

        For _i := 1 To Len(_aCabExcel)
	        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,_aCabExcel[_i],3,2)
	        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"%",3,2)
        Next

		_aFunc   := aSort(_aFunc,,,{ |x,y| x[1]+StrZero(x[2],6)+StrZero(x[3],6) < y[1]+StrZero(y[2],6)+StrZero(y[3],6) })
		_nPosAnt := 0
		_cMatAnt := ""
		_aDados   := {}
        ...
        For _i := 1 To Len(_aFunc)
			...
			If _aFunc[_i,1] == _cMatAnt
				If _aFunc[_i,2] == 1
					For _m := 1 To Len(_aCabExcel)
						aAdd(_aDados,0)
						aAdd(_aDados,0)
					Next
					oFWMsExcel:AddRow("ZZABSENT",_cPlanilha,_aDados)
					_aDados := {}
					_nPosAnt := 0
				EndIf
			Else
			...
			If _aFunc[_i,2] == 1
				For _l := 4 To Len(_aFunc[_i])
					aAdd(_aDados,_aFunc[_i,_l])
				Next
			Else
				If _aFunc[_i,3] - _nPosAnt > 1
					_nPosAnt++
					...
					Next
				EndIf
				aAdd(_aDados,_aFunc[_i,4])
				aAdd(_aDados,_aFunc[_i,5])
				_nPosAnt := _aFunc[_i,3]
			EndIf
	    ...
	    _cArqExcel:=StrTran(_cArqExcel,".csv",".xml")
	    If !".xml" $ _cArqExcel
	    	_cArqExcel += ".xml"
	    EndIf
	    oFWMsExcel:Activate()
	    oFWMsExcel:GetXMLFile(_cArqExcel)
	         
		If !ApOleClient("MsExcel")
			MsgBox("Microsoft Excel Não Instalado nesta Máquina !!!", "Atenção !!!", "INFO")
		Else
		    oExcel:=MsExcel():New()
		    oExcel:WorkBooks:Open(_cArqExcel)
		    oExcel:SetVisible(.T.)
		    oExcel:Destroy()
		EndIf
	...
	If _nModExcel == 3
	    ...
	    _cPlanilha := "Grupo de Empresa: "+AllTrim(SM0->M0_NOME)+" / Filial: "+AllTrim(SM0->M0_FILIAL)+Space(60)+"Absenteísmo - "+Transf(dDataDe,'@E')+' - '+Transf(dDataAte,'@E')+Space(60)+"Emissão: "+DToC(MsDate())
	    
        oFWMsExcel:AddTable("ZZABSENT",_cPlanilha)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"C. Custo",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Descrição C. Custo",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Matrícula",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Nome Completo",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Data Admissão",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Período/Competência",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Turno",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Descrição de Turno",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Horas Prevista",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Hrs.Real",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"AtestM<15",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Hrs.Injust",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Falta Just",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Suspensão",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Aus Legal",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Real %",3,2)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Nome Gestor",1)
        oFWMsExcel:AddColumn("ZZABSENT",_cPlanilha,"Nome BP",1)

		_aFunc   := aSort(_aFunc,,,{ |x,y| x[1]+x[3]+x[6] < y[1]+y[3]+y[6] })

        For _i := 1 To Len(_aFunc)

			oFWMsExcel:AddRow("ZZABSENT",_cPlanilha,_aFunc[_i])
...
_cAbono	:= cAlias+'->'+Right(cAlias,2)+'_ABONO'	    //-- Campo abono
...
If dbSeek(fFilFunc(cAlias)+SRA->RA_MAT)
	...
	Do While &cFil + &cMat == SRA->RA_FILIAL + SRA->RA_MAT
		...
		//Verifica se existe campo para considerar no Cálculo de Absenteísmo
		If SP6->(FieldPos("P6_ZZABSEN"))>0
			If Posicione("SP6",1,xFilial("SP6")+&_cAbono,"P6_ZZABSEN") == "2"
				(cAlias)->( dbSkip() )
				Loop
			EndIf
		EndIf

		If SP6->(FieldPos("P6_ZZABSRE"))>0 .And. _nModExcel == 3
			_nZZABSRel := Val(Posicione("SP6",1,xFilial("SP6")+&_cAbono,"P6_ZZABSRE"))
			If _nZZABSRel >= 1 .And. _nZZABSRel <= 5
				_aTotMod3[_nZZABSRel] += nQuant
			EndIf
		EndIf
...
aAdd( aParBox, { 2, "Gerar em Excel?"	,  2	,	{"1=Não gera","2=Modelo 1","3=Modelo 2","4=Modelo 3"}	,	100	, 'Pertence("1234")', .T.	 } )&& MV_PAR01
aAdd( aParBox, { 1, "Arquivo Destino" 	,  Space(254)	,	""	,	"MV_PAR02:=U_TelaArq()"	, ""		,	"",	100,	.F.	 } )&& MV_PAR02

If ParamBox( aParBox, "Geração de Relatório em Excel",NIL,,,,,,,"ZZABSENT",.T.,.T.)
	If ValType( MV_PAR01 ) == "N"
		...
		EndIf
	EndIf
	_lRet 		:= Val(SubStr(MV_PAR01,1,1)) > 1
	_nModExcel 	:= Val(SubStr(MV_PAR01,1,1)) - 1
	_cArqExcel 	:= MV_PAR02	
EndIf
...
///////////////////////////////
User Function TelaArq(oProcess)	
///////////////////////////////

cArqTxt	:=	cGetFile( "Arquivos .CSV | *.CSV ", 'Informe o arquivo a ser gerado.', ,'C:\TEMP\', .F., nOR( GETF_LOCALHARD, GETF_LOCALFLOPPY, GETF_NETWORKDRIVE ) )
