...
¦ Função    ¦ HLCTBP10   ¦ Autor ¦ Ulisses Junior           ¦ Data ¦ 18/06/2009 ¦
+-----------+------------+-------+--------------------------+------+------------+
¦ Descriçäo ¦ Geração do arquivo de relação de produção baseado nas notas de    ¦
¦           ¦ saída e estruturas.                                               ¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
User Function HLCTBP10()
Local nOpcao  := 0
Local aSay    := {}
Local aButton := {}
...
Private lEnd      := .F.
Private NomeProg  := "HlCtbP10"
Private lCopia    := .F.
Private cPerg     := "HLB010"
...
If nOpcao == 1
	Processa({|| ArqHlb10() }, "Aguarde...", "Processando informações...", .T. )
Endif

Return NIL

/*_______________________________________________________________________________
¦ Função    ¦ ArqHlb10   ¦ Autor ¦ Ulisses Junior           ¦ Data ¦ 18/06/2009 ¦
+-----------+------------+-------+--------------------------+------+------------+
¦ Descriçäo ¦ Geração arquivo temporátio base para montagem do arquivo final    ¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
Static Function ArqHlb10()
Private aProd := {},aAux := {}
...
¦ Descriçäo ¦ Geração arquivo temporátio base para montagem do arquivo final    ¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
Static Function ExpEstr(cProduto)
Local nReg, nSaldo, cProd
...
While !SG1->(Eof()) .and. SG1->G1_COD = cProduto
	...
	If ddatabase >= SG1->G1_INI .and. ddatabase <= SG1->G1_FIM
		...
		SG1->(dbGoto(nReg))
  		...
  		aadd(aProd,{SG1->G1_COD,SG1->G1_COMP,SG1->G1_QUANT})
	EndIf	
	SG1->(dbSkip())
...
cFileName := cPath +"RelProdV2.txt"
cTmpFile  := cPath +"RelProdV2.tmp"
