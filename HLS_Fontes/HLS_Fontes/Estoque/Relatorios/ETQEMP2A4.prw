...
//					oPrint:Say(_LI, _CO + 750 + ((1090 - 750) / 2),cValToChar(_nTotQtd),	oFont21N,,, 1, 2) //VFV
				  	MSBAR("CODE128",  _LIMSBAR + 4.0, _COMSBAR + 7.9, cValToChar(_nTotQtd), oPrint, .F., NIL, NIL, 0.027, 0.8, NIL, NIL, "A", .F.)
	   				ELSE
	   				//oPrint:Say(_LI, _CO + 750 + ((1090 - 750) / 2),cValToChar(QtdTot),	oFont21N,,, 1, 2) //VFV    
	   				MSBAR("CODE128",  _LIMSBAR + 4.0, _COMSBAR + 7.9,cValToChar(QtdTot), oPrint, .F., NIL, NIL, 0.027, 0.8, NIL, NIL, "A", .F.)
	   				ENDIF  
	   				ENDIF    
	    //---
		  /*	If SB1->B1_LADO $ "LR"
...
//-------------------------------------------------------------------------------------------------------------------
		oPrint:Say(_LI,_CO + (690 / 2), "CÓD DE BARRAS HLS",				oFont13,,, 1, 2)    
