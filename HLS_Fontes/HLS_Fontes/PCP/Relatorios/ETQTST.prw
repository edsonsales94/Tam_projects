...
Static Function SelRegs(nRegs)
    ...
    */
    ...
    _cQuery += 	"		    SB1.B1_QE, SB1.B1_COD, SB1.B1_CODMATR, SB1.B1_DESC, SB1.B1_LADO "
    _cQuery += 	" 	FROM "
...
//    _cQuery += 	"      C2_SEQUEN = '001' AND SC2.D_E_L_E_T_ = ' ' AND B1_TIPO = 'PI' AND B1_LOCPAD = '06'" // Alterado por Luciano Lamberti - 13-03-2019 (alteração dos PI´s por PP - bloco K)
    _cQuery += 	"      C2_SEQUEN = '001' AND SC2.D_E_L_E_T_ = ' ' AND B1_TIPO = 'PP' AND B1_LOCPAD = '06'" 
    _cQuery += 	"   ORDER BY "
...
Static Function ImpEtq(nQtdEtq, nSeq, cLote, nL, nC)
 ...
 //	MSBAR("CODE128", nLiMsBar + 4, nCoMsBar + 4, cTexto, oPrint, .F., NIL, NIL, 0.027, 0.8, NIL, NIL, "A", .F.) //VFV
	...
	oPrint:Line(nLi, nCo, nLi, nCo + 1090)						// Linha Horizontal
	nLi += 50
   	oPrint:Say(nLi, nCo, Space(10) + Substr(TSC2->B1_DESC, 01, 40) + TSC2->B1_LADO,			oFont13 )
