...
user function ETQQR2()					//etiqueta QR CODE com filtro para armazem e impressao apenas do código do produto -- Etiqueta do P.O.
...
Static Function imprRel()
	...
	Private aColImp			:= {}
	Private oPrinter 		:= FWMSPrinter():New("HLRPCP02")
	...
	Private nWidth			:= 290 //200    //alteração Luciano - 28-03-2019
	Private nHeight			:= 210 //200    //alteração Luciano - 28-03-2019
	...
	Private oFont24N 		:= TFont():New("Arial", 24, 24,, .T.,,,,, .F., .F.)
   	...
   	DEFINE MSDIALOG oDlg TITLE "Etiquetas Expedição" FROM 0,0 TO 400,800 PIXEL
		...
		While (cAlOP)->(!Eof())		
			...
			For nG := 1 to nQtdOP
				...
				Else //imprime lado esquerdo
					impDados(aColImp[11]-20,nLin,nG,nQtdOP)						//LL ACOLIMP valor 20 desloca a etiqueta para à direita da página
					nLin += 1050 //990    													//ESPAÇAMENTO ENTRE ETIQUETAS (LL)
					//oPrinter:Say(nLin,aColImp[11],str(nG)+"/"+str(nQtdOP),oFont14N)				
					If mod(nPosAtu,6) == 0     										//QUANTIDADE DE ETIQUETAS POR PÁGINA - VALOR 0 NAO ALTERA(LL) 
						oPrinter:EndPage()						
						oPrinter:StartPage()
						nLin := 010														//L
					EndIf
...
Static Function impDados(nPosVert,nLinAtu,nG,nQtdOP,cQtd)
	...
	//(inibido por Gilson Belini) Local cSequencia 	:= allTrim(str(nG))+"/"+allTrim(str(nQtdOP))
//	Local nRest			:=	0
	...
	nLinAtu += 080
	oPrinter:say(nLinAtu+005,nPosVert+015,"CÓDIGO HLS",oFont14) 
	...
	//oPrinter:say(nLinAtu+005,nPosVert+415,(cAlOP)->B1_CODMATR,oFont20N)	
	//nLinAtu += 080
	//oPrinter:say(nLinAtu+005,nPosVert+015,"DATA DA ENTREGA",oFont14)					RETIRADO A DATA DE ENTREGA A PEDIDO DE JOCILENE EM 20/04/2021
	//oPrinter:say(nLinAtu+005,nPosVert+415,dToc(stod((cAlOP)->C2_DATPRF)),oFont20N)
	oPrinter:say(nLinAtu+005,nPosVert+015,"N° OP",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+415,(cAlOP)->C2_NUM,oFont16N)
	//nLinAtu += 080
	//oPrinter:say(nLinAtu+005,nPosVert+715,"N° OP",oFont14)
	//oPrinter:say(nLinAtu+005,nPosVert+825,(cAlOP)->C2_NUM,oFont16N)
	nLinAtu += 080
	...
	cCodBar := (cAlOP)->B1_COD

	oPrinter:say(nLinAtu+005,nPosVert+115,allTrim(cValtoChar(nQtdResto)),oFont24N) //add Luciano 24-07-2019i		
	
	Else
		
		cCodBar := (cAlOP)->B1_COD
		
	oPrinter:say(nLinAtu+005,nPosVert+115,allTrim(cValtoChar((cAlOP)->B1_QE)),oFont24N) //add Luciano 24-07-2019i
	
	EndIf
	
	nLinAtu += 010
	oPrinter:say(nLinAtu+005,nPosVert+125,"___________",oFont14)		//ajuste para o desenho de um quadrado / retangulo para inserção da sequencia
	nLinAtu += 035														//solicitação feita pela area de injeção plastica em 29/09/21
	oPrinter:say(nLinAtu+005,nPosVert+120,"|",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+330,"|",oFont14)
	nLinAtu += 035
	oPrinter:say(nLinAtu+005,nPosVert+120,"|",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+330,"|",oFont14)
	nLinAtu += 005
	oPrinter:say(nLinAtu+005,nPosVert+015,"SEQ.",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+125,"___________",oFont14)
	nLinAtu += 075
	...
	oPrinter:say(nLinAtu+005,nPosVert+125,(cAlOP)->B1_LADO,oFont16N)	
	nLinAtu += 080
	oPrinter:say(nLinAtu+005,nPosVert+015,"C.P.I.I.:  ",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+135,(cAlOP)->C2_ZZCPII,oFont16N)
	nLinAtu += 080
	oPrinter:say(nLinAtu+005,nPosVert+015,"Motivo C.P.I.I.:  ",oFont14)
	oPrinter:say(nLinAtu+005,nPosVert+245,(cAlOP)->C2_ZZCPMOT,oFont16N)	
		
	nLinAtu += 010
	
	oPrinter:QRCode(nLinAtu+010, nPosVert+500, cCodBar, 90)
...
Static Function buscaDados()
    ...
    cQuery += 	"		    SB1.B1_QE, SB1.B1_CODMATR, SB1.B1_DESCHL, SB1.B1_LADO, SB1.B1_COD, SC2.C2_IMPETQ, SC2.C2_ZZCPII, SC2.C2_ZZCPMOT " + CRLF
    cQuery += 	" 	FROM " + CRLF
	...
	memoWrite("\SQL\HLRPCP02.SQL",cQuery)
...
Static Function ValidPerg()
	...
	If ParamBox(aParamBox,"HLRPCP02",@aRet,,,,,,,"HLRPCP02",.T.,.T.)
		lRet := .t.
	EndIf
