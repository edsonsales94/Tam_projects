...
Funcao principal para impressao de Etiquetas por Ordem de Producao. Imprime PI.
...
Static Function SelRegs(nRegs)
 ...
 //   _cQuery += 	"      C2_SEQUEN = '001' AND SC2.D_E_L_E_T_ = ' ' AND B1_TIPO = 'PI' AND B1_LOCPAD = '05' "// Alterado por Luciano Lamberti - 13-03-2019 (alteração dos PI´s por PP - bloco K)
    _cQuery += 	"      C2_SEQUEN = '001' AND SC2.D_E_L_E_T_ = ' ' AND B1_TIPO = 'PP' AND B1_LOCPAD = '05' "
    _cQuery += 	"   ORDER BY "
...
Static Function ImpEtq(nQtdEtq, nSeq, cLote, nL, nC)
   ...
   //	MSBAR("CODE128", nLiMsBar, nCoMsBar + 1.9 , cTexto, oPrint, .F., NIL, NIL, 0.027, 1.10, NIL, NIL, "A", .F.)
	...
	cTexto1 := TSC2->C2_NUM+C2_ITEM+C2_SEQUEN
	cTexto := TSC2->C2_NUM
 //	MSBAR("CODE128", nLiMsBar + 4, nCoMsBar + 4, cTexto, oPrint, .F., NIL, NIL, 0.027, 0.8, NIL, NIL, "A", .F.) //VFV
// 	MSBAR("CODE128", nLiMsBar + 3.2, nCoMsBar + 2.8, cTexto1, oPrint, .F., NIL, NIL, 0.020, 0.8, NIL, NIL, "A", .F.)
MSBAR("CODE128", nLiMsBar + 4, nCoMsBar + 4, cTexto1, oPrint, .F., NIL, NIL, 0.020, 0.8, NIL, NIL, "A", .F.)
	oPrint:Line(nLi, nCo, nLi, nCo + 1090)						// Linha Horizontal
	nLi += 50
