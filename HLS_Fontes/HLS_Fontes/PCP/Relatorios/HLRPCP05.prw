...
Etiquetas de Produtos Acabados para Expedição 
...
±±ºPrograma  HLRPCP05 º Autor ³Luciano Lamberti      º Data ³   22/01/21  º±±
...
user function HLRPCP05()	// Este programa imprime as etiquetas zebra QRCODE com quebra de embalagem
   	...
   	DEFINE MSDIALOG oDlg TITLE "Etiquetas Expedicao" FROM 0,0 TO 400,800 PIXEL
		...
		While (cAlOP)->(!Eof())		
			...
			For nG := 1 to nQtdOP
				...
				//MSCBWRITE("^FT39,542^A0N,34,33^FH\^FDEMB^FS "+(cAlOP)->MV_PAR08+"^FS") // inserido o campo para quebra de embalagem - 17/12/2020
				...
				If nG == nQtdOP .And. nQtdResto > 0
					...
					MSCBWRITE("^FT138,328^A0N,51,50^FH\^FD"+cValtoChar(nQtdResto)+"^FS") 
					...
					//MSCBWRITE("^FT138,328^A0N,51,50^FH\^FD"+allTrim(cValtoChar((cAlOP)->B1_QE))+"^FS")	
				...
				//MSCBWRITE("^FT138,328^A0N,51,50^FH\^FD"+allTrim(cValtoChar((cAlOP)->C2_QUANT))+"^FS") 
				MSCBWRITE("^FT138,382^A0N,34,33^FH\^FD"+cSequencia+"^FS")
...
Static Function buscaDados()
	...
	memoWrite("\SQL\HLRPCP05.SQL",cQuery)
...
Static Function ValidPerg()
	...
	//aAdd(aParamBox,{1,"Qtd Embalagem "	   						,space(2),"","","","",50,.F.})					// MV_PAR08	     
	//aAdd(aParamBox,{1,"Qtd Embalagem "	   						,2, aQtdEmb, 60, ".T.", .T.})						// MV_PAR08	 
