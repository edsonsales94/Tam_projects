...
WsRestful HLWEST01 Description "Integração Cadastro de Produto Fluig"
    ...
    WsData  enterprise  As String   Optional
...
WsMethod POST IncluiProduto WsService HLWEST01
	...
	If FWJsonDeserialize(cBody , @oRequest) 
        ...
        If FWFilExist(oRequest:enterprise, oRequest:branch) //FWFilExist(cEmpAnt, oRequest:branch) // Alterado por Anizio Cunha 11/01/2025

            cFilAnt := oRequest:branch  
 
            oModel := FwLoadModel ("MATA010")
            xcGrupo:= oRequest:B1_GRUPO
            xCodProduto:= U_TRETCOD(xcGrupo)

            oModel:SetOperation(MODEL_OPERATION_INSERT)
            
            oModel:Activate()

            oModel:SetValue("SB1MASTER", "B1_GRUPO",        /*oRequest:B1_GRUPO*/ xcGrupo)
            oModel:SetValue("SB1MASTER", "B1_COD",          xCodProduto)//oModel:SetValue("SB1MASTER", "B1_COD",      oRequest:B1_COD)
            ...
            oModel:SetValue("SB1MASTER", "B1_CODGTIN",      oRequest:B1_CODGTIN)
            oModel:SetValue("SB1MASTER", "B1_ORIGEM",       oRequest:B1_ORIGEM)
            oModel:SetValue("SB1MASTER", "B1_TE",           oRequest:B1_TE)
            oModel:SetValue("SB1MASTER", "B1_XSYSMAT",      oRequest:B1_XSYSMAT)
            oModel:SetValue("SB1MASTER", "B1_XDESCST",      oRequest:B1_XDESCST)            
            ...
            If oModel:VldData()
                ...
                /*oResponse["code"]               := 200
                oResponse["message"]            := EncodeUTF8("Cod. Produto no Protheus: "+ Alltrim(xCodProduto) +" incluso com sucesso")
                oResponse["detailedMessage"]    := EncodeUTF8("Produto incluso com sucesso")*/
                oResponse["produto"]  := xCodProduto
            Else
...
WsMethod PUT AlteraProduto WsService HLWEST01
	...
	If FWJsonDeserialize(cBody , @oRequest) 
        ...
        If FWFilExist(oRequest:enterprise, oRequest:branch) 

            cFilAnt := oRequest:branch  
 
            oModel := FwLoadModel ("MATA010")

            xCodProd:= oRequest:B1_COD
            lAlterCod:= U_tValCod(cFilAnt,oRequest:B1_GRUPO,oRequest:B1_COD)
            If lAlterCod
                ...
                If oModel:VldData()
                    ...
                    //oResponse["code"]               := 200
                    oResponse["message"]            := EncodeUTF8("cod. Produto: "+ Alltrim(xCodProd) +" alterado com sucesso")
                    oResponse["detailedMessage"]    := EncodeUTF8("Produto alterado com sucesso")
                ...
                Else
                    ...
                    oResponse["code"]               := 400
                    oResponse["message"]            := EncodeUTF8("Erro na alteração do produto")
                    oResponse['detailedMessage']    := EncodeUTF8(AllTrim(aError[MODEL_MSGERR_MESSAGE]) +"/"+ AllTrim(aError[MODEL_MSGERR_SOLUCTION]))
            ...
            Else 
                oResponse["code"]               := 400
                oResponse["message"]            := EncodeUTF8("Erro na Alteração do produto")
                oResponse['detailedMessage']    := EncodeUTF8("Grupo / Produto: "+ oRequest:B1_GRUPO+" / "+oRequest:B1_COD +" não encontrado!!!")
            EndIf       
