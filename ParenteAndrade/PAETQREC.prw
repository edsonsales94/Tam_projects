...
Static Function xfImpPA()
	...
	Local cLabel  := ""
	Local cPrinter := alltrim(MV_PAR09) //compartilhamento da impressora na rede
	...
	// cria a pasta etiquetas no servidor.
	if !existDir(cDir)
		makeDir(cDir)
	endif

	// cria a pasta temp local.
	if !file('C:\TEMP')
		makeDir('C:\TEMP')
	EndIf
	...
	for nX := 1 to MV_PAR07
		...
		// cPrinterPath:= alltrim(cNomePC)+"\ZEBRA-ZD220"
