...
User Function PROJ_CONTATOS(cProjIn, cA1In, cLojaIn)
    ...
    // Executa a View MVC
    FWExecView("Contatos do Projeto " + _cProj, "PROJ_01", MODEL_OPERATION_UPDATE, , {|| .T.})

    RestArea(aArea)
Return

User Function PROJ_01()
	Local aArea   := GetArea()
	Local oBrowse
...
Static Function _MVC_LoadZP8(cProj)
    ...
    If _MVC_OpenAlias("ZP8")
        ...
        If lHasOrd
            ...
            While ! ZP8->(EoF()) .and. ZP8->ZP8_FILIAL == cFil .and. ZP8->ZP8_CODPRO == cProj .and. ZP8->(Deleted()) == .F.
                aAdd(aRet, { 0, { ;
    ...
    If _MVC_OpenAlias("ZP8")
        ...
        Else
            If RecLock("ZP8", .T.)
                ...
                If FieldPos("ZP8_CODPRO")   >0 ; ZP8->ZP8_CODPRO    := cProj ; EndIf
...
Static Function _MVC_FindInZP8(cProj, cId, cFil, nRecnoOut)
    ...
    If lHasOrd
        ZP8->( dbSeek( cFil + cProj ) )
        While ! ZP8->(EoF()) .and. ZP8->ZP8_FILIAL == cFil .and. ZP8->ZP8_CODPRO == cProj
    ...
    Else
        ...
        While ! ZP8->(EoF())
            If ZP8->(Deleted()) == .F. .and. ZP8->ZP8_FILIAL == cFil .and. ;
               ZP8->ZP8_CODPRO == cProj .and. AllTrim(ZP8->ZP8_CONTID) == cId
                nRecnoOut := ZP8->( Recno() )
                lFound := .T.
                Exit
            EndIf
