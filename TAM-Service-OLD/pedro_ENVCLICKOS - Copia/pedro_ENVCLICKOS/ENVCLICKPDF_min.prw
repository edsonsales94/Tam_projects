...
/*/{Protheus.doc} User Function ENVCLICKPDF
    Gera UM PDF contendo N OS (grupo da ENVCLICKOS), reusando exatamente o layout do ITFATR04.
    @param aRegs    , array de JsonObject: Z1_FILIAL, Z1_CODIGO, Z1_TECNICO, Z1_CLIENTE, Z1_LOJA
    @param cFileBase, caractere: nome base do arquivo (SEM ".pdf")
    @param cOutDir  , caractere: diretório de saída
    @param lPreview , lógico  : se .T., abre Preview; caso contrário, gera silencioso
...
User Function ENVCLICKPDF(aRegs, cFileBase, cOutDir, lPreview)
    ...
    // === Contexto e variáveis EXATAMENTE como no ITFATR04 ===
    ...
    // === Sanitiza o nome base (sem .PDF) e usa FileName() original ===
    If Upper(Right(AllTrim(cFileBase),4)) == ".PDF"
        cFileBase := Left(AllTrim(cFileBase), Len(AllTrim(cFileBase)) - 4)
    EndIf
    cFileName := FileName(cFileBase)

    // === Instancia o printer UMA vez (como no ITFATR04) ===
    ...
    For n := 1 To IIf(ValType(aRegs) == "A", Len(aRegs), 0)
        ...
        If SZ1->(DbSeek(cFil + cCod))
            ...
            SA1->(DbSetOrder(1))
            If SA1->(DbSeek(xFilial("SA1")+SZ1->(Z1_CLIENTE+Z1_LOJA)))
    ...
    // === Finalização idêntica ao ITFATR04 ===
    oPrinter:Preview()
