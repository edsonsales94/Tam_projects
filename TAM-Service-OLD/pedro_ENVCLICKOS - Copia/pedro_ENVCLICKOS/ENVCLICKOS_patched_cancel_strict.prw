...
WSRESTFUL OSCLICKENV DESCRIPTION "Envio de OS apontadas para Clicksign – pré-seleção"
    ...
    // POST: cancelar documentos do envelope e limpar SZ1 se confirmado
    WSMETHOD POST cancelEnvelope DESCRIPTION ;
        "Cancela documentos no Clicksign e, somente após confirmação, limpa SZ1.Z1_ZP7ENV" ;
...
WSMETHOD POST cancelEnvelope WSRECEIVE WSREST OSCLICKENV
    ...
    If Empty(AllTrim(cBody))
        SetRestFault(400, "Body JSON obrigatório", .T.)
        Return .F.
    EndIf

    If !oReq:FromJson(cBody)
        SetRestFault(400, "JSON inválido. Esperado {\"envelope\": \"<id>\"}", .T.)
        Return .F.
    EndIf

    cEnv := AllTrim(oReq["envelope"])

    If Empty(cEnv)
        SetRestFault(400, "Campo 'envelope' obrigatório.", .T.)
        Return .F.
    EndIf

    // 1) Listar documentos do envelope
    aDocs := FT_ClickV3_ListDocs(cEnv, @aHttp)
    nDocs := Len(aDocs)

    // 2) Tentar cancelar todos os documentos (status := canceled)
    If nDocs > 0
        nCanc := FT_ClickV3_CancelDocs(cEnv, aDocs, @aHttp)
    EndIf

    // 3) Confirmar cancelamento via polling curto (até 8s)
    lOK := FT_ClickV3_ConfirmCanceled(cEnv, aDocs, 8, 1000, @aHttp)

    // 4) Apenas se confirmado, limpar SZ1.Z1_ZP7ENV das OS
    If lOK
    ...
    oRet["ok"]              := lOK
    oRet["env_id"]          := cEnv
    oRet["docs_total"]      := nDocs
    oRet["docs_cancelados"] := nCanc
    oRet["os_limpos"]       := nLimpos
    oRet["http_log"]        := aHttp
...
/* Lista documentos de um envelope. Retorna array de IDs. Acumula logs HTTP. */
Static Function FT_ClickV3_ListDocs(cEnvId, aHttp)
    Local aIds   := {}
    Local cUrl   := CKS_BASE_V3 + "/envelopes/" + cEnvId + "/documents"
    Local nCode  := 0
    Local cErr   := ""
    Local cResp  := ""
        ...
        If oJson:FromJson(cResp)
            If ValType(oJson["data"]) == "A"
                ...
                For i := 1 To Len(aData)
                    If ValType(aData[i]) == "O" .and. ValType(aData[i]["id"]) == "C"
                        AAdd(aIds, AllTrim(aData[i]["id"]))
                    EndIf
                Next
            EndIf
        EndIf
...
/* Cancela (PATCH status=canceled) todos os docs listados. Retorna quantos retornaram 2xx. */
Static Function FT_ClickV3_CancelDocs(cEnvId, aDocIds, aHttp)
    Local nOK   := 0
    Local i     := 0
    Local cDoc  := ""
    Local cUrl  := ""
    ...
    For i := 1 To Len(aDocIds)
        ...
        cUrl  := CKS_BASE_V3 + "/envelopes/" + cEnvId + "/documents/" + cDoc
        cResp := FT__HttpJSON("PATCH", cUrl, oBody:ToJson(), @nCode, @cErr)
        AAdd(aHttp, {"cancel_doc", nCode, IIf(Empty(cResp), cErr, FT__LogTrunc(cResp))})
...
/* Confirma se TODOS os documentos do envelope estão com status = canceled.
   Tenta 'nMaxTries' vezes, dormindo 'nSleepMs' entre cada tentativa. */
Static Function FT_ClickV3_ConfirmCanceled(cEnvId, aDocIds, nMaxTries, nSleepMs, aHttp)
    Local nTry    := 0
    Local i       := 0
    Local cDoc    := ""
    Local cUrl    := ""
    ...
    For nTry := 1 To nMaxTries
        lAll := .T.
        For i := 1 To Len(aDocIds)
            cDoc  := aDocIds[i]
            cUrl  := CKS_BASE_V3 + "/envelopes/" + cEnvId + "/documents/" + cDoc
            cResp := FT__HttpJSON("GET", cUrl, "", @nCode, @cErr)
            AAdd(aHttp, {"doc_detail", nCode, IIf(Empty(cResp), cErr, FT__LogTrunc(cResp))})
            ...
            oJson := JsonObject():New()
            If oJson:FromJson(cResp)
                If ValType(oJson["data"]) == "O" .and. ;
                   ValType(oJson["data"]["attributes"]) == "O" .and. ;
                   ValType(oJson["data"]["attributes"]["status"]) == "C"
                    If Upper(AllTrim(oJson["data"]["attributes"]["status"])) != "CANCELED"
                        lAll := .F.
                        Exit
                    EndIf
                Else
...
/* Limpa SZ1->Z1_ZP7ENV para todas as OS que referenciam cEnvId. Retorna quantas OS alteradas. */
Static Function FT_SZ1_ClearEnvById(cEnvId)
    ...
    cSQL := "SELECT R_E_C_N_O_ AS RECNO FROM " + cSZ1 + " WITH (NOLOCK) " + ;
            "WHERE D_E_L_E_T_ = '' AND Z1_ZP7ENV = '" + cEnvId + "'"
