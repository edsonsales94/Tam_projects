...
// ------------------------------- SERVICE ------------------------------------
WSRESTFUL OSCLICKENV DESCRIPTION "Envio de OS apontadas para Clicksign  pr-seleo"

    // GET sem parmetros: ltimos 15 dias, agrupado por TECNICO+PROJETO+CONTATO
    ...
    // GET sem parmetros: mesmos dados, mas gerando 1 PDF por grupo
    ...
    // GET criar envelope ? anexar documento ? incluir signatrio ? enviar
    ...
    // POST cancelar envelope (cancela docs e limpa SZ1.Z1_ZP7ENV somente após confirmar)
...
WSMETHOD POST testPdfOS WSRECEIVE WSREST OSCLICKENV
    ...
    If nRecno <= 0
        ConOut("[testPdfOS] RECNO invlido")
        SetRestFault(400, "Valor de 'recno' invlido.", .T.)
        Return .F.
    EndIf
    ...
    oJsonRet['pdf']   := cPdf
    If !lOk
        oJsonRet['err'] := "Arquivo pode ter sido gerado em caminho relativo do servidor (sem permisso/preview)."
    EndIf
...
WSMETHOD GET listCandidatas WSRECEIVE WSREST OSCLICKENV
   // Local lOk        := .T.
    ...
    ConOut(LOG_TAG + "INCIO GET /os (candidatas)")
...
WSMETHOD GET gerarPdfs WSRECEIVE WSREST OSCLICKENV
    ...
    ConOut(LOG_TAG + "INCIO GET /os/pdf (gerar PDFs)")
    ConOut(LOG_TAG + "Janela => dtini=" + DToS(dIni) + ", dtfim=" + DToS(dFim))

    // Agrupamento por Analista/Consultor + Projeto + Aprovador[Contato]
    aGroups := FT_SelectGroupSZ1_ComContato(dIni, dFim)

    // Diretrio de sada (ex.: RootPath\SPOOL\OSCLICK\)
    cDirOut := FT_GetOutDir()
    ConOut(LOG_TAG + "Sada de PDF em: " + cDirOut)
    ...
    For i := 1 To Len(aGroups)
        ...
        If Len(aRegs) > 0
            ...
            // Janela real da OS (datas mnima e mxima dos registros do grupo)
            FT_MinMaxDatas(@aRegs, @cDtIniOS, @cDtFimOS)

            // Nomes "bonitos" para cliente e consultor
            cNomeCli := FT_GetNomeCliente(cCli, cLoja)
            cNomeTec := FT_Consultor(cTec) // se no mapear, retorna o prprio cdigo

            // Padro de nome exigido pela regra de negcio:
            // PROJETO + NOME CLIENTE + NOME CONSULTOR + DT INCIO + DT FIM
            cFileBase := FT_MakeDocName(cProj, cNomeCli, cNomeTec, cDtIniOS, cDtFimOS) + ".pdf"

            // Gera um CAMINHO nico (evita sobreposio) mantendo o padro de base
            cFilePath := FT_UniquePath(cDirOut, cFileBase)

            // Extrai apenas o NOME do arquivo (sem diretrio)
            nSep      := Max(RAt("\", cFilePath), RAt("/", cFilePath))
            cFileName := SubStr(cFilePath, nSep + 1)

            // Alguns geradores esperam o nome SEM extenso
            ...
            // Gerao efetiva:
            // 1 parmetro = diretrio de sada
            // 2 parmetro = APENAS o nome (sem diretrio); aqui passamos SEM extenso por compatibilidade
            lOkPdf := FT_GeraPdfGrupo(cDirOut, cFileNameNoExt, oG)

            // Monta item de retorno (arquivo informado com .pdf, que  o que ser salvo ao final)
...
Static Function FT_SelectGroupSZ1_ComContato(dIni, dFim)
    ...
    ConOut(LOG_TAG + "FT_SelectGroupSZ1_ComContato  INCIO. dtini=" + cDtI + ", dtfim=" + cDtF)

    If Empty(cSZ1)
        ConOut(LOG_TAG + "ERRO: RetSqlName('SZ1') vazio (tabela no encontrada)")
        Return aGroups
    EndIf
    ...
    If Empty(cAliasQ) .or. Select(cAliasQ) == 0
        ConOut(LOG_TAG + "ERRO: MpSysOpenQuery no retornou alias vlido (SZ1)")
        Return aGroups
    EndIf
...
Static Function FT_GetContatosAgendaByCodigo(cAgdId)
    ...
    cAliasQ := MpSysOpenQuery(cSQL)
    If Empty(cAliasQ) .or. Select(cAliasQ) == 0
        ConOut(LOG_TAG + "ERRO: MpSysOpenQuery no retornou alias vlido (ZP9)")
        Return aContatos
    EndIf
...
Static Function FT_GetOutDir()
    ...
    // Cria em duas etapas; se j existir, MakeDir() apenas retorna .F. e segue.
    If !Empty(cBase)
        MakeDir(cBase + "\SPOOL")
        MakeDir(cBase + "\SPOOL\OSCLICK")
    Else
        // Caso extremo: RootPath no definido  ainda assim tenta criar no relativo
        MakeDir("\SPOOL")
...
Static Function FT_SanitizeName(cName)
    ...
    // remove acentos bsicos e caracteres invlidos para nome de arquivo
    c := StrTran(c, " ", "_")
    c := StrTran(c, "/", "-")
    c := StrTran(c, "\\", "-")
    c := StrTran(c, ":", "-")
...
/* Busca nome do consultor (por enquanto, retorna o prprio cdigo do tcnico) */
Static Function FT_Consultor(cTec)
Return AllTrim(cTec)

/* -------------------------- GERAO BSICA DE PDF ------------------------- */
/* Gera um PDF simplificado do grupo, listando as OS do perodo.
   - cDirOut: diretrio (deve estar dentro do RootPath para evitar erro do FWMSPrinter)
   ...
   Retorna .T. se gerou PDF; se falhar, tenta gravar .TXT com o mesmo contedo. */
/* Gera 1 PDF NICO por grupo (usando layout ITFATR04) */
Static Function FT_GeraPdfGrupo(cDirOut, cFile, oGroup)
    ...
    // Observao: aRegs  o array de objetos criado pelos mtodos GET (contm Z1_* completos) 
    Begin Sequence
...
/* Date -> AAAAMMDD; se vier C, devolve como veio; seno "" */
...
/* Clicksign v3: cria envelope, sobe PDF, adiciona signatrio e envia.
   cPdfPath  : caminho local do PDF
   cNomeEnv  : nome do envelope (exibio)
   cEmail/cNomeSigner: signatrio (aqui usamos override de teste)
...
Static Function FT_ClickV3_SendEnvelope(cPdfPath, cNomeEnv, cEmail, cNomeSigner)
    ...
    cB64 := FT__B64Clean(cB64) // remove CR/LF/espaos do base64
    If Empty(cB64)
...
/* ===== 3.1) Adicionar requisito (ligar signatrio ao documento) ===== */
    oAttr := JsonObject():New()
    oAttr["action"]       := "agree"        // requisito de concordncia/rubrica
   // oAttr["role"]        := "sign"   // papel do signatrio
...
/* ===== 3.2) Adicionar requisito de AUTENTICAO (ex.: token por e-mail) ===== */
    oAttr := JsonObject():New()
    oAttr["action"]   := "provide_evidence"
    // mtodo de autenticao desejado (opes comuns: "email_token", "sms_token", "whatsapp_token", "pix", "certificate")
    ...
    // relaciona o MESMO documento e signatrio usados no 3.1
    oRel     := JsonObject():New()
    oDoc     := JsonObject():New()
    oDocData := JsonObject():New()
    oDocData["type"] := "documents"
...
Static Function FT__HttpJSON(cMethod, cUrl, cBody, nCode, cErr)
    ...
    // POST usa HTTPSPost (no exige cert se passar "" nos 3 primeiros params)  TDN. :contentReference[oaicite:1]{index=1}
    If Upper(AllTrim(cMethod)) == "POST"
        cResp := HTTPSPost(cUrl, "", "", "", "", cBody, 120, aHead, @cHeadRet, .F.)
    Else
        // PATCH / PUT via HTTPQuote (permite definir o mtodo)  ver referncias/threads TDN. :contentReference[oaicite:2]{index=2}
...
/* L arquivo e retorna Base64 (usa a funo disponvel na tua build) */
Static Function FT_FileToBase64(cPath)
    ...
    Begin Sequence
        ...
        Else
            cB64 := "" // nenhuma funo de base64 disponvel
        EndIf
    Recover
        cB64 := ""
    End Sequence
...
/* Remove BOM, NUL e controles 0x00..0x1F (mantm TAB/LF/CR) */
...
/* Fallback textual simples: pega data.id aps a chave "data" */
...
/* Busca padro a partir de um offset (1-based). Retorna 0 se no achar. */
...
/* Gera um filename seguro e com extenso .pdf */
    ...
    For i := 1 To Len(cName)
        ...
        If (cChr >= "A" .and. cChr <= "Z") .or. ;
           ...
           cChr == "." .or. cChr == "_" .or. cChr == "-"
            cOut += cChr
        Else
            cOut += "_" // troca acentos, espaos e smbolos
        EndIf
    Next
...
/* Gera caminho nico se j existir arquivo com mesmo nome */
...
Static Function Grava_env(cEnvId, cDocId, cProj, cTec, cContId, cStatus, dDtEnv)
    ...
    // abre ZP7 no MESMO PADRO do teu ITVSA010
    If Select("ZP7") == 0
        ConOut(LOG_TAG + "ZP7.Grava_env abrindo ZP7 (TOPCONN/RetSqlName)")
        DbUseArea(.T., "TOPCONN", RetSqlName("ZP7"), "ZP7", .T., .F.)
    EndIf
    If Select("ZP7") == 0
        ConOut(LOG_TAG + "ZP7.Grava_env ERRO: alias ZP7 indisponvel"); Return .F.
    EndIf
    ...
    // UPDATE ou INSERT com RecLock/MsUnlock (padro do teu cdigo)
    If lUpd
...
Static Function FT_GetOSCodes(aRegs)
    ...
    // Validao bsica
    If ValType(aRegs) <> "A"
        ConOut(LOG_TAG + "FT_GetOSCodes: aRegs no  array (ValType=" + ValType(aRegs) + ")")
        Return aOut
    EndIf

    // Varredura e deduplicao
...
Static Function FT_SZ1_MarcaEnv(aOSCodes, cEnvId)
    ...
    If Empty(AllTrim(cEnvId)) .or. ValType(aOSCodes) != "A" .or. Len(aOSCodes) == 0
        ConOut(LOG_TAG + "SZ1.MarcaEnv ERRO: parmetros invlidos")
        Return 0
    EndIf

    // abre SZ1 se necessrio (padro TOPCONN do teu projeto)
    If Select("SZ1") == 0
        DbUseArea(.T., "TOPCONN", RetSqlName("SZ1"), "SZ1", .T., .F.)
    EndIf
    If Select("SZ1") == 0
        ConOut(LOG_TAG + "SZ1.MarcaEnv ERRO: alias SZ1 indisponvel")
        Return 0
    EndIf
    ...
    For i := 1 To Len(aOSCodes)
        ...
        If IndexOrd() > 0
            SZ1->(DbSeek(cKey))
            If ! SZ1->(Found())
                ConOut(LOG_TAG + "SZ1.MarcaEnv WARN: OS " + cOS + " no encontrada na ordem 1")
                Loop
            EndIf
        Else
            // sem ndice: varre (volume costuma ser baixo por grupo)
            SZ1->(DbGoTop())
            While ! SZ1->(EoF()) .and. AllTrim(SZ1->Z1_CODIGO) != cOS
                SZ1->(DbSkip())
            EndDo
            If SZ1->(EoF())
                ConOut(LOG_TAG + "SZ1.MarcaEnv WARN: OS " + cOS + " no localizada (varredura)")
                Loop
            EndIf
        EndIf

        If ! Empty(AllTrim(SZ1->Z1_ZP7ENV))
            ConOut(LOG_TAG + "SZ1.MarcaEnv SKIP: OS " + cOS + " j marcada com env=" + AllTrim(SZ1->Z1_ZP7ENV))
            Loop
        EndIf
...
WSMETHOD POST cancelEnvelope WSRECEIVE WSREST OSCLICKENV
    ...
    // 1) Cancelar todos os documentos do envelope (e confirmar)
    oCanc := FT_ClickV3_CancelEnvelope(cEnv)

    // 2) Limpar SZ1 só se a confirmação for ok
    If oCanc["ok"]
    ...
    ConOut(LOG_TAG + "FIM /os/cancel env=" + cEnv + " ok=" + IIf(oCanc["ok"],"T","F") + ;
        " docs_total=" + cValToChar(oCanc["docs_total"]) + ;
        " docs_cancelados=" + cValToChar(oCanc["docs_cancelados"]) + ;
        " os_limpos=" + cValToChar(nLimpos))
Return .T.

/* Cancela (PATCH status=canceled) e CONFIRMA (GET detalhes) os docs do envelope */
Static Function FT_ClickV3_CancelEnvelope(cEnvId)
    ...
    cResp := FT__HttpJSON("GET", cUrl, NIL, @nCode, @cErr)
    ...
    // Se não houver documentos, aborta
    If oRet["docs_total"] <= 0
        oRet["http_log"] := aHttp
        Return oRet
    EndIf

    // 2) PATCH status=canceled em cada doc
    ...
    // 3) Confirmar status "canceled" via GET detalhes de cada doc
    nConf := 0
    For i := 1 To Len(aDocs)
        cDocId := aDocs[i]
        cUrl  := CKS_BASE_V3 + "/envelopes/" + AllTrim(cEnvId) + "/documents/" + AllTrim(cDocId)
        cResp := FT__HttpJSON("GET", cUrl, NIL, @nCode, @cErr)
        AAdd(aHttp, {"doc_detail " + AllTrim(cDocId), nCode, IIf(Empty(cResp), cErr, FT__LogTrunc(cResp))})
        If nCode >= 200 .and. nCode < 300 .and. !Empty(AllTrim(cResp))
            cResp := FT__SanitizeJson(cResp)
            oJson := JsonObject():New()
            If oJson:FromJson(cResp) != NIL
                If ValType(oJson["data"]) == "O" .and. ValType(oJson["data"]["attributes"]) == "O"
                    Local cStat := Upper(AllTrim(cValToChar(oJson["data"]["attributes"]["status"])))
                    If cStat == "CANCELED" .or. cStat == "CANCELLED"
                        nConf++
                    EndIf
                EndIf
            EndIf
    ...
    oRet["ok"]       := (oRet["docs_total"] > 0 .and. nConf == oRet["docs_total"])
    oRet["http_log"] := aHttp
Return oRet

/* Limpa SZ1.Z1_ZP7ENV = "" (somente registros que possuem o envelope informado) */
Static Function FT_SZ1_ClearEnvById(cEnvId)
    ...
    // Consulta RECNO com o envelope informado
    ...
    ConOut(LOG_TAG + "SZ1.ClearEnv OK env=" + AllTrim(cEnvId) + " limpos=" + cValToChar(nLimpos))
Return nLimpos
