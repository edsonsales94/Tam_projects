...
ฑฑบPrograma  ณINCOMCUS  บ Autor ณ ENER FREDES        บ Data ณ  15/04/11   บฑฑ
...
User Function INCOMCUS(__aCookies,__aPostParms,__nProcID,__aProcParms,__cHTTPPage)
  Local cHTML
  Local cServerHttp
  Private aIdioma := {}                   
  ...
  cHTML += '  oFrm.action = "http://'+cServerHttp+'u_INCOMCUS.apl?&Campo="+cCampo+"&Pesquisa="+cPesquisa+"&P1='+cFil+'";
  cHTML += '  oFrm.submit();
  cHTML += '}                            
  ...
  cHTML += '  window.open("http://'+cServerHttp+'u_incomw01.apl?&cIdioma=P&lFrame =.f.&filial='+cFil+'&tipo=3&num="+cCusto+"&dini=&dfim=", "JanelaPrincipal" , "width = 1000,height = 1000 , top = 0,left = 0 ,Menubar = yes Location = yes,Directories = yes,status = yes,scrollbars=yes"); 
  cHTML += '  window.close(); 
  cHTML += '}                            
  cHTML += '</SCRIPT>
