...
User Function INFINR15(_cProj,_cTitulo,_cProg,_cPerg,_vPerg,_lCallMe,_cMCT)
	...
	If !_lCallMe
		...
		Private cCsvArq  := ""
		Private nHdl     := 0
		Private cEOL     := CHR(13)+CHR(10)    //retorno de linha
...
Static function RunReport(Cabec1,Cabec2,Titulo,nLin,_lCallMe,_cTitulo)
	...
	If !(Eof() .And. Bof())
		...
		If !_lCallMe
			fWrite(nHdl, "FILIAL"+Chr(9)+"DATA"+Chr(9)+"MCT"+Chr(9)+"CENTRO_CUSTO"+Chr(9)+"HISTORICO"+Chr(9)+"ENTRADA"+Chr(9)+"SAIDA"+Chr(9)+"JUSTIFICATIVA" + cEOL)
		Endif
	Endif
