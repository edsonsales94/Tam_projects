...
Local wnrel
Local aOrd:={OemToAnsi(STR0001),OemToAnsi(STR0002),OemToAnsi(STR0003),OemToAnsi(STR0004),OemToAnsi(STR0032),OemToAnsi(STR0005),OemToAnsi(STR0036),STR0048,"Centro de Custo","Bordero","Area"}  //"Por Data"###"Por Banco"###"Por Natureza"###"Alfabetica"###"Nro. Titulo"###"Dt.Digitacao"###"Por Lote" //"Por Data de Credito"
...
Private nomeprog:="FINR190B"
Private aLinha  := { },nLastKey := 0
Private cPerg   := PADR("RSFIN190",Len(SX1->X1_GRUPO))
...
Local cExp := ""
Local CbTxt,CbCont
Local nValor:=0,nDesc:=0,nJuros:=0,nCM:=0,nMulta:=0,nVlMovFin:=0
Local nTotValor:=0,nTotDesc:=0,nTotJuros:=0,nTotMulta:=0,nTotCm:=0,nTotOrig:=0,nTotBaixado:=0,nTotMovFin:=0,nTotComp:=0,nTotPA:=0
Local nGerValor:=0,nGerDesc:=0,nGerJuros:=0,nGerMulta:=0,nGerCm:=0,nGerOrig:=0,nGerBaixado:=0,nGerMovFin:=0,nGerComp:=0,nGerPA:=0
Local nFilOrig:=0,nFilJuros:=0,nFilMulta:=0,nFilCM:=0,nFilDesc:=0
Local nFilAbat:=0,nFilValor:=0,nFilBaixado:=0,nFilMovFin:=0,nFilComp:=0,nFilPA:=0
...
Case nOrdem == 9	//Ordem 9 Por Centro de Custo
	cCondicao := ".T."
	cCond2 := "CCUSTO"
	cChave := "E5_FILIAL+CCUSTO+DTOS(E5_DTDIGIT)+E5_PREFIXO+E5_NUMERO+E5_PARCELA+DTOS(E5_DATA)"
	titulo += "Por centro de Custo"
	bFirst := {||MsSeek(xFilial("SE5"),.T.)}
Case nOrdem == 10	//Ordem 10 Por Bordero
	cCondicao := ".T."
	cCond2 := "INVBOR"
	cChave := "E5_FILIAL+INVBOR+DTOS(E5_DTDIGIT)+E5_PREFIXO+E5_NUMERO+E5_PARCELA+DTOS(E5_DATA)"
	titulo += "Por Bordero"
	bFirst := {||MsSeek(xFilial("SE5"),.T.)}
Case nOrdem == 11	//Ordem 10 Por Stream
	cCondicao := ".T."
	cCond2 := "STREAM"
	cChave := "E5_FILIAL+STREAM+DTOS(E5_DTDIGIT)+E5_PREFIXO+E5_NUMERO+E5_PARCELA+DTOS(E5_DATA)"
	titulo += "Por Area"
	bFirst := {||MsSeek(xFilial("SE5"),.T.)}
...
#IFDEF TOP
	If TcSrvType() != "AS/400"
        ...
        If MV_PAR11 == 1
		  ...
		  cQuery +=         ",SE5.R_E_C_N_O_ SE5RECNO ,E1_CC CCUSTO,LEFT(E1_CC,2) STREAM,E1_INVOICE INVBOR"
  		Else
  		  cQuery := "SELECT " +SubStr(cQuery,2)
		  cQuery +=         ",SE5.R_E_C_N_O_ SE5RECNO ,E2_CC CCUSTO,LEFT(E2_CC,2) STREAM,E2_NUMBOR INVBOR"
  		EndIf  
		cQuery += "FROM " + RetSqlName("SE5")+" SE5 "
        If MV_PAR11 == 1
  		  cQuery += "INNER JOIN " + RetSqlName("SE1")+" SE1 ON E1_FILIAL = E5_FILIAL AND E1_PREFIXO = E5_PREFIXO AND E1_NUM = E5_NUMERO AND E1_CLIENTE = E5_CLIFOR AND E1_LOJA = E5_LOJA AND E1_PARCELA = E5_PARCELA AND E1_TIPO = E5_TIPO AND SE1.D_E_L_E_T_ <> '*'"
  		Else
  		  cQuery += "INNER JOIN " + RetSqlName("SE2")+" SE2 ON E2_FILIAL = E5_FILIAL AND E2_PREFIXO = E5_PREFIXO AND E2_NUM = E5_NUMERO AND E2_FORNECE = E5_CLIFOR AND E2_LOJA = E5_LOJA AND E2_PARCELA = E5_PARCELA AND E2_TIPO = E5_TIPO AND SE2.D_E_L_E_T_ <> '*'"
  		EndIf  
...
While !Eof() .and. M0_CODIGO == cEmpAnt .and. M0_CODFIL <= cFilAte
	...
	If nOrdem == 3 .and. ((MV_MULNATR .and. mv_par11 = 1) .or. (MV_MULNATP .and. mv_par11 = 2))
		Finr199(	@nGerOrig,@nGerValor,@nGerDesc,@nGerJuros,@nGerCM,@nGerMulta,@nGerAbat,@nGerBaixado,@nGerMovFin,@nGerComp,;
					@nFilOrig,@nFilValor,@nFilDesc,@nFilJuros,@nFilCM,@nFilMulta,@nFilAbat,@nFilBaixado,@nFilMovFin,@nFilComp,;
	...
	Else
		...
		While NEWSE5->(!Eof()) .And. NEWSE5->E5_FILIAL==xFilial("SE5") .And. &cCondicao .and. lContinua
			...
			While NEWSE5->(!EOF()) .and. &cCond2=cAnterior .and. NEWSE5->E5_FILIAL=xFilial("SE5") .and. lContinua
				...
				c_Filial    := If(Empty(cFilorig),NEWSE5->E5_FILIAL,cFilorig)
				...
				If (NEWSE5->E5_RECPAG == "R" .and. ! (NEWSE5->E5_TIPO $ "PA /"+MV_CPNEG )) .or. ;	//Titulo normal
					...
					Endif				
					If lBxTit
					  If SE1->E1_CC < MV_PAR40 .Or. SE1->E1_CC > MV_PAR41
 					    dbSelectArea("NEWSE5")
						NEWSE5->(dbSkip())		      // filtro de registros desnecessarios
						Loop
					  EndIf
				...
				Else
					...
					Endif				
					If lBxTit
					  If SE2->E2_CC < MV_PAR40 .Or. SE2->E2_CC > MV_PAR41
 					    dbSelectArea("NEWSE5")
						NEWSE5->(dbSkip())		      // filtro de registros desnecessarios
						Loop
					  EndIf
					  If SE2->E2_NUMBOR < MV_PAR38 .Or. SE2->E2_NUMBOR > MV_PAR39
 					    dbSelectArea("NEWSE5")
						NEWSE5->(dbSkip())		      // filtro de registros desnecessarios
						Loop
					  EndIf
					EndIf 
					cContr := SE2->E2_NUMCONT
					cCusto := SE2->E2_CC
					cBord_Inv := SE2->E2_NUMBOR
...
//						@li, 71 PSAY SubStr( cHistorico ,1,18)
						@li, 94 PSAY dBaixa
						IF nVlr > 0
							@li,105 PSAY nVlr  Picture tm(nVlr,14,nDecs)
						Endif
					Else
						If mv_par13 == 1  // Utilizar o Hist¢rico da Baixa ou Emiss„o
...
///							@li, 71 PSAY SubStr(ALLTRIM(cCheque)+"/"+Trim(cHistorico),1,18)
						    @li, 71 PSAY ALLTRIM(cCheque)+"/"+cCusto
						Else
					...
					Endif
					nCT++
					@li,120 PSAY nJuros     PicTure tm(nJuros,12,nDecs)
					@li,133 PSAY nMulta     PicTure tm(nMulta,10,nDecs)
					@li,144 PSAY nCM        PicTure tm(nCM ,10,nDecs)
...
//					@li,202 PSAY dDigit
					...
					@li,215 PSAY If(Empty(cFilorig),c_Filial,cFilorig)
					
					nTotOrig   += Iif(lOriginal,nVlr,0)     
					
					If cTipo == "PA " .and. cTipodoc == "PA"
 					   nTotPA += nValor
 					Else   
			...
			If (nTotValor+nDesc+nJuros+nCM+nTotMulta+nTotOrig+nTotMovFin+nTotComp+nTotPA)>0
				li++
				IF li > 55
					cabec(titulo,cabec1,cabec2,nomeprog,tamanho,15)
				Endif
				If nCT > 0
					...
					Elseif nOrdem == 9
						dbSelectArea("CTT")
						DbSetOrder(1)
						MsSeek(xFilial("CTT")+cAnterior)
						@li, 0 PSAY "SubTotal : " + cAnterior + " "+CTT_DESC01
					Elseif nOrdem == 10
						@li, 0 PSAY "SubTotal Bordero/Invoice: " + cAnterior 
					Elseif nOrdem == 11
						@li, 0 PSAY "SubTotal Area: " + cAnterior  + ' - '+Posicione("PB1",1,xFilial("PB1")+cAnterior ,"PB1_DESC")
					Endif
					If nOrdem != 5
						@li,105 PSAY nTotOrig     PicTure tm(nTotOrig,14,nDecs)
						@li,120 PSAY nTotJuros    PicTure tm(nTotJuros,12,nDecs)
						@li,133 PSAY nTotMulta    PicTure tm(nTotMulta,10,nDecs)
						@li,144 PSAY nTotCM       PicTure tm(nTotCM ,10,nDecs)
						...
						If nTotPA > 0
							li++
							@li,197 PSAY "Adiantam"
							@li,206 PSAY nTotPA      PicTure tm(nTotPA,14,nDecs)
						Endif
						li+=2
					Endif
					dbSelectArea("NEWSE5")
	...
	If mv_par17 == 1 .and. SM0->(Reccount()) > 1 .And. li != 80
		...
		@li,105 PSAY nFilOrig       PicTure tm(nFilOrig,14,nDecs)
		@li,120 PSAY nFilJuros      PicTure tm(nFilJuros,12,nDecs)
		@li,133 PSAY nFilMulta      PicTure tm(nFilMulta,10,nDecs)
		@li,144 PSAY nFilCM         PicTure tm(nFilCM ,10,nDecs)
		...
		If nFilPA > 0
			li++
			@li,197 PSAY "Adiantam"
			@li,206 PSAY nFilPA      PicTure tm(nFilPA,14,nDecs)
		Endif
		li+=2
		If Empty(xFilial("SE5"))
			Exit
...
If li != 80
	...
	@li,105 PSAY nGerOrig       PicTure tm(nGerOrig,14,nDecs)
	@li,120 PSAY nGerJuros      PicTure tm(nGerJuros,12,nDecs)
	@li,133 PSAY nGerMulta      PicTure tm(nGerMulta,10,nDecs)
	@li,144 PSAY nGerCM         PicTure tm(nGerCM ,10,nDecs)
	...
	If nGerPA > 0
		li++
		@li,197 PSAY "Adiantam"
		@li,206 PSAY nGerPA PicTure tm(nGerPA,14,nDecs)
	Endif
	If nGerBaixado+nGerMovFin+nGerPA > 0 
        li++                                       
        @li,206 PSAY "-----------------"
        li++
		@li,197 PSAY "Geral   "
		@li,206 PSAY (nGerBaixado+nGerMovFin+nGerPA)  PicTure tm(nGerBaixado,14,nDecs)
	Endif
	li++
	roda(cbcont,cbtxt,"G")
Endif
...
Local aArea := GetArea()
Local cPerg		:= PADR("RSFIN190",Len(SX1->X1_GRUPO))
