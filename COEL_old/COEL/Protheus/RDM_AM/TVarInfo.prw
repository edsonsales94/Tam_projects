...
/*/
	Class:		TVarInfo 
	Autor:		Marinaldo de Jesus ( Baseado na Ideia Original de Julio Wittwer )
	Data:		28/09/2011
	Descricao:	Reproduz as Caracteristicas da Funcao VarInfo resolvendo a Limitacao do Tamanho da String
	Sintaxe:	TVarInfo():New( uVarInfo , cVarName )
/*/
Class TVarInfo From LongClassName
	...
	Method New( uVarInfo , cVarName )
	Method Init( uVarInfo , cVarName )
...
/*/
User Function TVarInfoNew( uVarInfo , cVarName )
Return( TVarInfo():New( uVarInfo , cVarName ) )
...
/*/
	...
	Sintaxe:	TVarInfo():New( uVarInfo , cVarName )
/*/
Method New( uVarInfo , cVarName ) Class TVarInfo
...
/*/
Method Init( uVarInfo , cVarName ) Class TVarInfo
	...
	TVarInfo( @Self:aTVarInfo , @aStack , @cVarName , @uVarInfo )
...
Method Show() Class TVarInfo
	...
	IF (;
		...
		IF ( lShow )              
			ShellExecute( "Open", Self:cLocalFile , "" , Self:cLocalPath , SW_SHOWMAXIMIZED )
		EndIF
...
/*/
Static Function TVarInfo( aTVarInfo , aStack , cVarName , uVarInfo , nNivel )
	...
	IF ( cVarType == "O" )
		...
		TVarInfo( @aTVarInfo , @aStack , cVarName , ClassDataArray( uVarInfo ) , nNivel )
	...
	ElseIF ( cVarType == "A" )
		...
		nStack	:= aScanX( aStack , { |aStk,nAT| aStack[ nAT ][ STACK_PARAMETER ] == uVarInfo } )
		...
		IF ( nStack > 0 )
			...
			cLine	+= ") "
			cLine	+= "[CLONE OF "
			cLine	+= aStack[ nStack ][ STACK_NAME ]
			cLine	+= "]"

		Else

			aAdd( aStack , Array( STACK_ELEMENTS ) )

			nStack								:= Len( aStack )
			aStack[ nStack ][ STACK_NAME      ]	:= cVarName
			aStack[ nStack ][ STACK_PARAMETER ]	:= uVarInfo
		...
		For nBL := 1 To nEL
			...
			TVarInfo( @aTVarInfo , @aStack , cVarName+"["+cNStr+"]" , uVarInfo[ nBL ] , nNivel )
		Next nBL
