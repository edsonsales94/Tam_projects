...
Do While !Eof() .AND. xFilial("SF2")==SF2->F2_FILIAL .AND. SF2->F2_DOC <= mv_par02 .AND. lContinua
	...
	If ( xTIPO $"N_C_P_I_S_T_O_" )
		...
		xREGTRIB := SA1->A1_REGTRIB         // Tipo de tributacao do cliente Manaus
...
If zFranca == .T.
	...
	xZFR_TOTAL := 0
	@ nLin,  23 PSAY "DESCONTO DE " + Str( xICMS[1], 5, 2 ) + "%  REFERENTE AO ICMS + 3.65% DE PIS/COFINS"
...
@ nLin, 87  PSAY Chr(18) + xNUM_NF+Chr(15)                 // Numero da Nota Fiscal
@ nLin, 185  PSAY Chr(18) + xNUM_NF+Chr(15)                 // Numero da Nota Fiscal
nLin := 72
@ nLin, 000 PSAY " "
SetPrc(0,0)
...
If SM0->M0_CODIGO <> "03" //alterado por Andre
	If !Empty( xREC_CGC )
...
If !xTESPROD[1] $ "704/708"
	If SM0->M0_CODIGO <> "03"
		...
		//  AADD( xLINHASAD, "")
               // AADD( xLINHASAD, "")
              // AADD( xLINHASAD, "FERIAS COLETIVAS DO DIA 24/12/2007 A 06/01/2008")
              AADD( xLINHASAD, "Novo endereco de entrega e retirada: R.Casa do ator, 685 ")
              AADD( xLINHASAD, "Vila Olimpia Cep: 04546-002 ")
	EndIf
	...
	If SM0->M0_CODIGO == "03"
		If !Empty(xTRANS_AM)
			If !Empty(xCOD_TRAN)
				...
				AADD( xLINHASAD, ALLTRIM(xEND_TRANSP)+" - "+ALLTRIM(xMUN_TRANSP)+" - "+xEST_TRANSP)
				AADD( xLINHASAD, "Cep :"+xCEP_TRANSP+" - Tel :"+xTEL_TRANSP)
				AADD( xLINHASAD, "*** O frete de redespacho sera cobrado do cliente ***")
			EndIf
		EndIf
		AADD( xLINHASAD, "Empresa Habilitada para o Procedimento Simplificado")
		AADD( xLINHASAD, "de Internacao nos termos do ART.13 da INSRF  ")
		AADD( xLINHASAD, "242/02- ADE/ALFPTOMNS N: 24 DE 28/06/2004 DOU DE 29/06/04")
		AADD( xLINHASAD, "Isento de IPI")
		AADD( xLINHASAD, "Produzido na Zona Franca de Manaus")
		AADD( xLINHASAD, "DEC. 4.544/02 ART. 69 INC. II RESOLUCAO")
		AADD( xLINHASAD, "SUFRAMA N:284, DE 09/09/2002")
		//AADD( xLINHASAD, "ARE N: 48/2003")
		//AADD( xLINHASAD, "AIDF : 1866/2003 DE 24/07/2003")
		//AADD( xLINHASAD, "INSCRICAO MUNIC. DO REPRESENTANTE DA GRAFICA EM NANAUS ")
		//AADD( xLINHASAD, "CARLOS JUNIO C.CAVALCANTE INSC. MUNICIPAL: 55117-0 ")
		
		If xREGTRIB =="1"
			xMENTRIB = "OPTANTE DO SIMPLES"
		Endif
		If xREGTRIB =="2"
			xMENTRIB ="LUCRO PRESUMIDO"
		Endif
		If xREGTRIB =="3"
			xMENTRIB ="LUCRO REAL CUMULATIVO"
		Endif
		If xREGTRIB =="4"
			xMENTRIB ="LUCRO REAL NAO-CUMULATIVO"
		Endif
		If xREGTRIB =="5"
			xMENTRIB ="ZONA FRANCA DE MANAUS"
		Endif
		
		If !Empty(xREGTRIB)
			AADD( xLINHASAD, "Regime de Tributacao: "+xMENTRIB)
		Endif
