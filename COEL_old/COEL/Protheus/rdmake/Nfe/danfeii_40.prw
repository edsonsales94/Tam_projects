...
_cEntSai   := ""  //COEL: - se a nota é de entrada ou saida 
_limporta  := .F. //COEL: - se a nota é de importação
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						_cEntSai := "E" /////COEL:
							...
							If MsSeek(xFilial("SF1")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. SF1->(FieldPos("F1_FIMP")) <> 0 .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								...
								EndIf
									///
									///COEL: ESPECIFICO DE MENSAGEM SOMENTE DA COEL e COELMATIC/JABA 
									///
									If SM0->M0_CODIGO <> "03".OR.SM0->M0_CODIGO+SM0->M0_CODFIL=="0302"
										If !EMPTY(SF1->F1_DI)     
											_limporta := .t.
										Endif                    
									EndIf
...
// especifico COEL ----------------------------------------------------------------------------------
//TRATAMENTO PARA IMPRESSAO DE TXT -ESPECIFICO COEL
 //_NFEde := MV_PAR01
 //_NFEat := MV_PAR02
 //_NFEsr := MV_PAR03

IF MV_PAR04 = 2 //SE NOTA DE SAIDA
  // If MsgYesNo("ENVIA TXT DE CODIGOS DE BARRAS FRICON? ")
       U_QIMPDtxt(MV_PAR01,MV_PAR02,MV_PAR03)  //EXECUTA FUNCAO DE ENVIO DE EMAIL UTILIZANDO PARAMETROS DA DANFEII
  // ENDIF
ENDIF 
Return .T.
...
///COEL: 22/06/12
///-----------------------------------------------
IF ( TYPE("oEmitente:_EnderEmit:_Fone:Text") == "U" )
	cFoneEmit := SM0->M0_TEL
ELSE	
	cFoneEmit := oEmitente:_EnderEmit:_Fone:Text    
ENDIF
cUfedDest := oDestino:_EnderDest:_UF:Text        
IF ( cUfedDest=="EX" )
	IF ( LEFT(cFoneEmit,2) == "55" )
		cFoneEmit := TRANSF(cFoneEmit,"@R 99-99-9999-9999")
	ELSE
		cFoneEmit := "55-"+TRANSF(cFoneEmit,"@R 99-9999-9999")
	ENDIF
ELSE
	IF ( LEFT(cFoneEmit,2) == "55" )
		cFoneEmit := TRANSF(SUBS(cFoneEmit,3,10),"@R (99)9999-9999")
	ELSE
		cFoneEmit := TRANSF(cFoneEmit,"@R (99)9999-9999")
	ENDIF
ENDIF
...
If lMv_ItDesc
	For nX := 1 To nLenDet
        ...
        Aadd(aIndAux, {nX, SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),1,nMaxDes)})      ///Coel: Carlos: 19/11/11
	Next
...
For nZ := 1 To nLenDet
	...
	If Type("oImposto:_Imposto")<>"U"
		...
		EndIf
	EndIf
	///COEL: nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 50)
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
	   	...
	   	///COEL: nMaxCod := MaxCod(cAuxItem, 50)
...
///COEL: JABAQUARA
///
if _cEntSai == "E" .and. _lImporta  
   aadd(aMensagem,"Local embarque: " + alltrim(SF1->F1_ALFANDE))
   aadd(aMensagem,"DI " + alltrim(SF1->F1_DI))
   aadd(aMensagem,"DATA: " + alltrim(dtoc(sf1->f1_datadi)))

   aadd(aMensagem,"PIS R$ " + alltrim(Transform(sf1->f1_valimp6,"@ze 999,999,999.99")))
   aadd(aMensagem,"COFINS R$ " + alltrim(transform(sf1->f1_valimp5,"@ze 999,999,999.99"))) 
   aadd(aMensagem,"BASE PIS/COFINS R$ " + alltrim(transform(sf1->f1_basimp5,"@ze 999,999,999.99")))   
   if SF1->F1_VALIPI > 0
      aadd(aMensagem,"IPI R$ " + alltrim(transform(sf1->f1_valipi,"@ze 999,999,999.99")))           
   endif   
   aadd(aMensagem,"TAXA SICOMEX R$ " + alltrim(transform(sf1->f1_taxaimp,"@ze 999,999,999.99")))
      
   aadd(aMensagem,"Imposto de Importação (II): (" + alltrim(str(SD1->D1_ALIQII)) + "%) R$ " + alltrim(transform(sf1->f1_ii,"@ze 999,999,999.99")))
endif  
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	nLinCalc+=10
		...
		///COEL: oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)

	oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",cFoneEmit),oFont08N:oFont)    ///COEL: 22/06/12
...
///COEL: oDanfe:Say(252,503,aHrEnt[01],oFont08:oFont)
oDanfe:Say(252,503,"",oFont08:oFont)                ///COEL:
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro fatura                                                           ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
aAux := {{{},{},{},{},{},{},{},{},{}}}
