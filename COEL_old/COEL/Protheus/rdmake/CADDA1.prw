...
±±ºFuncao    ³ ACERTASB1      º Autor ³ Andre Rodrigues                       º Data ³ SET/2013 º±±
...
±±ºUso       ³ Especifico para clientes TOTVS - DAY BRASIL                                      º±±
...
@ 002,003 TO 045,187
@ 010,005 Say AnsiToOem('Este programa atualizará a tabela de preco dos produtos com base no arquivo.') OF oDlg PIXEL Size 185,020
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local aSavArea 	:= SaveArea1({Alias(),'DA1'})
...
For v := 1 to Len(aListaDbf)
	...
	If Empty(ARC->OK)                    
		...
		For x := 1 To Len(aDados)
          ...
          //If Alltrim(aDados[x][01]) == "01"
				//_cDA1_FIL		:= Alltrim(aDados[x][01])
			   //	_cDA1_IT		:= Alltrim(aDados[x][03])
				_cDA1_TAB		:= Strzero(VAL(aDados[x][01]),3)
				_cDA1_COD		:= Alltrim(aDados[x][02])
				//_cDA1_DESC		:= Alltrim(aDados[x][04])
				_cDA1_PREC		:= NOROUND(Val(StrTran(AllTrim(aDados[x][03]),',','.')),4)
				//_cDA1_DATV		:= DDATABASE
				
				          		   
				IncProc('Atualizando registro: ' + Alltrim(_cDA1_TAB)+" "+Alltrim(aDados[x][02]))
	
				dbSelectArea('DA1')
				dbSetOrder(1)//SB1_FILIAL+SB1_COD
				If dbSeek(xFilial('DA1')+_cDA1_TAB+_cDA1_COD)
				
					RecLock('DA1',.F.)
						
					//	DA1->DA1_FILIAL := _cDA1_FIL
					//	DA1->DA1_ITEM	:= _cDA1_IT
					//	DA1->DA1_CODTAB	:= _cDA1_TAB
					//	DA1->DA1_CODPRO	:= _cDA1_COD	
					//	DA1->DA1_PRCVEN	:= NoRound(_cDA1_PREC,4)
						DA1->DA1_PRCVEN	:= _cDA1_PREC
					//	DA1->DA1_DATVIG	:= _cDA1_DATV
					//	DA1->DA1_ATIVO	:="1"
					//	DA1->DA1_TPOPER	:="4"
					...
					//	DA1->DA1_MOEDA	:= 1
					DA1->(MsUnlock())
	                //alert("gravado item: "+ _cDA1_ITEM+ " tabela: "+_cDA1_CODTAB+ "Produto: "+ _cDA1_CODPRO)
				EndIf
			//Endif
		Next x
