...
±±³Descricao ³ Grava um arquivo de log de PV Emilinados Residuos          ³±±
...
cQuery := "SELECT C6_FILIAL, C6_NUM, C6_ITEM, C6_PRODUTO, C6_QTDVEN, C6_TES, C6_CF, C6_QTDVEN, C6_PRCVEN, C6_VALOR, C6_X_EMIS, C6_ENTREG, C6_DESCRI, C6_CLI, C6_LOJA, C6_QTDENT, C6_DATFAT, C6_BLQ, C6_X_DT2 "
cQuery := cQuery + "FROM SC6" + SM0->M0_CODIGO + "0 "
cQuery := cQuery + "WHERE C6_FILIAL = '"+xFilial("SC6")+"' AND "
cQuery := cQuery + "C6_BLQ = 'R' AND C6_NUM = '"+_NumPed+"' "
...
If nReg > 0
	...
	While TMP->C6_NUM == _NumPed //!EOF()
		...
		If !dbSeek(xFilial("SZL")+TMP->C6_NUM+TMP->C6_ITEM+"  "+TMP->C6_PRODUTO)  
			...
			ZL_TES    := TMP->C6_TES
			ZL_CFOP   := TMP->C6_CF
//			ZL_EMISSAO:= stod(TMP->C6_X_EMIS)          VERIFICAR QUANTIDADE A SER GRAVADO ( VENDIDA - ENTREGUE ) E TOTAL 
			ZL_DTDEL  := DDATABASE
