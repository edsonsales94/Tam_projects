...
*/
dDataref      := DDATABASE  
cDirDocs      := "\workflow\ENVIOP"       //MsDocPath() //D:\Totvs12\workflow\ENVIOP\
cArquivo      := StrZero(Year(DDATABASE),4)+StrZero(Month(DDATABASE),2)+StrZero(Day(DDATABASE),2)//ALLTRIM(STR(DAY(dDataref)))+ ALLTRIM(STR(Month(dDataref)	))+ALLTRIM(STR(YEAR(dDataref)))     //CriaTrab(,.F.) 
nHandle
cCrLf         := Chr(13) + Chr(10)
nX
cSql          := ""
...
U_QENVPOP((cDirDocs+"\"+"OP"+cArquivo+".CSV"),'rogerio@oliminet.com.br')
...
//envio de mail ----------------------------------------------------------------------------------------------------------
USER Function QENVPOP(_ARQUIVO,EMAIL)  //ENVIA O ARQUIVO GERADO NO DISCO POR EMAIL
local _ARQUIVO
LOCAL EMAIL
local atacha           
