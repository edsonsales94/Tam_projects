...
User Function WFWFINA()
	Processa({|| PROCWACO()},"Aguarde Processando...") 
Return

Static Function PROCWACO()
Local  c_Alias := ALIAS()
Local  aItens  := {}
Local  xOrdProd :=""
...
cSELECT := 	'SA1.A1_NOME,SE1.E1_PREFIXO,SE1.E1_NUM,SE1.E1_VENCREA,SE1.E1_PARCELA,SE1.E1_VALOR,SE1.E1_EMISSAO,SA1.A1_EMAILFI,SA1.A1_NOME '

cFROM  :=	RetSqlName('SE1') + ' SE1'

cINNER := 'INNER JOIN '+ RetSqlName('SA1')+' SA1 ON SA1.A1_COD = SE1.E1_CLIENTE AND SA1.A1_LOJA = SE1.E1_LOJA  ' 
		  
cWHERE :=	'E1_VENCREA = (SELECT CONVERT(char(8), dateadd(day,2,getdate()), 112)) AND '+;
     	    'E1_TIPO ='+ "'NF'"+ ' AND E1_PREFIXO <>' +'5  '+' AND SE1.E1_VENCREA >='+ " '20161020'"+ ' AND '+;
...
While 	TCPA->( ! Eof() )
AADD(aItens , {TCPA->E1_NUM,TCPA->E1_PREFIXO,TCPA->A1_NOME,TCPA->E1_PARCELA,TCPA->E1_VALOR,TCPA->E1_EMISSAO,TCPA->E1_VENCREA,TCPA->A1_EMAILFI})
TCPA->(Dbskip())
...
cxCC   := "Financeiro@coel.com.br;kadrekes@gmail.com"
...
For ix:=1 To Len(aItens)
	IncProc("Processando Titulo Nr. "+aItens[ix][1])
	If !Empty(aItens[ix][8])
		...
		cmBody += "<img width= "+'"090px"'+" height= "+'"075px"'+" align="+'"Top "'+" src= "+ '"'+"www.coel.com.br/wp-content/themes/coel/assets/img/logo.png"+ '"'+"/>" +chr(13)+chr(10)
	 	cmBody += "Prezado Cliente, "+"<b>"+aItens[ix][3]+"</b>"+chr(13)+chr(10)
		cmBody += "O boleto de cobrança referente ao titulo abaixo foi enviado para seu endereço "+chr(13)+chr(10)
		cmBody += "(ou pelo DDA, caso o tenha cadastrado) "+chr(13)+chr(10)
		...
		cmBody += "Caso tenha alguma duvida, por favor, entre em contato com nosso departamento financeiro. "+chr(13)+chr(10)
