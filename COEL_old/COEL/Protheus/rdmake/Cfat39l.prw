...
User Function Cfat39l()        // incluido pelo assistente de conversao do AP6 IDE em 02/09/02
...
±±³Programa  ³ CFAT39I  ³ Autor ³ADILSON TAKESHIMA      ³ Data ³16/10/2001³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Exporta Dados de PV para Estatistica                       ³±±
...
Aadd(Estrutura,{ "MESANO"       ,"C",06,0 } )
Aadd(Estrutura,{ "ANOMES"       ,"C",06,0 } )
Aadd(Estrutura,{ "DTFATUR"      ,"D",08,0 } )
Aadd(Estrutura,{ "DTENTRE"      ,"D",08,0 } )
...
Aadd(Estrutura,{ "CODNICHO"     ,"C",05,0 } )
Aadd(Estrutura,{ "NICHO"        ,"C",20,0 } )
Aadd(Estrutura,{ "CODVEN"       ,"C",06,0 } )
Aadd(Estrutura,{ "VENDEDOR"     ,"C",40,0 } )
Aadd(Estrutura,{ "CODREG"       ,"C",03,0 } )
Aadd(Estrutura,{ "REGIAO"       ,"C",20,0 } )
...
Processa({||RunProc()},"Exp Dados p/Estatistica")// Substituido pelo assistente de conversao do AP6 IDE em 02/09/02 ==> Processa({||Execute(RunProc)},"Exp Dados p/Estatistica")
Return
...
While Eof() == .f. .And. Dtos(SC5->C5_EMISSAO) <= Dtos(MV_PAR02)
    ...
    While SC6->C6_NUM == SC5->C5_NUM
        ...
        RecLock("TRB",.t.)
          ...
          ENDIF
          TRB->DTENTRE  := SC6->C6_ENTREG
          TRB->MESANO   := Subs(Dtos(SC5->C5_EMISSAO),1,4)+Subs(Dtos(SC5->C5_EMISSAO),5,2) //STRING PARA EMISSAO
          TRB->ANOMES   := Subs(Dtos(SC6->C6_ENTREG),1,4)+Subs(Dtos(SC6->C6_ENTREG),5,2) //STRING PARA ENTREGA
          ...
          TRB->PRCLIS   := Posicione("SB1",1,xFilial("SB1")+SC6->C6_PRODUTO,"B1_PRV1")
          TRB->VLRDESC  := 100 - ( (TRB->UNIT / TRB->PRCLIS) * 100 )
          TRB->CODSUB   := Posicione("SB1",1,xFilial("SB1")+SC6->C6_PRODUTO,"B1_GRUPO")
          TRB->SUBGRUPO := Subs(Tabela("03",TRB->CODSUB,.f.),1,13)
          ...
          TRB->CODNICHO := Posicione("SA1",1,xFilial("SA1")+TRB->CODCLI,"A1_ATIVIDA")
          TRB->NICHO    := Tabela("99",Posicione("SA1",1,xFilial("SA1")+TRB->CODCLI,"A1_ATIVIDA"),.f.)
          IF  !EMPTY(SC5->C5_VEND1)
            ...
            ELSE
               ...
               TRB->VENDEDOR := Posicione("SA3",1,xFilial("SA3")+TRB->CODVEN,"A3_NOME")
          ENDIF
          TRB->CODREG   := Posicione("SA1",1,xFilial("SA1")+TRB->CODCLI,"A1_REGIAO")
          TRB->REGIAO   := Tabela("98",Posicione("SA1",1,xFilial("SA1")+TRB->CODCLI,"A1_REGIAO"),.f.)
        MsUnLock()
