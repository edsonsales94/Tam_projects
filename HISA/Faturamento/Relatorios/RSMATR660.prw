...
±±³Autor   o ³ Orismar Silva                            ³ Data ³ 04/02/14 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Incluir o Periodo no relatorio                             ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±

±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
User Function RSMATR660()
...
oReport:= TReport():New("MATR660",STR0039,"MTR660", {|oReport| ReportPrint(oReport)},STR0040 + " " + STR0041 + " " + STR0042)	// "Resumo de Vendas"###"Emissao do Relatorio de Resumo de Vendas, podendo o mesmo"###"ser emitido por ordem de Tipo de Entrada/Saida, Grupo, Tipo"###"de Material ou Conta Contábil."
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oReport:SetTitle(oReport:Title() + " - " + IIf(oReport:Section(1):GetOrder() == 1,STR0043,IIF(oReport:Section(1):GetOrder()==2,STR0044,IIF(oReport:Section(1):GetOrder()==3,STR0045,IIF(oReport:Section(1):GetOrder()==4,STR0046,IIF(oReport:Section(1):GetOrder()==5,STR0047,STR0048))))) + " - " + GetMv("MV_MOEDA"+STR(mv_par08,1)) +" Periodo: "+dtoc(mv_par01) +"  a  "+dtoc(mv_par02))	// "Resumo de Vendas"###"Por Tp/Saida + Produto"###"Por Tipo"###"Por Grupo"###"P/Ct.Contab."###"Por Produto"###"Por Tp/Salida + Serie + Nota"
...
#IFDEF TOP
	...
	cCposSD2 := "%"
	For nY:=0 To 9
		If !Empty( FWTamSX3("D2_VALIMP"+Str(nY,1)) )
			cCposSD2 += ", "+"D2_VALIMP"+Str(nY,1)
		Endif
	Next
	cCposSD2 += "%"
	...
	SELECT 	D2_FILIAL,D2_COD,D2_LOCAL,D2_SERIE,D2_TES,D2_TP,D2_GRUPO,D2_CONTA,D2_EMISSAO,D2_TIPO,D2_DOC,D2_QUANT,D2_TOTAL,%Exp:cExpAdi%
			D2_VALIPI,D2_PRCVEN,D2_ITEM,D2_CLIENTE,D2_LOJA,F2_MOEDA,F2_TXMOEDA %Exp:cCposSD2% 
...
If oReport:Section(1):GetOrder() = 1 .Or. oReport:Section(1):GetOrder() = 6							// Por Tes	
	cVaria := "D2_TES"
	cChave := "D2_FILIAL+D2_TES+"+IIf(oReport:Section(1):GetOrder()==1,"D2_COD","D2_SERIE+D2_DOC")
ElseIF oReport:Section(1):GetOrder() = 2																// Por Tipo
	cVaria := "D2_TP"
	cChave := SD2->(IndexKey(2))
ElseIF oReport:Section(1):GetOrder() = 3																// Por Grupo
	cVaria := "D2_GRUPO"
	cChave := "D2_FILIAL+D2_GRUPO+D2_COD"
ElseIF oReport:Section(1):GetOrder() = 4																// Por Conta Contabil
	cVaria := "D2_CONTA"
	cChave := "D2_FILIAL+D2_CONTA+D2_COD"
Else																									// Por Produto
	cVaria := "D2_COD"
	cChave := "D2_FILIAL+D2_COD+D2_LOCAL+D2_SERIE+D2_DOC"
EndIF
...
oTmp:AddIndex("01", Separa(cChave,"+",.F.) )
...
//IndRegua("TRB",cIndTrab,cChave,,,STR0049)     					// "Selecionando Registros..."
...
If nOrdem = 1 .Or. nOrdem = 6	// Por Tes
	...
	//cIndTrab := SubStr(cIndice,1,7)+"A"
	cChave   := "D2_FILIAL+D2_TES+"+IIf(nOrdem==1,"D2_COD","D2_SERIE+D2_DOC")
ElseIF nOrdem = 2			// Por Tipo
	dbSetOrder(2)
	//cIndTrab := SubStr(cIndice,1,7)+"A"
	cChave   := SD2->(IndexKey())
...
//IndRegua("TRB",cIndTrab,cChave,,,STR0021)   //"Selecionando Registros..."
...
±±ºPrograma  ³AjustaSX1 ºAutor  ³Leonardo Ruben      º Data ³  06/07/04   º±±
...
u_HMPutSx1( cPerg, "12","Imprime Documento      ?","¿Imprime documento      ?","Print Document         ?","mv_chc","N",1,0,1,"C","","","","",;
...
u_HMPutSx1(cPerg,"13","Produto De  ?","De Producto ?","From Product?","mv_chd","C",30,0,0,"G","","SB1","","S",;
...
u_HMPutSx1(cPerg,"14","Produto Ate ?","A Producto  ?","To Product  ?","mv_che","C",30,0,0,"G","","SB1","","S",;
