...
TRCell():New(oFatCli,"TB_CLI"		,cNomArq0,RetTitle("A1_COD")	,PesqPict("SA1","A1_COD"	)	,TamSx3("A1_COD")		[1]	,/*lPixel*/,{|| (cNomArq0)->TB_CLI })			// "Codigo"
TRCell():New(oFatCli,"TB_LOJA"		,cNomArq0,RetTitle("A1_LOJA"),PesqPict("SA1","A1_LOJA"	)	,TamSx3("A1_LOJA")		[1]	,/*lPixel*/,{|| (cNomArq0)->TB_LOJA })			// "Loja"
...
TRCell():New(oFatCli,"TB_VALOR1"	,cNomArq0,STR0021			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR1 })		// "Faturamento sem ICMS"
TRCell():New(oFatCli,"TB_VALOR2"	,cNomArq0,STR0022			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR2 })		// "Valor da Mercadoria"
TRCell():New(oFatCli,"TB_VALOR3"	,cNomArq0,STR0023			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR3 })		// "Valor Total"
TRCell():New(oFatCli,"TB_RANKIN"	,cNomArq0,STR0024			,"@E 9999"						,4							,/*lPixel*/,{|| (cNomArq0)->TB_RANKIN })		// "Ranking"
TRCell():New(oFatCli,"TB_VALOR4"	,cNomArq0,STR0021			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR4*-1 })		// "Faturamento sem ICMS"
TRCell():New(oFatCli,"TB_VALOR5"	,cNomArq0,STR0022			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR5*-1 })		// "Valor da Mercadoria"
TRCell():New(oFatCli,"TB_VALOR6"	,cNomArq0,STR0023			,PesqPict("SF2","F2_VALBRUT")	,TamSx3("F2_VALBRUT")	[1]	,/*lPixel*/,{|| (cNomArq0)->TB_VALOR6*-1 })		// "Valor Total"
TRCell():New(oFatCli,"TB_TIPO" 		,cNomArq0,"Tipo"				,/*Picture*/			 		,/*Tamanho*/				,/*lPixel*/,/*{|| code-block de impressao }*/)	// Se Devolucao imprime "DEV"
...
TRCell():New(oTotal,"CLOJA"		,cNomArq0,RetTitle("A1_LOJA")	,PesqPict("SA1","A1_LOJA"	),TamSx3("A1_LOJA")		[1]	,/*lPixel*/,/*{|| code-block de impressao }*/)			// "Loja"
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
TRPosition():New(oReport:Section(1),"SA1",1,{|| xFilial("SA1")+(cNomArq0)->TB_CLI+(cNomArq0)->TB_LOJA })

//Posiciona na tabela SF2 antes da impressão de cada linha
TRPosition():New(oReport:Section(1),"SF2",2,{|| xFilial("SF2")+(cNomArq0)->TB_CLI+(cNomArq0)->TB_LOJA+(cNomArq0)->TB_DOC+(cNomArq0)->TB_SERIE })
...
// Alias da tabela
//------------------
cNomArq0 := oTmp:GetAlias()

//cNomArq0 := CriaTrab(aCampos)
//dbUseArea( .T.,, cNomArq0,cNomArq0, if(.T. .OR. .F., !.F., NIL), .F. )
cNomArq1 := SubStr(cNomArq0,1,7)+"1"
cNomArq2 := SubStr(cNomArq0,1,7)+"2"
cNomArq3 := SubStr(cNomArq0,1,7)+"3"
cNomArq4 := SubStr(cNomArq0,1,7)+"4"
IndRegua(cNomArq0,cNomArq1,"TB_CLI+TB_LOJA",,,OemToAnsi(STR0012))	//"Selecionando Registros..."
...
dbSelectArea(cNomArq0)
dbCommit()
dbClearIndex()

// Lista por: 1=Cliente, 2=Ranking, 3=Estado
If mv_par07 == 1			// Listar por Cliente   
	IndRegua(cNomArq0,cNomArq2,"StrZero(1000000000000 - TB_VALOR3 + TB_VALOR6,18,"+Str(nDecs)+")",,,OemToAnsi(STR0012))	// "Selecionando Registros..."
	dbGoTop()
	nRank:=1
	While !Eof()
		RecLock(cNomArq0,.F.)
...
Else							// Listar por Estado	
	IndRegua(cNomArq0,cNomArq1,"TB_EST+TB_CLI+TB_LOJA",,,OemToAnsi(STR0012))	// "Selecionando Registros..."
...
dbSelectArea(cNomArq0)
dbGoTop()
If (cNomArq0)->(RecCount()) > 0
	oReport:SetMeter(RecCount())		// Total de Elementos da regua
	nVal01 :=nVal02 := nVal03 := nVal04 := nVal05 := nVal06 := 0

	While !oReport:Cancel() .And. !(cNomArq0)->(Eof())
	
		// Lista por: 1=Cliente, 2=Ranking, 3=Estado
		If mv_par07 == 1				// Listar por Cliente   
			cChave := (cNomArq0)->TB_CLI+(cNomArq0)->TB_LOJA
			bChave := {|| !oReport:Cancel() .And. !(cNomArq0)->(Eof()) .And. (cNomArq0)->TB_CLI+(cNomArq0)->TB_LOJA == cChave }
		ElseIf mv_par07 == 2			// Listar por Ranking	
			cChave := (cNomArq0)->TB_RANKIN	            
			bChave := {|| !oReport:Cancel() .And. !(cNomArq0)->(Eof()) .And. (cNomArq0)->TB_RANKIN == cChave }		
		Else							// Listar por Estado	
			cChave := (cNomArq0)->TB_EST
			bChave := {|| !oReport:Cancel() .And. !(cNomArq0)->(Eof()) .And. (cNomArq0)->TB_EST == cChave }		
		Endif
		...
		While Eval(bChave)
			...
			nValor1 := (cNomArq0)->TB_VALOR1
			nValor2 := (cNomArq0)->TB_VALOR2
			nValor3 := (cNomArq0)->TB_VALOR3
			nValor4 := (cNomArq0)->TB_VALOR4
			nValor5 := (cNomArq0)->TB_VALOR5
			nValor6 := (cNomArq0)->TB_VALOR6
			
			nVal01 += (cNomArq0)->TB_VALOR1
			nVal02 += (cNomArq0)->TB_VALOR2
			nVal03 += (cNomArq0)->TB_VALOR3
			nVal04 += (cNomArq0)->TB_VALOR4
			nVal05 += (cNomArq0)->TB_VALOR5
			nVal06 += (cNomArq0)->TB_VALOR6
			
			// Impressao das Devolucoes
			If (cNomArq0)->TB_VALOR4 <> 0 .Or. (cNomArq0)->TB_VALOR5 <> 0 .Or. (cNomArq0)->TB_VALOR6 <> 0
				...
				oReport:Section(1):Cell("TB_TIPO"):SetValue((cNomArq0)->TB_TIPO)
...
cDelArq := cNomArq0+GetDBExtension()
...
While !Eof() .And. xFilial()=SF2->F2_FILIAL 
	...
	If lProcessa
		...
		If cPaisLoc <> "BRA" .and. SF2->(FieldPos("F2_TXMOEDA")) > 0
		...
		If nTOTAL > 0
			...
			If Ascan( aCodCli, SF2->F2_CLIENTE ) == 0
				Aadd( aCodCli, SF2->F2_CLIENTE )
			Endif
...
If dbseek(xFilial()+cSerieOri+cNumOri+cClienteOri+cLojaOri,.F.)
	...
	For nX :=1 to Len(aNotDev)
		...
		If dbSeek(aNotDev[nX])
			...
			While !Eof() .and. xFilial()==SD1->D1_FILIAL .And.SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE ==SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE
...
If mv_par07 == 1
	...
	IndRegua(cNomArq0,cNomArq2,"StrZero(1000000000000 - TB_VALOR3 + TB_VALOR6,18,"+Str(nDecs)+")",,,OemToAnsi(STR0012))
	dbGoTop()
	nRank:=1
	While !Eof()
		RecLock(cNomArq0,.F.)
	...
	dbClearIndex()
	IndRegua(cNomArq0,cNomArq1,"TB_CLI+TB_LOJA",,,OemToAnsi(STR0012))
ElseIf mv_par07 == 2
	IndRegua(cNomArq0,cNomArq2,"StrZero(1000000000000 - TB_VALOR3 + TB_VALOR6,18,"+Str(nDecs)+")",,,OemToAnsi(STR0012))
Else
	IndRegua(cNomArq0,cNomArq1,"TB_EST+TB_CLI+TB_LOJA",,,OemToAnsi(STR0012))
Endif
...
While !Eof() .And. xFilial()=SF2->F2_FILIAL 
	...
	If lProcessa
		...
		If nTOTAL > 0
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			If mv_par09 == 3
				u_GravaDev(SF2->F2_DOC,SF2->F2_SERIE,SF2->F2_CLIENTE,SF2->F2_LOJA,nMoeda,cEstoq,cDupli)
			Endif
		Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User FuncTion GravaDev(cNumOri,cSerieOri,cClienteOri,cLojaOri,nMoeda,cEstoq,cDupli)
Local cNum        := ""
Local cSerie      := ""
Local cFornece    := ""
Local cLoja       := ""
...
If cPaisLoc=="BRA" 
	u_HMPutSx1("MTR590","15","Deduz PIS/COFINS e ICMS?"  ,"Deduce PIS/COFINS e ICMS?","It deduces  PIS/COFINS e ICMS?","mv_chf","N",1,0,2,"C","","","","","mv_par15","Sim"            ,"Si"             ,"Yes"          ,"","Nao"            ,"No"                  ,"No"                         ,"","","","","","","","","",aHelpP,aHelpE,aHelpS)
EndIf
