...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦ Programa  ¦ RSFATR19   ¦ Autor ¦Orismar Silva         ¦ Data ¦ 11/03/2019 ¦¦¦
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦           ¦ Relatório de Faturamento por cliente x produto global         ¦¦¦
¦¦¦           ¦ no período.                                                              ¦¦¦
¦¦+-----------+---------------------------------------------------------------+¦¦
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
+-------------------------------------------------------------------------------+
/*/
User Function RSFATR19()      
...
Static Function ReportDef()
	...
	oReport := TReport():New("RSFATR19", cTitulo, cPerg , {|oReport| PrintReport(oReport)},"Emitirá Relatório de Faturamento por Cliente x Produto Global")

	oReport:SetPortrait()
	oReport:SetTotalInLine(.F.)
	oReport:ShowHeader()

	oSection1 := TRSection():New(oReport,"Relatório de Faturamento por cliente x produto global",{""})
...
Static Function PrintReport(oReport)
   ...
   if mv_par09 = 1//Considera Venda/Devolução - Venda
	  ...
	  _cQuery :=" SELECT B1_CODJP,D2_CLIENTE,A1_CONTA,RDATA, SUM(D2_QUANT) D2_QUANT, SUM(TOTAL) TOTAL FROM ( "
	  _cQuery +=" SELECT B1_CODJP,D2_CLIENTE,A1_CONTA,SUBSTRING(D2_EMISSAO,1,6) RDATA, SUM(D2_QUANT) D2_QUANT ,SUM(D2_TOTAL) TOTAL "
	  ...
	  _cQuery +=" GROUP BY B1_CODJP,D2_CLIENTE,A1_CONTA,SUBSTRING(D2_EMISSAO,1,6) 
	  
	  if mv_par08 = 1//Inclui Devolução - Sim
	     _cQuery +=" UNION ALL 
	     _cQuery +=" SELECT B1_CODJP,D1_FORNECE AS D2_CLIENTE,A1_CONTA,SUBSTRING(D1_DTDIGIT,1,6) RDATA, -SUM(D1_QUANT) D2_QUANT , -SUM(D1_TOTAL-D1_VALDESC) TOTAL
	     ...
	     _cQuery +=" GROUP BY B1_CODJP,D1_FORNECE,A1_CONTA,SUBSTRING(D1_DTDIGIT,1,6)
	  endif
	else
       _cQuery :=" SELECT B1_CODJP,D2_CLIENTE,A1_CONTA,RDATA, SUM(D2_QUANT) D2_QUANT, SUM(TOTAL) TOTAL FROM ( "
	   _cQuery +=" SELECT B1_CODJP,D1_FORNECE AS D2_CLIENTE,A1_CONTA,SUBSTRING(D1_DTDIGIT,1,6) RDATA, -SUM(D1_QUANT) D2_QUANT , -SUM(D1_TOTAL-D1_VALDESC) TOTAL
	...
	_cQuery +=" ) A GROUP BY B1_CODJP,D2_CLIENTE,A1_CONTA,RDATA
	_cQuery +=" ORDER BY D2_CLIENTE,A1_CONTA,B1_CODJP
