...
Local lIpiDev   	:= GetNewPar("MV_IPIDEV",.F.)   	        //Apenas para devolu��o de compra de IPI (nota de sa�da). T-S�ra gerado na tag vIPI e destacado no campo//VALOR IPI do cabe�alho do danfe. F-Ser� gerado na tag vIPIDevol e destacado nas informa��es complementares do danfe.
...
Local lIpiBenef		:= GetNewPar("MV_IPIBENE",.F.) 				   //Nota de sa�da de retorno com tipo = Beneficiamento. .T.- Ser� gerado na tag vOutro e destacado nas informa��es//complementares do danfe e no campo OUTRAS DESPESAS ACESSORIAS. .F. - S�ra gerado na tag vIPI e destacado no campo//VALOR IPI do cabe�alho do danfe (procedimento padr�o)
Local lIPIOutB 	    := GetNewPar("MV_IPIOUTB",.F.) .And. lIpiBenef //Quando habilitado juntamente com o MV_IPIBENE, o valor do IPI ser� gerado na tag vOutro, destacado nas informa��es complementares e no campo OUTRAS DESPESAS ACESSORIAS.
...
Local lDevSimpl		:= GetNewPar("MV_DEVSIMP",.F.)	//Define se sera gravado no XML o valor e base de ICMS para nf de devolucao do Simples nacional. (Padrao F - n�o leva)
...
local cDesc99		:= "" //Descri��o da forma de pagamento quando 99 - outros faturamento //----------------------  INICIO HISAMITSU - Jean 16/05/2011
//----------------------  INICIO HISAMITSU - Jean 16/05/2011
Local nValorII      := 0
Local ix

Private axPed	    := {}
Private lxRefNFE    := .T.
Private cEmailFat   := SuperGetMV("MV_XEMAILF",,"")
Private lchindnf    := .T.
Private aNfVinc   	:= {}
//----------------------  FIM HISAMITSU
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				//Verifica se � uma venda de origem do Venda Direta ou SIGALOJA
				dbSelectArea("SL1")
				...
				If !Empty(SF2->F2_DUPL)	.Or. lVLojaDir
					...
					If lVLojaDir
						//Verifica se � venda com Entrega ou Retira Posterior, pois pode ter ocorrido em outra filial.
						If !Empty(SL1->L1_ORCRES)
							cFilTit := xFilial("SE1",SL1->L1_FILRES)
							SL1->(DbSetOrder(1)) //L1_FILIAL + L1_NUM
							//Posiciona no Or�amento Pai
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If (cAliasSD2)->D2_TIPO == "D" 
						If SM0->M0_ESTENT == "PR" .And. (cAliasSD2)->D2_ICMSRET > 0
							...
							para atender a ICMS/PR 2017 (Decreto 7.871/2017) Art. 9, Se��o I, Anexo IX 
					...
					// Destacar ICMS pr�prio no XML quando MV_ICMDEVO = .F. e nota n�o seja tipo Devolu��o - DSERTSS1-16233
					...
					If lContinua
						...
						// inicio HISAMITSU
    				    IF SA1->A1_EST <> "AM" .And. !("DCI MENSAL " $ cMensCli)
						   cMensCli += Padr("DCI MENSAL",80)
					    EndIf
					    If Alltrim(SF4->F4_CODIGO) <> '594'
						   If SF4->F4_BASEICM > 0 .And. !("BASE DE CALCULO DO ICMS REDUZIDA CONFORME " $ cMensCli)
							  cMensCli += Padr("BASE DE CALCULO DO ICMS REDUZIDA CONFORME ",80)
							  cMensCli += Padr("INCISO VI DO ART.19 DA LEI 2826/2003",80)
						   EndIf
					    EndIf					
                        //fim HISAMITSU
                        ...
                        if lxRefNFE						   
						   if !EMPTY(SC5->C5_XREFNFE)
                              ...
                              For ix:=1 to nLinha
                                  ...
                                  if ix = 1 //Consula a NF-e de sa�da recusada
                                     cAreaSF2  := SF2->(GetArea()) 
                                     dbSelectArea("SF2")
                                     SF2->(dbSetOrder(17))
                                     SF2->(dbSeek(SD2->D2_FILIAL+cChaveNF))
                                     aAdd( aNfVinc, { SF2->F2_EMISSAO, SF2->F2_SERIE, SF2->F2_DOC,POSICIONE("SA1",1,XFILIAL("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA,"A1_CGC"),SF2->F2_EST,SF2->F2_ESPECIE, SF2->F2_CHVNFE  } )
                                     RestArea(cAreaSF2)
                                  elseif ix = 2 //Consulta a NF-e de Entrada e o CT-e
                                     dbSelectArea("SF1")
                                     SF1->(dbSetOrder(8))
                                     SF1->(dbSeek(SD2->D2_FILIAL+cChaveNF))
                                     aAdd( aNfVinc, { SF1->F1_EMISSAO, SF1->F1_SERIE, SF1->F1_DOC,POSICIONE("SA1",1,XFILIAL("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA,"A1_CGC"),SF1->F1_EST,SF1->F1_ESPECIE, SF1->F1_CHVNFE  } )
                                  else //Consulta o CT-e
                                     dbSelectArea("SF1")
                                     SF1->(dbSetOrder(8))
                                     SF1->(dbSeek(SD2->D2_FILIAL+cChaveNF))
                                     aAdd( aNfVinc, { SF1->F1_EMISSAO, SF1->F1_SERIE, SF1->F1_DOC,POSICIONE("SA2",1,XFILIAL("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA,"A2_CGC"),SF1->F1_EST,SF1->F1_ESPECIE, cChaveNF  } )                                     
                                  endif
                              Next   
                           endif   
                           lxRefNFE := .F.
						...
						//INICIO HISAMITSU
					    If !lMensagem
					       if Len(Alltrim(SF2->F2_XOBSN)) > 0 .and. !AllTrim(SF2->F2_XOBSN) $ cMensCli
					          cMensCli += AllTrim(SF2->F2_XOBSN)
					       Endif
					       lMensagem := .T.
				        Endif  
				        if SF2->F2_DESCONT > 0 
				           IF !("DESCONTO CONCEDIDO POR REPASSE FISCAL R$  " $ cMensCli)
						      cMensCli += Padr("DESCONTO CONCEDIDO POR REPASSE FISCAL R$ "+ Transform(SF2->F2_DESCONT,"@E 999,999,999.99"),80)
					       EndIf
				        endif				   
                        //fim HISAMITSU
                        //---------------------------------------- INCLUIDO - HISAMITSU
					    IF !Empty(SC5->C5_PEDCLI) .And. !("Pedido Cliente: " $ cMensCli)
						   cMensCli += Padr("Pedido Cliente: "+ SC5->C5_PEDCLI,80)
					    EndIf
					    IF !("Nosso Pedido: " $ cMensCli)
						   cMensCli += Padr("Nosso Pedido: "+ SC5->C5_NUM,80)
					    EndIf
					    IF !Empty(SF4->F4_MENS1) .And. !(Alltrim(SF4->F4_MENS1) $ cMensCli)
						   cMensCli += Padr(Alltrim(SF4->F4_MENS1),80)
					    EndIf
					    IF !Empty(SF4->F4_MENS2) .And. !(Alltrim(SF4->F4_MENS2) $ cMensCli)
						   cMensCli +=  Padr(Alltrim(SF4->F4_MENS2),80)
					    EndIf
					    IF !Empty(SF4->F4_MENS3) .And. !(Alltrim(SF4->F4_MENS3) $ cMensCli)
						   cMensCli +=  Padr(Alltrim(SF4->F4_MENS3),80)
					    EndIf
					    IF !Empty(SF4->F4_MENS4) .And. !(Alltrim(SF4->F4_MENS4) $ cMensCli)
						   cMensCli +=  Padr(Alltrim(SF4->F4_MENS4),80)
					    EndIf
					    //---------------------------------------- FIM - HISAMITSU
						cDCR += IIF(!Empty(SB1->B1_DCR),Alltrim(SB1->B1_DCR) +" / ","") // HISAMITSU
						
						 //--------------- INCLU�DO HISAMITSU - POR JEAN VICENTE 12/01/2011 - Alterado por ADSON 20/09/2012
					    _PrcMax := Posicione("DA1",1,XFILIAL("DA1")+SC5->C5_TABELA+SC6->C6_PRODUTO,"DA1_PRCMAX")

					    //HISAMITSU
					    //If !(Alltrim(SC6->C6_PRODUTO)$"61222|61223") // solicitacao dia 29/03/2017
						   If cVeramb >= "4.00"
						      //ALERT("Lote: "+SC6->C6_LOTE1)
						      aadd(aLote,{SC6->C6_LOTE1,SC6->C6_QTDLOTE,SC6->C6_FABRIC,SC6->C6_LTVALID,IIF(_PrcMax>0,_PrcMax,SC6->C6_PRCVEN)})
						      //aadd(aMed,{SC6->C6_LOTE1,SC6->C6_QTDLOTE,SC6->C6_FABRIC,SC6->C6_LTVALID,IIF(_PrcMax>0,_PrcMax,SC6->C6_PRCVEN),ALLTRIM(SC6->C6_OPC),""}) 
						      if SC5->C5_CLIENTE = '000307'   //Somente para o cliente D. CENTER incluir a tag aMed						        
						        cCodAnvisa := ALLTRIM(Posicione("SB1",1,XFILIAL("SB1")+SC6->C6_PRODUTO,"B1_XANVISA"))
						        if !EMPTY(cCodAnvisa)
						            aadd(aMed,{"","","","",IIF(_PrcMax>0,_PrcMax,SC6->C6_PRCVEN),cCodAnvisa,""})
						        else 
						           aadd(aMed,{"","","","","","",""})						        
						        endif
						      else
						      ...
						      aadd(aMed,{SC6->C6_LOTE1,SC6->C6_QTDLOTE,SC6->C6_FABRIC,SC6->C6_LTVALID,IIF(_PrcMax>0,_PrcMax,SC6->C6_PRCVEN),"",""})
						      aadd(aLote,{}) 
						   endif
					    /*
					    Else
                        ...
                        */   
						...
						EndIf
						lCodLan := .F.
						If Upper(SM0->M0_ESTENT) $ cCodCST //TAG cBenef buscar o conte�do da tabela 5.2 no sistema quando for do PR.
							dbSelectArea("CDV")
						...
						Else
							...
							If SM0->M0_ESTENT <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0
								...
								If CDA->(MsSeek(cSeekCDA))
									//While alltrim(cSeekCDA) == alltrim(CDA->(CDA_FILIAL + CDA_TPMOVI + CDA_ESPECI + CDA_FORMUL + CDA_NUMERO + CDA_SERIE + CDA_CLIFOR + CDA_LOJA + CDA_NUMITE))// HISAMITSU
                                    While cSeekCDA == CDA->(CDA_FILIAL + CDA_TPMOVI + CDA_ESPECI + CDA_FORMUL + CDA_NUMERO + CDA_SERIE + CDA_CLIFOR + CDA_LOJA + ALLTRIM(CDA_NUMITE))  //While cSeekCDA == CDA->(CDA_FILIAL + CDA_TPMOVI + CDA_ESPECI + CDA_FORMUL + CDA_NUMERO + CDA_SERIE + CDA_CLIFOR + CDA_LOJA + CDA_NUMITE) // HISAMITSU
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							IIF(Empty(SC6->C6_DESCRI),ALLTRIM(SB1->B1_DESC),Alltrim(SC6->C6_DESCRI)+Space(25-(Len(Alltrim(SC6->C6_DESCRI))%25))+IIF(!Empty(SC6->C6_LOTE1)," Lote: "+SC6->C6_LOTE1,"")),; //HISAMITSU //cDescProd,;
							...
							SB1->B1_UM,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(!(lMvNFLeiZF),iif((cAliasSD2)->D2_CLIENTE = "000194" .AND. (cAliasSD2)->D2_LOJA $ '03|04',(cAliasSD2)->D2_TOTAL+nDesconto,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),; // HISAMITSU //IIF(!((cAliasSD2)->D2_TIPO$"IP" .Or. ((cAliasSD2)->D2_TIPO $ "D" .And. cTpOrig == "P")) ,IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR-nDesVrIcms,(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC+nDesVrIcms)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
						...
						ElseiF AliasIndic("CDL")
							...
							While !CDL->(Eof()) .And. CDL->CDL_FILIAL+CDL->CDL_DOC+CDL->CDL_SERIE+CDL->CDL_CLIENT+CDL->CDL_LOJA == xFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA
						    	If CDL->(FieldPos("CDL_PRODNF")) <> 0 .And. CDL->(FieldPos("CDL_ITEMNF")) <> 0 .And. AllTrim(CDL->CDL_PRODNF)+AllTrim(CDL->CDL_ITEMNF) == AllTrim((cAliasSD2)->D2_COD)+AllTrim((cAliasSD2)->D2_ITEM)
									...
									aAdd(aDados,{"NAT_EXP","Natureza", IIF(CDL->(FieldPos("CDL_NATEXP"))<>0, CDL->CDL_NATEXP ,"") }) //Adicionado para utilizar na verifica��o da montagem das tags do grupo de exporta��o indireta (I52 - exportind) substituindo o I53 - nRE.
						...
						If AliasIndic("F0A") .AND. F0A->(FieldPos("F0A_LOTE")) > 0 .And. !Empty(F0A->F0A_LOTE)
							...
							*/ //HISAMITSU
						//ElseIf !Empty(aMed) .And. !Empty(aMed[1][1])//HISAMITSU
						//	aadd(aLote,{CD7->CD7_LOTE,CD7->CD7_QTDLOT,CD7->CD7_FABRIC,CD7->CD7_VALID,""})//HISAMITSU
						Else
						//	aadd(aLote,{})//HISAMITSU
	   					EndIf	
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "TST" 
									...
									aImp := {;
												...
												IIf(CD2->CD2_ALIQ > 0,CD2->CD2_ALIQ,0),;			//pICMSRet - Al�quota da Reten��o
				...
				// Msg Zona Franca de Manaus / ALC
					...
					If !SF2->F2_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )//Desnecessario seek redundante na SF3 pois o campo F2_DESCZFR ja possui os valores de ZFR de toda a venda
					    ...
					    Else
						 //HISAMITSU
					    	if !(SF3->F3_CLIEFOR = "000194" .AND. SF3->F3_LOJA $ '03|04')
				...
				//HISAMITSU       
   		        If !Empty(cDCR)
				   cMensCli := Alltrim(cMensCli)+" "+Padr(" DCR N.: "+aLLTRIM(cDCR),80)
			    EndIf
                //HISAMITSU	
				//Tratamento implementado para atender a ICMS/PR 2017 (Decreto 7.871/2017) 
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			#ENDIF
			if (cAliasSD1)->D1_TES = '305' .AND. lchindnf  //HISAMITSU - NOTA DE ESTORNO
		           CHINDNF((cAliasSD1)->D1_NFORI,(cAliasSD1)->D1_SERIORI,aNfVinc)
		           lchindnf := .F.	
		           lVinc    := .T.	
		           //ALERT("CHINDNF")	
		    endif
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf 
			  //--------  INICIO - HISAMITSU - JEAN 02/07/2011
			   aadd(axPed,{"",""})
			   //--------  FIM - HISAMITSU  		
		    ...
		    EndDo
			//------------------- INICIO - HISAMITSU
			If !lEasy .And. Len(aDI)>0
				For nX:=1 To Len(aDI)
					If Len(aDI[nX])>0 .And. !Empty(aDI[nX][19][03])
						nValorII += aDI[nX][19][03]
					EndIf
				Next nX
			EndIf
...
If FunName() <> "SPEDNFSE"
	IF cVeramb >= "4.00"
	   //Ajute para alimentar o aDetPag
		...
		If	cTPNota $ '3-4' .or. ( cTPNota == "2" .and. (aTotal[2]+aTotal[03] == 0 ))
		...
		Else		
			...
			if cTipo == "1"
				cDesc99	:= &(SuperGetMV("MV_MFATIPR",,'"Negocia��o Futura"')) //Descri��o da forma de pagamento quando 99 - outros faturamento 
			else
				cDesc99	:= &(SuperGetMV("MV_TPAGCOM",,'"Negocia��o Futura"')) //Descri��o da forma de pagamento quando 99 - outros compras
			endIf
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					aAgrPis[nX],aAgrCofins[nX],nIcmsDif,aICMUFDest[nX],@nvFCPUFDest,@nvICMSUFDest,@nvICMSUFRemet,cAmbiente,aIPIDevol[nX],@nvBCUFDest, aItemVinc[nX], @npFCPUFDest,@npICMSUFDest,@npICMSInter,@npICMSIntP,aLote[nX],@cMensDifal,@aTotICMSST,len(aProd), nX, @nValDifer,axPed[nX])//HISAMITSU //aAgrPis[nX],aAgrCofins[nX],nIcmsDif,aICMUFDest[nX],@nvFCPUFDest,@nvICMSUFDest,@nvICMSUFRemet,cAmbiente,aIPIDevol[nX],@nvBCUFDest, aItemVinc[nX], @npFCPUFDest,@npICMSUFDest,@npICMSInter,@npICMSIntP,aLote[nX],@cMensDifal,@aTotICMSST,len(aProd), nX, @nValDifer)
	...
	If nValDifer > 0
		cMensCpl += "Diferimento do ICMS que exceder 12% - Base Legal Item II da Subse��o III da Se��o IV do Ap�ndice II do RICMS/RS. Valor do ICMS Diferido R$ " + ConvType(nValDifer,15,2) + "."
	EndIf
...
If(!Empty(aNfVinc)	.And. Empty(aExp[1])) .or.(!Empty(aNfVinc).And. !Empty(aExp[1]) .and. lEECFAT)
	cString += '<NFRef>'
	//ALERT("aNfVinc: "+STR(Len(aNfVinc)))
	For nX := 1 To Len(aNfVinc)
	    if !EMPTY(aNfVinc[nX][03])//HISAMITSU
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	EndIf
	//HISAMITSU
	if LEFT(aDest[1],8) = '61585865'   //CLIENTE DROGASIL E RAIA DROGASIL
		If !Empty(aDest[16]) .or. !Empty(AllTrim(aTransp[07]))
			//cMailTrans := ";"+AllTrim(cEmailFat)
		Else 
			cMailTrans := AllTrim(cEmailFat)
		EndIf 		
	endif  
...
EndIf
//--------------------- INICIO - HISAMITSU - JEAN 16/05/2011
if Len(axPed) > 0 .And. !Empty(axPed[01])
	cString += '<xPed>'+ConvType(axPed[01])+'</xPed>'
end
//--------------------- FIM - HISAMITSU
...
If Len(aPedCom) > 0 .And. !Empty(aPedCom[01])
	//cString += '<xPed>'+ConvType(aPedCom[01])+'</xPed>'//--  ALTERADO HISAMITSU
...
If Len( aRefECF ) > 0	 	//Nota de esp�cie ECF vinculada
...
If Len( aNfVincRur ) > 0 	//Nota de esp�cie NFP vinculada 
	...
	If !Empty(cCondPag)
		Do Case
			...
			Case cCondPag == "BOL" //BOLETO BANCARIO
				cForma := "15"
			//Case cCondPag == "XX" //Dep�sito Banc�rio
			//	cForma := "16"
			Case cCondPag == "PX" //Pagamento Instant�neo (PIX)
				cForma := "17"
			Case cCondPag == "PD" //Transfer�ncia banc�ria, Carteira Digital
				cForma := "18"
			Case cCondPag == "FID" //Programa de fidelidade, Cashback, Cr�dito Virtual
				cForma := "19"
...
Valida��o com a tabela 5.2 para enviar o codigo SEM CBENEF de acordo com a UF
...
Funcao responsavel por determinar se � um estorno de devolu��o 

@param  cTipo - Tipo do pedido de venda
@param  cCFOP - numero do CFOP utilizado

@return	lRet - .T. se � opera��o de estorno de devolu��o/
				.F. se n�o � estorno de devolu��o
...
/*/{Protheus.doc} EstDevSeek
	Fun��o responsavel por posicionar e verificar a integridade da opera��o 
	de estorno de devolu��o.
	@type  Static Function
	@author Felipe Sales Martinez
	@since 09/10/2020
	@version 12
	@param cAliasSD2 - alias com as informa��es do item da nota
	@return lRet: .T. posicionou e validou corretamente
			lRet: .F. problemas na opera��o
/*/
...
Retorna de acordo com o indicador de presen�a indica se pode ser informado
o indIntermed
@param		cIndPres, String, indicado de presen�a
@param		cIntermediador, String, Codigo do intermediador da operacao de venda
@return		lRet, boleano, se � necessario informar o IndIntermed ou n�o
@author  	Felipe Sales Martinez
@since   	11/03/2021
@version 	1.0
/*/
...
//Retirar o parametro apos a data de entrada em produ��o da NT 2020.006 (prevista para 01/09/2021)
if ( cIndPres $ "2,3,4,9" .or. (cIndPres == "1" .and. !empty(cIntermediador)) ) .and. date() >= cTod(SuperGetMv('MV_NT2006I',.F.,"05/04/2021"))
	if empty(cIntermediador)
		cIndIntermed := "0" //0=Opera��o sem intermediador (em site ou plataforma pr�pria)
	else
		cIndIntermed := "1" //1=Opera��o em site ou plataforma de terceiros (intermediadores/marketplace)
	endIf
endIf
...
@param		aNota, Array, Informa��es da nota
...
//TODO: se valor de Default deverar ser retirado apos entrada da NT2020.006 em produ��o (prevista para 01/09/2021)
if empty(cIndPres)
	If aNota[5] == "N"
