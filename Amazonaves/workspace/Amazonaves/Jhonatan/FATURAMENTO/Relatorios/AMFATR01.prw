...
¦¦+-----------+------------+-------+----------------------+------+------------+¦¦
¦¦¦ Descriçäo ¦ Script de Proposta - Amazonaves 							  ¦¦¦
¦¦+-----------+---------------------------------------------------------------+¦¦
...
cQry += " CJ_EMISSAO, CJ_VALIDA, A1_COD, A1_NOME, A1_END, CJ_XNUMOS, CJ_MOEDA, Z4_XSTATUS, Z4_XCODBE, "
cQry += " A1_CEP, A1_PESSOA, A1_CGC, A1_INSCR, A1_TEL,A1_CONTATO , A1_MUN, A1_EST, E4_DESCRI, A1_BAIRRO, CK_VALDESC, CJ_DESCONT,CK_DESCONT,CK_ENTREG " 
...
Private cDatVal, cImposto, cEmbala, cCondPag, cFabric, cAeronave, cMoeda, cDesmoeda // cConv, cPacien, cMedico, cAnvisa, cPrazoEn 
...
nCol3:=nCol2+475	//Partnumber produto
nCol4:=nCol3+775	//Desc Produto
nCol5:=nCol4+100	//Tipo
nCol6:=nCol5+100	//Unidade
nCol7:=nCol6+200	//Prazo
nCol8:=nCol7+200	//Quantidade
nCol9:=nCol8+200	//Preco unitario
nCol10:=nCol9+200	//%Desconto
nCol11:=nCol10+200	//Valor desconto
nLinFim:=2400 		//Total
...
If lPrint
	...
	oPrn:Box (nLine, nCol10 -5,nLine + 60, nCol11 - 5)
	oPrn:Box (nLine, nCol11- 5,nLine + 60, 3150)
	...
	oPrn:Say(nLine ,nCol6 ,PadR("Prazo"        ,10)  ,oFont2n,,,,3)
	oPrn:Say(nLine ,nCol7 ,PadR("Qtde"         ,10)  ,oFont2n,,,,3)
	oPrn:Say(nLine ,nCol8 ,PadR("Preco Unit."  ,12)  ,oFont2n,,,,3)
	oPrn:Say(nLine ,nCol9 ,PadR("(%)Desc."     ,10)  ,oFont2n,,,,3)
	oPrn:Say(nLine ,nCol10 ,PadR("Val.Desc."   ,10)  ,oFont2n,,,,3)
	oPrn:Say(nLine ,nCol11,PadR("Total"        ,15)  ,oFont2n,,,,3)
Else
	nLine += 070
Endif
...
While !(cTmp)->(Eof())
	...
	If lPrint
		...
		oPrn:Say(nLine,nCol6,PadR(Alltrim(dtoc(stod((cTmp)->CK_ENTREG))),10)                              ,cReserv,,xCor,,3)
		oPrn:Say(nLine,nCol7,PadR(Alltrim(Transform((cTmp)->CK_QTDVEN,"@E 999,999.99")),10)               ,cReserv,,xCor,,3)
		oPrn:Say(nLine,nCol8,PadR(Alltrim(Transform(nPrcunit         ,"@E 9,999,999.99")),12)             ,cReserv,,xCor,,3)
		oPrn:Say(nLine,nCol9,PadR(Alltrim(Transform(nAliqDesc         ,"@E 9,999,999.99")),12)            ,cReserv,,xCor,,3)
		oPrn:Say(nLine,nCol10,PadR(Alltrim(Transform(nValDesc         ,"@E 9,999,999.99")),12)            ,cReserv,,xCor,,3)
		oPrn:Say(nLine,nCol11,PadR(Alltrim(Transform(nValor          ,"@E 999,999,999.99")),15)           ,cReserv,,xCor,,3)
...
cMoeda := (cTmp)->CJ_MOEDA
cDesmoeda := ""
//cPrazoEn := (cTmp)->CK_ENTREG
//cConv    := (cTmp)->CJ_XCONV
...
Static Function AmFatR01e(nPaginas)
	...
	If lPrint
		oPrn:Say(nLine,nAux2     ,Upper("Moeda : ")  ,oFont1n,,,,1)
		oPrn:Say(nLine,nAux2+10  ,cDesmoeda                       ,oFont2 ,,,,3)
	Endif
	ver_pag(nDifPixel,nPaginas)
