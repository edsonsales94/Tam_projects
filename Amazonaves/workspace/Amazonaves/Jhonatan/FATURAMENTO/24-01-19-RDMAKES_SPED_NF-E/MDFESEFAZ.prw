...
*/    		
cString += '<tpEmit>1</tpEmit>'  //2 - Transportador de Carga Própria OBS: Para emitentes de NF-e e pelas transportadoras quando estiverem fazendo transporte de carga própria 
	...
	While TRB->(!Eof())
		...
		If !Empty(TRB->TRB_MARCA)
			...
			While TRB->(!Eof()) .and. cCodMun == TRB->TRB_CODMUN
				...
				If !Empty(TRB->TRB_MARCA)
					cString += '<infCTe>'
					cString += '<chCTe>'+ TRB->TRB_CHVNFE +'</chCTe>'
...
Static Function MDFeTotais()
	...
	cString += "<tot>"
	cString += "<qCTe>" + ConvType(nQtNFe,0)   + "</qCTe>"
