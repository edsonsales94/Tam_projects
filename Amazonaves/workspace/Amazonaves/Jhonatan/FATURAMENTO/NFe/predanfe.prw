...
oFWSX1:AddGroup("PREDANFE")
oFWSX1:SearchGroup()
aPergunte := oFWSX1:GetGroup("PREDANFE")
cPerg   := PADR("PREDANFE",LEN(aPergunte[1])," ")
...
If /*IsReady(,,,lUsaColab) .And.*/ !lPreDanfe .Or. Pergunte(cPerg,.T.)
	If lPreDanfe
		dbSelectArea("SF2")
		dbSetOrder(1)
...
Static Function GetIdentPre()
	...
	cQry := "SELECT S001.M0_CODFIL"
	cQry += " FROM CWUX2C_158359_PR_DV.dbo.SYS_COMPANY S001"
//	cQry += " FROM CWUX2C_158358_PR_PD.dbo.SYS_COMPANY S001"
	cQry += " WHERE S001.D_E_L_E_T_ = ' '"
//	cQry += " AND S001.ENTATIV = 'S'"
	cQry += " AND S001.M0_CGC = '"+Trim(SM0->M0_CGC)+"'"
	cQry += " AND S001.M0_INSC = '"+Trim(SM0->M0_INSC)+"'"

	dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQry),cAlias,.T.,.T.)
	cRet := (cAlias)->ID_ENTWS23Q
	dbCloseArea()
	RestArea(aArea)
...
aTotais[02] := Transform(iif(XmlChildEx(oTotal:_ICMSTOT,"_VICMS") == NIL,0,Val(oTotal:_ICMSTOT:_vICMS:TEXT)),"@e 9,999,999,999,999.99")
...
Local cURL			:= PadR(GetNewPar("MV_SPEDURL","http://localhost:8261/sped"),250)
Local cModel		:= "55"
...
Static Function ConverteXML()
	...
	AAdd( aInclui, { "<dhSaiEnt>","<tpEmis>1</tpEmis>", 1, .T.})
	AAdd( aInclui, { "<total>", "<ICMSTot>", 1, .T.})
	AAdd( aInclui, { "</total>", "</ICMSTot>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vProd>"+LTrim(Str(SF2->F2_VALMERC,10,nDec))+"</vProd>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vSeg>0</vSeg>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vDesc>0</vDesc>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vFrete>0</vFrete>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vII>0</vII>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vIPI>0</vIPI>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vIPIDevol>0</vIPIDevol>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vPIS>0</vPIS>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vCOFINS>0</vCOFINS>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vOutro>0</vOutro>", 0, .T.})
	...
	If !Empty(cFile)
		...
		For nX:=1 To Len(aInclui)
			If (nPos := AScan( aLinhas , aInclui[nX,1] )) > 0 .And. (!aInclui[nX,4] .Or. AScan( aLinhas , aInclui[nX,2] ) == 0)
		...
		cFile := ""
		aEval( aLinhas , {|x| cFile += x /*, FWrite(nHandle, x + CRLF)*/ } )
...
Static Function ConvImposto(aLinhas)
	Local nIni, nFim, nX, cCab, nLin, aImp, cCST
	Local nMin := 999999
	Local nMax := 0

	// Enquanto houver tag de imposto modifica para o layout da SEFAZ
	While (nIni := AScan( aLinhas , "<imposto>" )) > 0
		aImp := {}
		nLin := 2
		cCab := ""
		cCST := ""
		nFim := AScan( aLinhas , "</imposto>" )
		...
		nMax := Max(nMax,AScan(aLinhas,"</"+cCab+">"))  // Calcula a linha final dos impostos
	Enddo
