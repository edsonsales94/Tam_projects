...
/*/
User Function Predanfe(lPreNFE,oSetupNFE)
Local oSetup, oDanfe  //, nDevice
//Local aIndArq    := {}
//Local nHRes      := 0
//Local nVRes      := 0
Local aDevice    := {}
...
Local cIdEnt     := ""
Local cFilePrint := ""
Local cPerg      := PADR("PREDANFE",Len(SX1->X1_GRUPO))

Private lPreDanfe := (ValType(lPreNFE) == "U" .Or. lPreNFE)
...
cIdEnt     := GetIdEntPre()
cFilePrint := If( lPreDanfe , "PRE", "") + "DANFE_" + cIdEnt + Dtos(MSDate())+StrTran(Time(),":","")
...
If /*IsReady(,,,lUsaColab) .And.*/ !lPreDanfe .Or. IsInCallStack("MATA410") .Or. Pergunte(cPerg,.T.)
	If lPreDanfe
		If IsInCallStack("MATA410")
			mv_par01 := SC5->C5_NOTA
			mv_par02 := SC5->C5_SERIE
		Endif
		...
		If !dbSeek(XFILIAL("SF2")+mv_par01+mv_par02)
			...
			Return
		ElseIf Trim(SF2->F2_ESPECIE) <> "SPED"
			Alert("Esse documento não se refere a uma NF-e !")
			Return
		Endif
		aNotas := {}
	...
	If ValType(oSetupNFE) == "U" .And. ( !oDanfe:lInJob )
	...
	// ----------------------------------------------
	If ValType(oSetupNFE) <> "U" .Or. oSetup:Activate() == PD_OK // PD_OK =1
		oSetup := If( ValType(oSetupNFE) == "U" , oSetup, oSetupNFE)
		...
		If oSetup:GetProperty(PD_ORIENTATION) == 1
			...
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			PretNfeSef(cIdEnt,,,oDanfe, oSetup, cFilePrint, .T.)
		Else
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
Static Function GetIdentPre()
	/*Local cQry
	Local aArea  := GetArea()
	Local cAlias := GetNextAlias()
	Local cRet   := ""

	cQry := "SELECT S001.ID_ENT"
	cQry += " FROM TSS.dbo.SPED001 S001"
	cQry += " WHERE S001.D_E_L_E_T_ = ' '"
	cQry += " AND S001.ENTATIV = 'S'"
	cQry += " AND S001.CNPJ = '"+Trim(SM0->M0_CGC)+"'"
	cQry += " AND S001.IE = '"+Trim(SM0->M0_INSC)+"'"
...
Return RetIdEnti() //cRet

Static Function PretNfeSef(cIdEnt, cVal1, cVal2, oDanfe, oSetup, cFilePrint, lIsLoja, nTipo	)
	...
	While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
		...
		If (cAliasSF3)->(Eof())
			...
			if lUsaColab
				...
				Else
					aXml := {}
					AAdd( aXML , {"SEM VALOR FISCAL",cFile,"","","",Ctod(""),"","2","SEM VALOR FISCAL","",})
					//Alert("Usando o XML da pre-danfe")
				Endif
			endif
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						If aNotas[nX][02]=="E"
							...
							If MsSeek(xFilial("SF1")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. SF1->(FieldPos("F1_FIMP")) <> 0 .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								RecLock("SF1")
								If !SF1->F1_FIMP$"D" .And. !lPreDanfe
									SF1->F1_FIMP := "S"
								EndIf
								If SF1->(FieldPos("F1_CHVNFE")) > 0 .And. !lPreDanfe
									SF1->F1_CHVNFE := SubStr(NfeIdSPED(aXML[nX][2],"Id"),4)
								EndIf
								If SF1->(FieldPos("F1_HAUTNFE")) > 0 .and. SF1->(FieldPos("F1_DAUTNFE")) > 0 .And. !lPreDanfe //grava a data e hora de autorização da NFe
							...
							if FindFunction("GFECHVNFE") .and. lMVGfe  // Integração com o GFE
								...
								dbSelectArea("SA2")
								dbSetOrder(1)
								If SA2->(MsSeek(xFilial("SA2")+ SF1->F1_FORNECE + SF1->F1_LOJA,.T.)) .And. !lPreDanfe
						...
						Else
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
								RecLock("SF2")
								If !SF2->F2_FIMP$"D" .And. !lPreDanfe
									SF2->F2_FIMP := "S"
								EndIf
								If SF2->(FieldPos("F2_CHVNFE")) > 0 .And. !lPreDanfe
									SF2->F2_CHVNFE := SubStr(NfeIdSPED(aXML[nX][2],"Id"),4)
								EndIf
								If SF2->(FieldPos("F2_HAUTNFE")) > 0 .and. SF2->(FieldPos("F2_DAUTNFE")) > 0 .And. !lPreDanfe //grava a data e hota de autorização da NFe
								...
								IF SF2->(FieldPos("F2_FILDEST"))> 0 .And. SF2->(FieldPos("F2_FORDES"))> 0 .And.SF2->(FieldPos("F2_LOJADES"))> 0 .And.SF2->(FieldPos("F2_FORMDES"))> 0 .And. !EMPTY (SF2->F2_FORDES)
							       SF1->(dbSetOrder(1))
							    	If SF1->(MsSeek(SF2->F2_FILDEST+SF2->F2_DOC+SF2->f2_SERIE+SF2->F2_FORDES+SF2->F2_LOJADES+SF2->F2_FORMDES)) .And. !lPreDanfe
							    	...
							    	Endif
							    EndiF
							ElseIf MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. cModalidade == "7" ) .And. !lPreDanfe // Contingencia FSDA
								RecLock("SF2")
								SF2->F2_CHVNFE := SubStr(NfeIdSPED(aXML[nX][2],"Id"),4)
								MsUnlock()
							EndIf
							// Atualização dos campos da Tabela GFE
							if FindFunction("GFECHVNFE") .and. lMVGfe .And. !lPreDanfe  // Integração com o GFE
						...
						dbSelectArea("SFT")
						dbSetOrder(1)
						If SFT->(FieldPos("FT_CHVNFE"))>0 .And. !lPreDanfe
						...
						IF SF1->(!EOF()) .And. SF2->(FieldPos("F2_FILDEST"))> 0 .And. SF2->(FieldPos("F2_FORDES"))> 0 .And.SF2->(FieldPos("F2_LOJADES"))> 0 .And.SF2->(FieldPos("F2_FORMDES"))> 0 .And. !EMPTY (SF2->F2_FORDES)
							...
							If SFT->(MsSeek(cChaveSFT)) .And. !lPreDanfe
						...
						//-----------------------------------------------------------------------
						oRetNF := XmlParser(StrTran(aXML[nX][2],"&","E"),"_",@cAviso,@cErro)
						...
						EndIf
						If Empty(cAviso) .And. Empty(cErro)
							//Alert("Vai tentar imprimir")
							ImpDet(@oDanfe,oNFe,cAutoriza,cModalidade,oNfeDPEC,cCodAutDPEC,aXml[nX][6],aXml[nX][7],aNotas[nX],lImpSimp,nTipo,aXml[nX][11])
							lExistNfe := .T.
						Else
							Alert("Ocorreu um problema na leitura do XML. " + If(Empty(cErro),"Aviso: " + Trim(cAviso),"Erro: "+Trim(cErro)))
						Endif
						oNfe     := nil
						oNfeDPEC := nil
...
If ntipo <> 1
	...
	PRIVATE oFont17N   := TFontEx():New(oDanfe,"Times New Roman",16,16,.T.,.T.,.F.)// 12
...
//³Calculo do Imposto                                                      ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
XmlNewNode(oTotal:_ICMSTOT,'_VBC'   ,'VBC'   ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_VICMS' ,'VICMS' ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vBCST' ,'vBCST' ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vST'   ,'vST'   ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vProd' ,'vProd' ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vFrete','vFrete','NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vSeg'  ,'vSeg'  ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vDesc' ,'vDesc' ,'NOD')
XmlNewNode(oTotal:_ICMSTOT,'_vOutro','vOutro','NOD')
...
If lPreDanfe
	oDanfe:Say(055,285, "PRE",oFont18N:oFont)
	oDanfe:Say(070,255, "NOTA FISCAL",oFont17N:oFont)
Else
...
If Empty(cCodAutDPEC)
	If Empty(cChaveCont)
		If lPreDanfe
			oDanfe:Say(130,405,"SEM VALOR FISCAL",oFont18N:oFont)
		Else
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lPreDanfe
				oDanfe:Say(013+nLinhavers,285, "PRE",oFont18N:oFont)
				oDanfe:Say(028+nLinhavers,255, "NOTA FISCAL",oFont17N:oFont)
			Else
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lPreDanfe
			oDanfe:Say(013,285, "PRE",oFont18N:oFont)
			oDanfe:Say(028,255, "NOTA FISCAL",oFont17N:oFont)
		Else
...
If lPreDanfe
	oDanfe:Say(055,285, "PRE",OFONT18N:oFont)
	oDanfe:Say(070,258, "NOTA FISCAL",OFONT17N:oFont)
Else
...
Static Function ConverteXML()
	Local aNFe, cFile, nX, nY, aField
	Local aSD2    := SD2->(GetArea())
	Local nDesc   := 0
	Local cArq, cPath, nHandle //, cLine
	Local lGrava  := .F.
	Local aTroca  := {}
	Local aInclui := {}
	//Local lCabec  := .T.
	Local aLinhas := {}
	Local nDec    := TamSX3("F2_VALMERC")[2]
	...
	SD2->(dbSetOrder(3))
	SD2->(dbSeek(SF2->F2_FILIAL+SF2->F2_DOC+SF2->F2_SERIE,.T.))
	While !SD2->(Eof()) .And. SD2->D2_FILIAL+SD2->D2_DOC+SD2->D2_SERIE == SF2->F2_FILIAL+SF2->F2_DOC+SF2->F2_SERIE
		nDesc += SD2->D2_DESCON + SD2->D2_DESCZFC + SD2->D2_DESCZFP
		SD2->(dbSkip())
	Enddo
	
	// Adiciona as tags a serem alteradas para compatibilizar com o XML da SEFAZ
	AAdd( aTroca , {"Lgr>","xLgr>"})
	AAdd( aTroca , {"Bairro>","xBairro>"})
	AAdd( aTroca , {"Ender>","xEnder>"})
	AAdd( aTroca , {"Mun>","xMun>"})
	AAdd( aTroca , {"cxMun>","cMun>"})
	AAdd( aTroca , {"Pais>","xPais>"})
	AAdd( aTroca , {"cxPais>","cPais>"})
	AAdd( aTroca , {"tpNFe>","finNFe>"})
	AAdd( aTroca , {"Fant>","xFant>"})
	AAdd( aTroca , {"Nome>","xNome>"})
	AAdd( aTroca , {"Prod>","xProd>"})
	AAdd( aTroca , {"cxProd>","cProd>"})
	AAdd( aTroca , {"vxProd>","vProd>"})
	AAdd( aTroca , {"<ICMSCONS>",""})
	AAdd( aTroca , {"</ICMSCONS>",""})
	AAdd( aTroca , {"vICMSST>","vST>"})
	AAdd( aTroca , {"pagamento>","pag>"})
	AAdd( aTroca , {"Fisco>","infAdFisco>"})
	AAdd( aTroca , {"Cpl>","infCpl>"})
	AAdd( aTroca , {"Contato>","xContato>"})
	AAdd( aTroca , {"indForma>","tPag>"})
	AAdd( aTroca , {"<Dup>","<nDup>"})
	AAdd( aTroca , {"</Dup>","</nDup>"})
	AAdd( aTroca , {"dtVenc>","dVenc>"})
	AAdd( aTroca , {'<infNFe versao="T01.00">','<infNFe versao="T01.00" Id="'+Replicate("9",44)+'">'})
	
	// Novos elementos a serem adicionados no XML
	AAdd( aInclui, { {"<dhSaiEnt>","<dhEmi>"},"<tpEmis>1</tpEmis>", 1, .T.})
	AAdd( aInclui, { "<total>", "<ICMSTot>", 1, .T.})
	AAdd( aInclui, { "</total>", "</ICMSTot>", 0, .T.})
	AAdd( aInclui, { "<vNF>", "<vProd>"+LTrim(Str(SF2->F2_VALMERC+If(SF2->F2_VALBRUT==SF2->F2_VALMERC,SF2->F2_DESCONT+SF2->F2_DESCZFR,0),10,nDec))+"</vProd>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vSeg>0</vSeg>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vDesc>"+LTrim(Str(nDesc,10,nDec))+"</vDesc>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vFrete>0</vFrete>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vII>0</vII>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vIPI>0</vIPI>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vIPIDevol>0</vIPIDevol>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vPIS>0</vPIS>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vCOFINS>0</vCOFINS>", 0, .F.})
	AAdd( aInclui, { "<vNF>", "<vOutro>0</vOutro>", 0, .F.})
	AAdd( aInclui, { "<tPag>", "<vPag>"+LTrim(Str(SF2->F2_VALFAT,10,nDec))+"</vPag>", 0, .T.})
	
	SD2->(RestArea(aSD2))
	
	MsgRun("Gerando XML da nota...","Aguarde...",{|| aNFe := ExecBlock("XmlNfeSef",.F.,.F.,{{"1","",SF2->F2_SERIE,SF2->F2_DOC,SF2->F2_CLIENTE,SF2->F2_LOJA,{}},"4.00","4.00",{"",""}}) })
	
	cFile := aNfe[2]
	
	// Lê o arquivo texto gerado para processar sua impressão
	...
	While !FT_FEOF()
		...
		cFile += cLine
		If lCabec
			cFile := StrTran(cFile,'><infNFe','><nfeProc versao="4.00" xmlns="http://www.portalfiscal.inf.br/nfe">'+;
	...
	If !Empty(cFile)
		// Adiciona os fechamentos de tags incluindo durante o processamento
		cFile += '</NFe>'
		cFile += '</nfeProc>'
		
		// Substitui as tags do XML montado pelo XML padrão da SEFAZ
		aEval( aTroca , {|x| cFile := StrTran(cFile,x[1],x[2]) } )
		
		// Quebra o XML em um vetor para adicionar ou retirar novas tags
		While (nPos := At("><",cFile)) > 0
			AAdd( aLinhas , SubStr(cFile,1,nPos) )
			cFile := SubStr(cFile,nPos+1,Len(cFile))
		Enddo
		
		AddElemento(@aLinhas,2,'<NFe xmlns="http://www.portalfiscal.inf.br/nfe">')
		AddElemento(@aLinhas,2,'<nfeProc versao="4.00" xmlns="http://www.portalfiscal.inf.br/nfe">')
		
		// Adiciona novos elementos ao vetor do XML
		For nX:=1 To Len(aInclui)
			aField := If( ValType(aInclui[nX,1]) <> "A" , { aInclui[nX,1] }, aInclui[nX,1] )
			For nY:=1 To Len(aField)
				If (nPos := AScan( aLinhas , aField[nY] )) > 0 .And. (!aInclui[nX,4] .Or. AScan( aLinhas , aInclui[nX,2] ) == 0)
					AddElemento(@aLinhas,nPos + aInclui[nX,3],aInclui[nX,2])
					Exit
				Endif
			Next
		Next
		
		AAdd( aLinhas , cFile )   // Adiciona a última linha do XML
		
		ConvImposto(@aLinhas)    // Converte tags para o modelo SEFAZ
		...
		// Monta o XML novamente
		cFile := ""
		aEval( aLinhas , {|x| cFile += x , If(lGrava,FWrite(nHandle, x + CRLF),) } )
...
Static Function ConvImposto(aLinhas)
	Local nFim, nL, nX, cCab, nLin, aImp, cCST, nMin, nMax, nPII, nPIF
	Local nIni  := 0
	Local aItem := {}
	
	// Identifica a quantidade de itens existentes na NFe
	While (nIni := AScan( aLinhas , {|x| "<det " $ x } , nIni+1 ) ) > 0
		AAdd( aItem , aLinhas[nIni] )
	Enddo

	For nL:=1 To Len(aItem)
		nPII := AScan( aLinhas , aItem[nL] )          // Posição inicial da tag detalhes do item
		nPIF := AScan( aLinhas , "</det>" , nPII )    // Posição final da tag detalhes do item
		nMin := 999999
		nMax := 0
		
		// Enquanto houver tag de imposto modifica para o layout da SEFAZ
		While (nIni := AScan( aLinhas , "<imposto>", nPII )) > 0 .And. nIni < nPIF
			aImp := {}
			nLin := 2
			cCab := ""
			cCST := ""
			nFim := AScan( aLinhas , "</imposto>" , nPII )
			nMin := Min(nMin,nIni)   // Calcula a linha inicial dos impostos
			
			// Monta as tags do modelo SEFAZ
			For nX:=nIni+1 To nFim-1
				If "<codigo>" $ aLinhas[nX]
					cCab := StrTran(StrTran(aLinhas[nX],"<codigo>",""),"</codigo>","")
					AAdd( aImp , "<"+cCab+">" )
				ElseIf "cpl>" $ aLinhas[nX] .Or. "Tributo>" $ aLinhas[nX]
				ElseIf "aliquota>" $ aLinhas[nX]
					AAdd( aImp , StrTran(aLinhas[nX],"aliquota>","p"+cCab+">") )
				ElseIf "valor>" $ aLinhas[nX]
					AAdd( aImp , StrTran(aLinhas[nX],"valor>","v"+cCab+">") )
				Else
					AAdd( aImp , aLinhas[nX] )
				Endif
				If "<CST>" $ aLinhas[nX]
					cCST := StrTran(StrTran(aLinhas[nX],"<CST>",""),"</CST>","")
				Endif
				nLin++
			Next
			
			AAdd( aImp , "</"+cCab+">" )   // Adiciona o fim da tag
			
			If cCab == "ICMS"
				AddElemento(@aImp,Len(aImp),"</"+cCab+cCST+">")
				AddElemento(@aImp,2,"<"+cCab+cCST+">")
			ElseIf cCab $ "PIS,COFINS"
				AddElemento(@aImp,Len(aImp),"</"+cCab+"Outr>")
				AddElemento(@aImp,2,"<"+cCab+"Outr>")
			Endif
			
			// Retira as tags do modelo antigo
			For nX:=1 To nLin
				aDel( aLinhas , nIni )
				aSize( aLinhas , Len(aLinhas)-1 )
			Next
			
			// Adiciona as tags do modelo SEFAZ
			For nX:=Len(aImp) To 1 Step -1
				AddElemento(@aLinhas,nIni,aImp[nX])
			Next
			
			nPIF := AScan( aLinhas , "</det" , nPII )            // Posição final da tag detalhes do item
			nMax := Max(nMax,AScan(aLinhas,"</"+cCab+">",nPII))  // Calcula a linha final dos impostos
		Enddo
		
		AddElemento(@aLinhas,nMax+1,"</imposto>")
		AddElemento(@aLinhas,nMin+0,"<imposto>")
		
		// Zera o valor do ICMS caso a base esteja zerada
		If (nIni := AScan( aLinhas , "<vBC>" , nPII )) > 0 .And. Val(StrTran(StrTran(aLinhas[nIni],"<vBC>",""),"</vBC>","")) == 0
			If (nIni := AScan( aLinhas , "<vICMS>" , nPII )) > 0
				aLinhas[nIni] := "<vICMS>0</vICMS>"
			Endif
		Endif
	Next
...
Static Function AddElemento(aLinhas,nPos,xValor)
	aSize( aLinhas , Len(aLinhas) + 1)
	AIns( aLinhas , nPos )
	aLinhas[nPos] := xValor
Return
...
Static Function ValidPerg(cPerg)
	u_AMPutSX1(cPerg,"01",PADR("Numero da Nota",29)+"?","","","mv_ch1","G", TamSX3("F2_DOC"  )[1],0,0,"G","","   ","","","mv_par01")
	u_AMPutSX1(cPerg,"02",PADR("Serie da Nota ",29)+"?","","","mv_ch2","G", TamSX3("F2_SERIE")[1],0,0,"G","","   ","","","mv_par02")
Return
